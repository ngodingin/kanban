...
| ID | Status | CL | % | Prior | Goal Description | Reference | Dependency |
|---|:--:|:--:|:--:|:--:|---|---|---|
| 1.4.1 | ✅ | [CL-24](#cl-24)<br>[CL-23](#cl-23)<br>[CL-14](#cl-14)<br>[CL-13](#cl-13)<br>[QA-CL-07](#qa-cl-07)<br>[QA-CL-12](#qa-cl-12) | 100 | P0 | `POST /api/v1/projects/:project_id/archive` — otorisasi Owner-only interim, `expected_version` wajib, panggil `archiveProject` (1.1) | [02-SPEC C.4](docs/02-SPEC.md), A.3 | 1.1, 1.3.3 |
| 1.4.2 | ✅ | [CL-24](#cl-24)<br>[CL-23](#cl-23)<br>[CL-16](#cl-16)<br>[CL-15](#cl-15)<br>[QA-CL-08](#qa-cl-08)<br>[QA-CL-12](#qa-cl-12) | 100 | P0 | `POST /api/v1/projects/:project_id/restore` — hanya valid dari ARCHIVED (Project tidak punya ancestor lain sehingga INV-LIFE-002 trivially satisfied di level Project), otorisasi Owner-only interim, panggil `restoreProject` (1.1) | [02-SPEC C.4](docs/02-SPEC.md), INV-LIFE-002 | 1.1, 1.3.3 |
| 1.4.3 | ✅ | [CL-24](#cl-24)<br>[CL-23](#cl-23)<br>[CL-18](#cl-18)<br>[CL-17](#cl-17)<br>[QA-CL-09](#qa-cl-09)<br>[QA-CL-12](#qa-cl-12) | 100 | P0 | `POST /api/v1/projects/:project_id/delete` — terminal, tidak dapat direstore setelahnya, otorisasi Owner-only interim, panggil `deleteProject` (1.1) | [02-SPEC C.4](docs/02-SPEC.md), INV-LIFE-004 | 1.1, 1.3.3 |
...
- ~~`[NEEDS-DECISION]` D.4 — formula otorisasi `card.comment.update` tidak ada (Review-CL-03 poin 1, sebagian)~~ → **DISELESAIKAN 2026-08-22 (manusia):** `card.comment.update` MUST scoped ke komentar milik actor sendiri, berlaku mutlak termasuk Owner (business invariant, bukan grant Group). Ditambahkan **BR-034A** + formula `can_comment_update` di D.4; SOT dinaikkan ke 2.2.0 melalui Review-CL-05. **Catatan:** ini menyelesaikan pertanyaan "apa yang dilakukan permission ini", TAPI **belum menyelesaikan** pertanyaan "siapa dapat permission ini secara default di D.2" (Review-CL-03 poin 1 bagian `card.restore`/`member.read`/`permission_group.read`/`api_key.read` tetap backlog terbuka) — manusia menilai itu bukan hal mendesak karena Permission Group cuma paket default yang bisa diubah Owner kapan saja setelah TASK-1.7/1.8 CRUD tersedia (bukan capability yang hilang, cuma default assignment).
