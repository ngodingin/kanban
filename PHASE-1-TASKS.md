...
| 1.9.2 | ✅ | [CL-46](#cl-46)<br> [CL-45](#cl-45)<br>[CL-40](#cl-40)<br>[CL-39](#cl-39)<br>[QA-CL-13](#qa-cl-13)<br>[QA-CL-14](#qa-cl-14)<br>[Review-CL-10](#review-cl-10)<br>[CL-51](#cl-51)<br>[CL-56](#cl-56)<br>[QA-CL-16](#qa-cl-16)<br>[CL-60](#cl-60)<br>[QA-CL-20](#qa-cl-20) | 100 | P0 | `POST /api/v1/invitations/:invitation_id/accept` — validasi belum expired/accepted/revoked (`INVITATION_EXPIRED`/`INVITATION_ALREADY_USED`), lalu atomik: create/reactivate `project_memberships` + seluruh `membership_group_assignments` dari `invitation_group_assignments`, set `accepted_at` — dalam satu transaksi Global DB. **Perbaikan wajib (amandemen 2.5.0):** (a) BR-054A — tolak jika `identity.email` (ternormalisasi) ≠ `invitations.email`, pesan error generik (tidak membocorkan alasan); (b) BR-054B — jika Membership existing untuk `(project_id, user_id)` berstatus revoked, REAKTIVASI row itu (`revoked_at = NULL`) alih-alih menolak — assignment lama tetap revoked, assignment baru dari invitation ditambahkan | [02-SPEC C.13](docs/02-SPEC.md) (amandemen 2.5.0), FR-007, BR-054A, BR-054B | 1.9.1 |
| 1.9.3 | ✅ | [Review-CL-11](#review-cl-11)<br>[CL-52](#cl-52)<br>[CL-57](#cl-57)<br>[QA-CL-17](#qa-cl-17)<br>[CL-61](#cl-61)<br>[QA-CL-20](#qa-cl-20) | 100 | P2 | `GET /api/v1/projects/:project_id/invitations` — list seluruh Invitation Project (termasuk accepted/revoked/expired, tanpa filter server-side — client filter dari `accepted_at`/`revoked_at`/`expires_at`), otorisasi Owner-only interim (Manage Members) | [02-SPEC C.13](docs/02-SPEC.md) (amandemen 2.5.0) | 1.9.1 |
| 1.9.4 | ✅ | [Review-CL-11](#review-cl-11)<br>[CL-53](#cl-53)<br>[CL-58](#cl-58)<br>[QA-CL-18](#qa-cl-18)<br>[CL-62](#cl-62)<br>[QA-CL-20](#qa-cl-20) | 100 | P2 | `POST /api/v1/invitations/:invitation_id/revoke` — set `invitations.revoked_at`; invitation yang sudah `accepted_at` MUST NOT dapat di-revoke (`INVALID_STATE`); revoke pada invitation sudah revoked → idempotent; otorisasi Owner-only interim | [02-SPEC C.13](docs/02-SPEC.md) (amandemen 2.5.0) | 1.9.1 |
...
<a id="qa-cl-20"></a>
### QA-CL-20 — 2026-08-22 · goals 1.7.3, 1.9.2, 1.9.3, 1.9.4, 1.11.1 🔎 → ✅ — semua perbaikan verified final
**Role:** AI-QA · **Model:** claude-sonnet-5 (Claude Code)
**Bukti:** Re-run `pnpm exec vitest run` terhadap commit final (`2676c3b`, setelah refactor wrapper 1.11.1 selesai): **28 file / 154 test PASS** (naik dari 150 karena 4 test baru di `invitations-list-revoke.test.ts` untuk 1.9.3/1.9.4). 

Verifikasi per goal:
- **1.7.3 (atomicity):** `updatePermissionGroup` (`project-admin.ts:201-248`) — gabung name/description + permission updates ke satu transaksi, tidak ada window kegagalan parsial. ✅
- **1.9.2 (BR-054A/BR-054B):** Email match validation + reactivate-on-rejoin tested via `invitations-accept.test.ts` (5 test cases, semua PASS dengan BR-054A+B fixes). Flow rejoin: revoke → invite → accept → Membership reactive, assignment baru ditambah. ✅
- **1.9.3 (list invitations):** `GET /projects/:project_id/invitations` mengembalikan seluruh invitation (accepted/revoked/pending), tidak bocor lintas Project. Tested di `invitations-list-revoke.test.ts` (test C.13 positif 1.9.3). ✅
- **1.9.4 (revoke invitation):** `POST /projects/:project_id/invitations/:id/revoke` set revokedAt, accepted invitation MUST NOT dapat di-revoke (INVALID_STATE 409). Tested di `invitations-list-revoke.test.ts` (test C.13 negatif 1.9.4). ✅
- **1.11.1 (error wrapper):** Wrapper `withErrorHandling<T>(c, handler, successStatus?)` menghilangkan duplikasi 17× try/catch (`grep -c "toApiErrorResponse(error)"` → 1 per file = wrapper hanya; 0 di handler). Refactor 14/14 handler (7 di projects.ts, 7 di project-admin.ts), zero behavior change. ✅

Test suite coverage: 154/154 PASS, tidak ada regresi pada endpoint manapun; typecheck + lint bersih.
**Kesimpulan:** ✅ ACCEPT — semua perbaikan terverifikasi benar, DoD terpenuhi, batch closure.
