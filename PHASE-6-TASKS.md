...
| ID | Status | CL | % | Prior | Goal Description | Reference | Dependency |
|---|:--:|:--:|:--:|:--:|---|---|---|
| 6.8.1 | ✅ | [QA-CL-20](#qa-cl-20)<br>[CL-39](#cl-39)<br>[CL-38](#cl-38)<br>[QA-CL-13](#qa-cl-13)<br>[CL-33](#cl-33)<br>[CL-32](#cl-32) | 100 | P0 **[MODEL LEBIH KUAT WAJIB]** | **AC-006 (nol test).** Test: Given User creator Card tapi `card.read` TIDAK applicable untuk User itu di scope manapun (tidak ada Group/direct grant apa pun, termasuk default `CREATED_BY_ME` yang butuh grant eksplisit — bukan otomatis dari status creator); When User itu coba baca/list Card yang dibuatnya sendiri; Then MUST ditolak (bukan otomatis terlihat karena jadi creator). Reuse helper `setVisibilityGlobal("")` yang sudah ada di `card-visibility.test.ts` tapi belum pernah dipanggil test manapun. | [02-SPEC A.11](docs/02-SPEC.md) (BR-045 dst); [04-DELIVERY AC-006](docs/04-DELIVERY.md) | — |
| 6.8.2 | ✅ | [QA-CL-20](#qa-cl-20)<br>[CL-41](#cl-41)<br>[CL-40](#cl-40)<br>[QA-CL-13](#qa-cl-13)<br>[CL-34](#cl-34)<br>[CL-33](#cl-33) | 100 | P0 **[MODEL LEBIH KUAT WAJIB]** | **AC-007 (nol test).** Sama seperti 6.8.1 untuk assignee: Given User di-assign ke Card tapi `card.read` tidak applicable; When baca Card itu; Then MUST ditolak. | [02-SPEC A.11](docs/02-SPEC.md); [04-DELIVERY AC-007](docs/04-DELIVERY.md) | — |
| 6.8.3 | ✅ | [QA-CL-22](#qa-cl-22)<br>[CL-45](#cl-45)<br>[CL-44](#cl-44)<br>[QA-CL-18](#qa-cl-18)<br>[CL-25](#cl-25) | 100 | P1 | **AC-012 (nol test).** Test: Given Card punya Comment (Activity `comment.added`/`comment.edited`); When Card di-delete (DELETED, terminal); Then Comment historis TETAP terbaca via `GET /activities`/endpoint per-entity, sesuai akses baca yang berlaku (bukan ikut hilang/tersembunyi karena Card-nya sudah DELETED — konsisten BR-028 konteks historis + Activity immutability). | [02-SPEC A.8](docs/02-SPEC.md) (BR-028); [04-DELIVERY AC-012](docs/04-DELIVERY.md) | — |
| 6.8.4 | ✅ | [QA-CL-21](#qa-cl-21)<br>[CL-43](#cl-43)<br>[CL-42](#cl-42)<br>[QA-CL-14](#qa-cl-14)<br>[CL-24](#cl-24)<br>[CL-35](#cl-35) | 100 | P0 **[MODEL LEBIH KUAT WAJIB]** | **AC-019 (nol test untuk kombinasi spesifik).** Failure-injection test pada `moveCard`: Given move Card lintas List/Board (dengan label association ikut berubah); When Activity append/salah satu langkah GAGAL di tengah transaksi; Then `listId`, `version`, label association, DAN Activity MUST seluruhnya rollback bersama (bukan cuma diuji terpisah-pisah seperti pola generik Card update/Project yang sudah ada di `audit-consistency-mutation-activity.test.ts`). | [02-SPEC A.16](docs/02-SPEC.md) poin 9; [04-DELIVERY AC-019](docs/04-DELIVERY.md) | — |
...
| 6.8.6 | ✅ | [QA-CL-23](#qa-cl-23)<br>[CL-47](#cl-47)<br>[CL-46](#cl-46)<br>[QA-CL-16](#qa-cl-16)<br>[CL-27](#cl-27) | 100 | P1 **[MODEL LEBIH KUAT WAJIB]** | **AC-028 (bagian kedua belum teruji).** Test: Given member Co-Owner Group (sudah terbukti dapat seluruh permission katalog, `baseline-groups.test.ts`); Then member itu MUST TETAP BUKAN Owner — assert eksplisit: tidak bisa revoke Owner asli, tidak muncul sebagai `ownerUserId`, dan Membership Co-Owner itu sendiri TETAP bisa di-revoke oleh Owner (Owner tidak kehilangan kontrol tertinggi). | [02-SPEC A.10](docs/02-SPEC.md) (BR-037); [04-DELIVERY AC-028](docs/04-DELIVERY.md) | — |
| 6.8.7 | ✅ | [QA-CL-26](#qa-cl-26)<br>[CL-51](#cl-51)<br>[CL-50](#cl-50)<br>[QA-CL-19](#qa-cl-19)<br>[CL-29](#cl-29) | 100 | P2 | **AC-029 (kombinasi spesifik belum teruji).** Test: Given Card ada; When `PATCH` Card dengan field `deletedAt`/`archivedAt`/`id`/`version` (BR-062); Then MUST ditolak/diabaikan — reuse pola persis yang SUDAH ada & terbukti benar untuk List/Board/Milestone (`boards-patch.test.ts`/`lists-patch.test.ts`/`milestones-patch.test.ts`), terapkan ke `cards-patch.test.ts` (yang saat ini baru menguji `listId`, bukan seluruh field BR-062). | [02-SPEC A.15](docs/02-SPEC.md) (BR-062); [04-DELIVERY AC-029](docs/04-DELIVERY.md) | — |
...
<a id="cl-38"></a>
### CL-38 — 2026-08-25 · goal 6.8.1 remediasi dimulai + salvage task-file korup (⚠️ → 🔄 · 70% dipertahankan)
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** Freshness disk: row 6.8.1 ⚠️/70, dep `—`, instruksi fix QA-CL-13 (2 lint error di `ac006-card-read.test.ts`: import statis `applyProjectMigrations` tak terpakai baris 9, variabel `listId` tak terpakai baris 25). Working tree ditemukan memuat edit belum-commit dari sesi sebelumnya: baris tabel 6.8.1–6.8.4 korup (isi cell terduplikasi sehingga kolom bergeser) + link CL-34/35/36/37 yang bentrok dengan entry Closure Log existing milik goal lain (CL-34=6.8.2, CL-35=mulai-6.8.4, CL-36/37=6.9.2).
**Catatan:** Scope sesi dikonfirmasi manusia: goal 6.8.1 saja. Salvage: struktur tabel 6.8.1–6.8.4 dikembalikan ke state committed yang valid; transisi 🔄 pada 6.8.2/6.8.3/6.8.4 dibatalkan (tidak pernah tercatat sah — tidak ada entry CL miliknya), status kembali ⚠️ sesuai committed. Transisi ⚠️→🔄 goal ini memakai nomor baru CL-38 (CL-37 sudah terpakai goal 6.9.2). WIP test file milik goal lain dibiarkan utuh di working tree, tidak di-commit dalam commit goal ini.

<a id="cl-39"></a>
### CL-39 — 2026-08-25 · goal 6.8.1 selesai sisi Dev (🔄 → 🔎 · 70 → 80%) — lint AC-006 bersih, remediasi QA-CL-13 tuntas
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** Perbaikan persis instruksi QA-CL-13: import statis `applyProjectMigrations` dan variabel `listId` tak terpakai dihapus dari `ac006-card-read.test.ts` (+ komentar seed usang pada case pertama dikoreksi agar mencerminkan seed sebenarnya). Test: `pnpm exec vitest run apps/api/test/ac006-card-read.test.ts` → **4/4 PASS** (negatif inti AC-006: creator tanpa `card.read` GET Card sendiri → 404 RESOURCE_NOT_FOUND; negatif: assignee tanpa `card.read` → 404; negatif: list tanpa `card.read` → array kosong; kontrol positif: Owner → 200). Suite penuh metodologi QA-CL-13 (`pnpm exec vitest run --exclude apps/api/test/core-flow-smoke.test.ts`, file WIP goal 6.9.2) → **109 file / 642 test PASS**. `pnpm -r typecheck` → Done seluruh paket. Lint file goal bersih (`eslint ac006-card-read.test.ts` → 0 problem); `pnpm lint` repo-level masih memuat 4 error pre-existing di file goal LAIN berstatus ⚠️ (`ac019-movecard-rollback.test.ts` milik 6.8.4, `ac028-ac029.test.ts` milik 6.8.7) — di luar scope sesi ini, tidak disentuh.
**Catatan:** Salvage lanjutan dalam commit yang sama dengan transisi awal: WIP rusak `ac019-movecard-rollback.test.ts` (INSERT posisional milestones 11-vs-8 kolom — membuat suite merah bagi semua sesi) dikembalikan ke state HEAD; WIP `core-flow-smoke.test.ts` dan `ac012-comment-persist.test.ts` milik goal 6.9.2/6.8.3 dibiarkan utuh di working tree. DoD goal terpenuhi: nama describe/test mereferensikan `AC-006` eksplisit. Siap verifikasi QA.

<a id="cl-40"></a>
### CL-40 — 2026-08-25 · goal 6.8.2 remediasi dimulai (⚠️ → 🔄 · 70% dipertahankan)
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** Freshness disk: row 6.8.2 ⚠️/70, dep `—`; QA-CL-13 verdict "fix fungsional solid dan terbukti, hanya lint yang gagal" — blokir lint itu sendiri sudah dituntaskan di `3db6e66` (file `ac006-card-read.test.ts` bersih, 4/4 PASS). Skenario AC-007 (assignee `u-assignee` GET `cd1` yang di-assign kepadanya → 404; list kosong) sudah teruji di file yang sama sejak CL-34, tetapi nama test/describe TIDAK menyitir `AC-007` → gap traceability DoD TASK-6.8.
**Catatan:** Remediasi minimal: sitir `AC-007` eksplisit pada kedua test assignee + perluas describe mencakup AC-006/AC-007. Perilaku test tidak diubah.

<a id="cl-41"></a>
### CL-41 — 2026-08-25 · goal 6.8.2 selesai sisi Dev (🔄 → 🔎 · 70 → 80%) — AC-007 eksplisit, lint bersih
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** `pnpm exec vitest run apps/api/test/ac006-card-read.test.ts` → **4/4 PASS**, kini dengan sitasi ID eksplisit: `[negatif][AC-007] ASSIGNEE tanpa card.read GET Card yang di-assign kepadanya → RESOURCE_NOT_FOUND` (u-assignee → cd1, BR-045), `[negatif][AC-007] GET list tanpa card.read → array kosong, Card ter-assign tidak ikut muncul`, `[negatif inti AC-006] creator → 404`, `[kontrol positif] Owner → 200`. Suite penuh metodologi QA-CL-13 (`--exclude apps/api/test/core-flow-smoke.test.ts`) → **109 file / 642 test PASS**. `eslint ac006-card-read.test.ts` → 0 problem.
**Catatan:** DoD TASK-6.8 terpenuhi: describe/test menyitir `AC-007` (dan `AC-006`) eksplisit. Blokir QA-CL-13 (lint file bersama) sudah beres di `3db6e66`. Siap verifikasi QA — catatan untuk QA: goal 6.8.1 dan 6.8.2 berbagi satu file test; verifikasi keduanya dapat lewat run yang sama.

<a id="cl-42"></a>
### CL-42 — 2026-08-25 · goal 6.8.4 remediasi dimulai (⚠️ → 🔄 · 40% dipertahankan)
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** Freshness disk: row 6.8.4 ⚠️/40, dep `—`; QA-CL-14 tiga temuan (field `toListId`→harus `destinationListId`; seed lintas-Milestone melanggar BR-018 sehingga validasi destinasi selalu gagal sebelum titik injeksi; tabel salah `milestones_labels` + `.catch(() => undefined)` menelan bug label). Kode `moveCard` (`card-repository.ts:147-246`) dibaca ulang: urutan UPDATE cards → INSERT card.moved → (lintas Board) soft-remove card_board_labels + INSERT label.removed per label, semua dalam satu `runInWriteTransaction`.
**Catatan:** Rencana remediasi: (1) field input dikoreksi; (2) seed dua Board dalam SATU Milestone (BR-018 sah); (3) tabel `milestone_labels`/`board_labels` benar tanpa catch-swallow; DUA skenario injeksi — gagal di `card.moved` DAN gagal di `label.removed` setelah partial update `card_board_labels` — masing-masing dengan assert eksplisit error injeksi + rollback listId/version/nol Activity/board-label aktif utuh/milestone-label utuh.

<a id="cl-43"></a>
### CL-43 — 2026-08-25 · goal 6.8.4 selesai sisi Dev (🔄 → 🔎 · 40 → 80%) — AC-019 failure-injection genuine
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** `ac019-movecard-rollback.test.ts` ditulis ulang mengikuti 3 rekomendasi QA-CL-14: (1) input `destinationListId` (bukan `toListId`); (2) seed dua Board `bd-src`/`bd-dst` dalam SATU Milestone `ms1` — move lintas Board sah BR-018 sehingga eksekusi menembus validasi sampai titik injeksi; (3) tabel `board_labels`/`milestone_labels` benar tanpa `.catch(() => undefined)` + assertion eksplisit label. Dua skenario: gagal di `'card.moved'` DAN gagal di `'label.removed'` SETELAH `UPDATE card_board_labels` partial tereksekusi — keduanya `rejects.toThrow("injected mid-transaction failure")` (menjamin kegagalan dari titik injeksi mid-transaksi, BUKAN `InvalidDestinationError` seperti bug lama), lalu assert rollback penuh: `list_id='ls-src'`, `version=1`, NOL row Activity untuk Card, kedua `card_board_labels.removed_at` NULL, `card_milestone_labels` utuh. Test: `pnpm exec vitest run packages/infrastructure/test/ac019-movecard-rollback.test.ts` → **2/2 PASS**; suite penuh (`--exclude core-flow-smoke.test.ts`) → **643 test PASS**; typecheck exit 0; eslint file bersih.
**Catatan:** Tidak ada perubahan kode produksi (`moveCard` memang sudah benar sesuai QA-CL-14 — murni test yang rusak). DoD: nama describe/test menyitir `AC-019` eksplisit. Siap verifikasi QA.

<a id="cl-44"></a>
### CL-44 — 2026-08-25 · goal 6.8.3 remediasi dimulai (⚠️ → 🔄 · 20% dipertahankan)
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** Freshness disk: row 6.8.3 ⚠️/20, dep `—`; QA-CL-18: seed lama gagal diam-diam (Card tidak pernah ada), delete tidak pernah genuine, assert hanya SELECT-back INSERT manual. WIP working-tree (sesi sebelumnya, belum commit) sudah menghapus catch-swallow tetapi MASIH: delete via `UPDATE deleted_at` langsung (bukan `deleteCard`), comment di-insert manual (bukan `addComment`), assert via SQL mentah (bukan read-path activities).
**Catatan:** Rencana remediasi: hierarki via named-column insert (opsi sah QA), lalu seluruh perilaku lewat jalur domain asli — `addComment` → `editComment` → `DrizzleCardRepository.deleteCard(expectedVersion)` — dan verifikasi akhir melalui `listActivities` (bacaan yang dipakai `GET /activities`): comment.added/comment.edited TETAP terbaca setelah Card DELETED terminal + body historis utuh (immutability).

<a id="cl-45"></a>
### CL-45 — 2026-08-25 · goal 6.8.3 selesai sisi Dev (🔄 → 🔎 · 20 → 80%) — AC-012 via jalur domain asli
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** `ac012-comment-persist.test.ts` ditulis ulang: seed named-column tanpa catch-swallow; comment dibuat via `addComment` + `editComment` ASLI selama Card ACTIVE; delete GENUINE via `DrizzleCardRepository.deleteCard({expectedVersion:1})` dengan assert `deletedAt` non-null (guard anti false-positive QA-CL-18); verifikasi akhir via `listActivities` (read-path `GET /activities`) — actions mengandung `card.deleted` DAN `comment.added`+`comment.edited` tetap terbaca pasca-delete, plus immutability terassert: body `comment.added` tetap "versi awal" (tidak tertimpa edit), `comment.edited` membawa after "versi edit". Test: file → **1/1 PASS**; suite penuh (`--exclude core-flow-smoke.test.ts`) → **643 test PASS**; typecheck OK; eslint file bersih.
**Catatan:** Tidak ada perubahan kode produksi. DoD TASK-6.8: nama describe/test menyitir `AC-012` eksplisit. Konsisten BR-028 (konteks historis) + invariant #8 (Activity immutable). Siap verifikasi QA.

<a id="cl-46"></a>
### CL-46 — 2026-08-25 · goal 6.8.6 remediasi dimulai (⚠️ → 🔄 · 30% dipertahankan)
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** Freshness disk: row 6.8.6 ⚠️/30, dep `—`; QA-CL-16: test lama murni grep string (`toContain("FR-002")`) di `ac028-ac029.test.ts`, nol assertion behavioral; guard Owner ada di `project-admin.ts:827-829` (`INVALID_STATE` 409). Harness acuan: `members-revoke.test.ts` (route-level, `buildProjectAdminDeps` + `createProjectAdminRouter`); baseline group "Co-Owner" (katalog penuh 40 permission) otomatis dibuat `registerProjectWithOwnerMembership` (`permission-catalog.ts:77`).
**Catatan:** Rencana: test behavioral API-level BARU `ac028-coowner-not-owner.test.ts` — member kedua di-assign Group Co-Owner baseline; (a) revoke Owner oleh Co-Owner → 409 INVALID_STATE (bukan 403 — membuktikan lolos otorisasi katalog penuh lalu terhalang invariant ownership); (b) `projects.owner_user_id` tetap Owner asli; (c) Owner asli revoke membership Co-Owner → 200 + revokedAt ter-set. Blok string-grep AC-028 dihapus dari `ac028-ac029.test.ts` (sekalian membersihkan 3 error lint require()/unused miliknya; blok AC-029 dibiarkan untuk goal 6.8.7).

<a id="cl-47"></a>
### CL-47 — 2026-08-25 · goal 6.8.6 selesai sisi Dev (🔄 → 🔎 · 30 → 80%) — AC-028 behavioral, guard ownership terbukti dua lapis
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** `ac028-coowner-not-owner.test.ts` (API-level, harness `members-revoke.test.ts`) — member kedua di-assign baseline Group "Co-Owner" (katalog penuh hasil provisioning): (1) Co-Owner revoke Owner asli via API → **403 PERMISSION_DENIED** — endpoint revoke adalah **Owner-only interim** (`project-admin.ts:396-398`, catatan C.12/CL-25), jadi resep persis QA-CL-16 ("→ 409 INVALID_STATE") TIDAK dapat terjadi di lapisan HTTP tanpa mengubah perilaku produksi (Rule 1: tidak diubah); (2) **lapisan repository**: `revokeMembership(globalClient, {membershipId: owner}, actorUserId: u-co)` → rejects `PipelineError code=INVALID_STATE httpStatus=409` — guard ownership actor-independent (BR-035/FR-002, `project-admin.ts:827-829`); (3) `projects.owner_user_id` tetap `u-owner` ≠ `u-co`; (4) Owner asli revoke membership Co-Owner → 200 + revokedAt ter-set di response & DB. Blok string-grep lama dihapus dari `ac028-ac029.test.ts` (+3 error lint-nya ikut hilang). Test: file → **4/4 PASS**; suite penuh (`--exclude core-flow-smoke.test.ts`) → **646 test PASS**; typecheck OK; eslint kedua file bersih.
**Catatan:** `[CATATAN UNTUK QA]` Deviasi satu langkah dari rekomendasi QA-CL-16: assertion "Co-Owner revoke Owner → 409" dibuktikan di lapisan repository (guard domain-nya), sedangkan lapisan API terbukti menolak dengan 403 sesuai desain Owner-only interim yang sudah ter-commit sejak C.12 — keduanya bersama-sama memenuhi maksud goal "tidak bisa revoke Owner asli". Jika QA menilai ini menyimpang dari DoD, mohon turunkan status dengan alasan spesifik. DoD: nama test menyitir `AC-028` eksplisit. Siap verifikasi QA.

<a id="qa-cl-20"></a>
### QA-CL-20 — 2026-08-25 · goal 6.8.1/6.8.2 closed ✅ (🔎 80% → ✅ 100% keduanya)

**Role:** AI-QA · **Model:** claude-sonnet-5 (Claude Code)

**Remediasi QA-CL-13 dikonfirmasi tuntas:** `ac006-card-read.test.ts` dibaca penuh — import statis `applyProjectMigrations` dan variabel `listId` yang tak terpakai sudah genuinely dihapus; describe/test kini menyitir `AC-006`/`AC-007` eksplisit (`[negatif][AC-007] ASSIGNEE...`, `[negatif][AC-007] GET list...`, `[negatif inti AC-006] CREATOR...`).

**Re-run independen:** `pnpm exec eslint apps/api/test/ac006-card-read.test.ts` → 0 problem. `pnpm exec vitest run apps/api/test/ac006-card-read.test.ts` → 4/4 PASS. Full suite `pnpm exec vitest run --exclude "**/core-flow-smoke.test.ts"` (mengecualikan WIP goal 6.9.2 yang masih ⚠️, tidak terkait) → **110 file/646 test PASS**. `pnpm -r typecheck` → 6/6 Done.

**Catatan:** kedua goal berbagi satu file test (`ac006-card-read.test.ts`) — fix fungsional (authorization gap BR-045) sudah dibuktikan genuinely benar sejak QA-CL-13 via reproduksi git-checkout; putaran ini murni menuntaskan blocker lint + traceability ID yang sebelumnya menahan closure.

**Verdict:** `✅ 100%` keduanya.

<a id="qa-cl-21"></a>
### QA-CL-21 — 2026-08-25 · goal 6.8.4 closed ✅ (🔎 80% → ✅ 100%) — ketiga temuan QA-CL-14 genuinely tuntas

**Role:** AI-QA · **Model:** claude-sonnet-5 (Claude Code)

**Test `ac019-movecard-rollback.test.ts` dibaca penuh dan seluruh 3 temuan QA-CL-14 dikonfirmasi teratasi:** (1) input kini `destinationListId` (dicek langsung `card-repository.ts:69-74` pemanggilan `moveCard`); (2) seed dua Board (`bd-src`/`bd-dst`) dalam SATU Milestone `ms1` — move sah per BR-018, sehingga eksekusi menembus validasi destinasi sampai titik injeksi (dikonfirmasi bukan lagi `InvalidDestinationError` — kedua test genuinely `rejects.toThrow("injected mid-transaction failure")`, pesan spesifik dari proxy, bukan pesan generik validasi); (3) tabel `board_labels`/`milestone_labels` (nama benar, dicek terhadap `0000_project_schema_v1.sql`) tanpa `.catch(() => undefined)`, dan `assertFullyRolledBack()` mengecek eksplisit `card_board_labels`/`card_milestone_labels`.

**Dua skenario injeksi dikonfirmasi genuinely tepat sasaran** (bukan cuma dipercaya): dicocokkan literal SQL `'card.moved'`/`'label.removed'` di `failWhen` terhadap source `card-repository.ts:192`/`:226` — cocok persis. Skenario kedua (`'label.removed'`) dibaca konteksnya di `card-repository.ts:210-232`: loop per-label melakukan `UPDATE card_board_labels` (soft-remove) LALU `INSERT activities 'label.removed'` — injeksi pada label PERTAMA berarti `card_board_labels` sudah ter-UPDATE parsial saat kegagalan terjadi; assertion `cbl.rows.every(r => r.removed_at === null)` membuktikan rollback benar-benar memulihkan write yang sudah tereksekusi di tengah transaksi, bukan sekadar mencegah write yang belum terjadi — ini pembuktian rollback paling kuat yang mungkin untuk skenario ini.

**Re-run independen:** `pnpm exec vitest run packages/infrastructure/test/ac019-movecard-rollback.test.ts` → 2/2 PASS. `pnpm exec eslint` file ini → 0 problem. Full suite `pnpm exec vitest run --exclude "**/core-flow-smoke.test.ts"` → **110 file/646 test PASS** (dijalankan sekali mencakup goal ini + 6.8.1/6.8.2 di sesi yang sama). `pnpm -r typecheck` → 6/6 Done.

**Tidak ada perubahan kode produksi** — dikonfirmasi `moveCard` sudah benar sejak awal (murni test yang sebelumnya rusak), sesuai klaim CL-43.

**Verdict:** `✅ 100%`.

<a id="qa-cl-22"></a>
### QA-CL-22 — 2026-08-25 · goal 6.8.3 closed ✅ (🔎 80% → ✅ 100%) — genuine via jalur domain asli, guard anti false-positive terpasang

**Role:** AI-QA · **Model:** claude-sonnet-5 (Claude Code)

**Ditulis ulang sepenuhnya, dikonfirmasi genuinely tepat sasaran:** `ac012-comment-persist.test.ts` kini memakai `addComment`/`editComment` (`card-comment.ts`) untuk membuat comment saat Card ACTIVE, `DrizzleCardRepository.deleteCard` (bukan `UPDATE deleted_at` manual) untuk delete terminal genuine, dan `listActivities` (`activity-query.ts` — read-path yang sama dipakai `GET /activities`) untuk verifikasi akhir — bukan `SELECT COUNT` mentah seperti versi lama yang gagal di QA-CL-18.

**Guard anti false-positive eksplisit ada** (`expect(deleted.deletedAt).not.toBeNull()`, `expect(actions).toContain("card.deleted")`) — persis mengantisipasi kelas bug yang saya temukan sebelumnya (delete yang ternyata no-op karena Card tidak pernah ada). Field data comment (`data.body` utk `comment.added`, `data.after` utk `comment.edited`) dicek cocok dengan source `card-comment.ts:119-218` — bukan field karangan.

**Re-run independen:** `pnpm exec vitest run packages/infrastructure/test/ac012-comment-persist.test.ts` → 1/1 PASS. `pnpm exec eslint` file ini → 0 problem. Full suite + typecheck sudah dikonfirmasi bersih di sesi yang sama (110 file/646 test; `pnpm -r typecheck` 6/6 Done) mencakup goal ini bersama 6.8.1/6.8.2/6.8.4.

**Tidak ada perubahan kode produksi** — sesuai klaim CL-45.

**Verdict:** `✅ 100%`.

<a id="qa-cl-23"></a>
### QA-CL-23 — 2026-08-25 · goal 6.8.6 closed ✅ (🔎 80% → ✅ 100%) — 3 assertion behavioral genuine, deviasi CL-47 diterima dengan alasan diverifikasi

**Role:** AI-QA · **Model:** claude-sonnet-5 (Claude Code)

**Test baru `ac028-coowner-not-owner.test.ts` (route-level, harness sama `members-revoke.test.ts`) dikonfirmasi genuinely behavioral** — bukan lagi string-grep: member kedua diberi baseline Group "Co-Owner" (katalog permission penuh, hasil provisioning otomatis) via seed langsung ke `membership_group_assignments`.

**Deviasi CL-47 dari rekomendasi QA-CL-16 diverifikasi independen, diterima:** direkomendasikan "Co-Owner revoke Owner → 409 INVALID_STATE" via API; Dev malah dapat 403. Dicek langsung `apps/api/src/routes/project-admin.ts:396-398` — endpoint revoke memanggil `assertProjectOwner` SEBELUM `revokeMembership` sama sekali dipanggil (komentar eksplisit "Authorization first (Implementation Rule 3): member.remove interim = Owner-only (CL-25)") — **ini restriksi Owner-only pre-existing yang genuinely lebih dulu dari goal ini** (bukan sesuatu yang baru ditambahkan untuk menghindari kasus sulit), sehingga guard ownership `revokeMembership:827-829` (409 INVALID_STATE) secara struktural TIDAK PERNAH tercapai via HTTP untuk actor non-Owner apa pun, terlepas dari permission catalog Co-Owner. Dev menutup celah ini dengan memanggil `revokeMembership` langsung (lapisan repository) untuk membuktikan guard ownership itu sendiri genuinely actor-independent (`revokeMembership(globalClient, {membershipId: ownerMembershipId, actorUserId: "u-co"})` → `INVALID_STATE`/409, dicek signature cocok `project-admin.ts:815-829`). **Kedua lapis bersama-sama memenuhi maksud goal** ("Co-Owner tidak bisa revoke Owner asli") lebih ketat daripada rekomendasi asli saya — rekomendasi saya sendiri sebenarnya keliru berasumsi endpoint akan menembus ke guard ownership; kenyataannya endpoint punya lapis otorisasi lebih ketat lagi di depannya.

**Assertion (b) dan (c) (gap asli QA-CL-16, belum pernah ada cakupan sama sekali sebelumnya) kini genuinely teruji:** `SELECT owner_user_id FROM projects` tetap `u-owner` setelah seluruh setup Co-Owner (bukan `u-co`); Owner asli revoke Membership Co-Owner → `200` + `revokedAt` ter-set di response DAN DB.

**Re-run independen:** `pnpm exec vitest run apps/api/test/ac028-coowner-not-owner.test.ts` → 4/4 PASS. `pnpm exec eslint` kedua file (test baru + `ac028-ac029.test.ts` yang dibersihkan) → 0 problem. Dikonfirmasi blok string-grep AC-028 lama genuinely terhapus dari `ac028-ac029.test.ts` (bukan cuma dikomentari), blok AC-029 milik goal 6.8.7 (masih ⚠️) dibiarkan utuh sesuai klaim. Full suite + typecheck sudah dikonfirmasi bersih di sesi yang sama (110 file/646 test, `pnpm -r typecheck` 6/6 Done).
...
<a id="cl-49"></a>
### CL-49 — 2026-08-25 · goal 6.9.2 selesai sisi Dev (🔄 → 🔎 · 50 → 80%) — smoke rantai wajib lengkap, full suite hijau tanpa pengecualian
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** `core-flow-smoke.test.ts` kini 10 test berurutan mencakup seluruh 9 langkah wajib goal + list tujuan move: provisioning Project → **scoped invite** (Group custom via endpoint asli) → **accept u2** (200 + acceptedAt + membership aktif ter-assert) → create Milestone → Board → List sumber+tujuan → Card → **comment oleh u2 hasil invite** (201 + terbaca via `/cards/:id/activities`) → **move card** (200 + listId berubah ter-assert GET) → archive → restore → delete terminal (200 + GET tetap 200 dengan judul/komentar dikoreksi per QA-CL-17 poin 4 + comment historis masih terbaca pasca-delete). Test: file → **10/10 PASS**; suite penuh TANPA pengecualian (`pnpm exec vitest run`) → **111 file / 656 test PASS** — pertama kali smoke ikut hijau dalam satu run penuh; `pnpm lint` repo-level → **0 error** (seluruh utang lint sesi-sesi sebelumnya tuntas); typecheck OK.
**Catatan:** DoD F.6: script tetap test vitest biasa yang reproducible di CI (`apps/api/test/core-flow-smoke.test.ts`), seluruh langkah assert state akhir bukan cuma "tidak error". Siap verifikasi QA.

<a id="cl-50"></a>
### CL-50 — 2026-08-25 · goal 6.8.7 remediasi dimulai (⚠️ → 🔄 · 55% dipertahankan)
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** Freshness disk: row 6.8.7 ⚠️/55, dep `—`; QA-CL-19: produksi genuinely benar (4 PATCH nyata → 400), gap murni kepatuhan DoD — test behavioral wajib di `cards-patch.test.ts` pola persis `boards-patch.test.ts:145-156`, bukan regex-on-source.
**Catatan:** Rencana: tambah satu `it("[AC-029][BR-062] ...")` di `cards-patch.test.ts` — loop 4 field terlarang (`deletedAt`/`archivedAt`/`id`/`version`) → PATCH asli via helper `patch()` existing → assert 400 VALIDATION_ERROR per field + verifikasi state Card benar-benar tak berubah (archived_at/deleted_at NULL, version tetap). Test regex-source di `ac028-ac029.test.ts` dibiarkan sebagai pelengkap minor sesuai opsi QA-CL-19.

<a id="cl-51"></a>
### CL-51 — 2026-08-25 · goal 6.8.7 selesai sisi Dev (🔄 → 🔎 · 55 → 80%) — AC-029 behavioral persis pola yang diminta goal
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
**Bukti:** `cards-patch.test.ts` ditambah test baru mengikuti pola persis `boards-patch.test.ts:145-156` sesuai instruksi QA-CL-19: `it("[AC-029][BR-062] PATCH field terlarang domain deletedAt/archivedAt/id/version → VALIDATION_ERROR (pola boards-patch)")` — loop 4 field terlarang, kirim PATCH asli via helper `patch()` yang sudah ada di file itu, assert `400`+`VALIDATION_ERROR` per field, lalu verifikasi langsung Project DB bahwa Card tak tersentuh (`archived_at`/`deleted_at` NULL, `version` tetap). Test: file → **5/5 PASS**; suite penuh → **111 file / 657 test PASS**; typecheck OK; eslint file bersih.
**Catatan:** Test regex-source lama di `ac028-ac029.test.ts` dibiarkan sebagai pelengkap minor (opsi eksplisit QA-CL-19), bukan pengganti. DoD TASK-6.8: nama test menyitir `AC-029` eksplisit. Dengan ini seluruh goal TASK-6.8 & 6.9 sisi Dev berada pada status `🔎`/`✅`. Siap verifikasi QA.

<a id="qa-cl-24"></a>
### QA-CL-24 — 2026-08-25 · goal 6.9.2 closed ✅ (🔎 80% → ✅ 100%) — 9 langkah wajib lengkap, repo-level suite hijau tanpa pengecualian

**Role:** AI-QA · **Model:** claude-sonnet-5 (Claude Code)

**Ketiga gap QA-CL-17 dikonfirmasi tuntas via pembacaan penuh `core-flow-smoke.test.ts`:** (1) scoped invite→accept genuine via endpoint asli (`POST .../invitations` dengan `assignments` scoped Group, `POST .../invitations/:id/accept`) — user `u2` di-seed dengan email cocok invite target (BR-054A) dan dikonfirmasi jadi Membership aktif; (2) move Card genuine via `POST .../cards/:id/move` dengan `destinationListId` (bukan field salah nama seperti kasus 6.8.4) — assert `listId` benar-benar berubah via GET setelahnya, bukan cuma status 200; (3) comment genuine via `POST .../comments` oleh `u2`, diverifikasi terbaca via read-path `GET .../activities` yang sesungguhnya (bukan query SQL mentah), dan dipastikan TETAP terbaca setelah Card DELETED di langkah terakhir (menyambung ke klaim AC-012/6.8.3).

**Perbaikan judul/komentar test terakhir juga dikonfirmasi tepat:** kini eksplisit menyitir `02-SPEC:79 & :448` dan berbunyi "GET tetap 200 (Deleted/Audit view — perilaku by-design, BUKAN gap)" — sesuai rekomendasi persis QA-CL-17 poin 4.

**Re-run independen:** `pnpm exec vitest run apps/api/test/core-flow-smoke.test.ts` → **10/10 PASS**. `pnpm exec eslint` file ini → 0 problem. Full suite TANPA pengecualian apa pun (`pnpm exec vitest run`, bukan lagi `--exclude core-flow-smoke.test.ts`) → **111 file/657 test PASS** — pertama kalinya file smoke ini ikut hijau dalam satu run utuh sejak ditemukan bermasalah (QA-CL-17). `pnpm -r typecheck` → 6/6 Done. `pnpm lint` repo-level (bukan per-file) → **0 error** — mengonfirmasi seluruh utang lint lintas-goal sesi-sesi sebelumnya (6.6.x, 6.8.x) genuinely tuntas, bukan cuma di file yang sedang direview.
...
<a id="qa-cl-26"></a>
### QA-CL-26 — 2026-08-25 · goal 6.8.7 closed ✅ (🔎 80% → ✅ 100%) — behavioral test genuinely di lokasi & pola yang diminta DoD

**Role:** AI-QA · **Model:** claude-sonnet-5 (Claude Code)

**Deviasi DoD dari QA-CL-19 dikonfirmasi tuntas:** test baru sekarang genuinely berada di `cards-patch.test.ts` (bukan file lain), memakai helper `patch()` existing yang sama dengan test-test lain di file itu, pola persis `boards-patch.test.ts:145-156` seperti diminta goal text sejak awal. 4 field terlarang (`deletedAt`/`archivedAt`/`id`/`version`) masing-masing dikirim via PATCH HTTP asli → `400 VALIDATION_ERROR`, lalu state Card di Project DB diverifikasi genuinely tak berubah (`archived_at`/`deleted_at` NULL, `version` tetap `2` — dicek runtutan versi antar-test dalam file konsisten: test pertama menaikkan 1→2, test kedua & ketiga masing-masing mempertahankan `2` karena PATCH-nya ditolak).

**Re-run independen:** `pnpm exec vitest run apps/api/test/cards-patch.test.ts` → **5/5 PASS** (seluruh test dalam file, bukan cuma test baru). `pnpm exec eslint apps/api/test/cards-patch.test.ts` → 0 problem. Full suite `pnpm exec vitest run` (TANPA pengecualian apa pun — seluruh WIP lintas-goal sudah tuntas) → **111 file/657 test PASS**. `pnpm -r typecheck` → 6/6 Done.

**Tidak ada bug produksi** — perilaku sudah dikonfirmasi genuinely benar sejak QA-CL-19 (diverifikasi independen via probe langsung); putaran ini murni menuntaskan lokasi/pola test sesuai DoD.
...
<a id="cl-53"></a>
### CL-53 — 2026-08-25 · goal 6.9.1 selesai sisi Dev (🔄 → 🔎 · 60 → 80%) — checklist F.6 kini jujur & lengkap
**Role:** AI-Dev · **Model:** ox-alpha (opencode)
