...
import { hasPermission, resolveCardVisibilityFilter, resolveEffectivePermissions, type CardReadVisibility, type PermissionHierarchyInput, type ResolveEffectivePermissionsInput, type ScopedDirectPermissionInput, type ScopedGroupAssignmentInput, } from "@kanban/domain";
...
const group = (permissions: readonly GroupEntry[], scopeType: ScopedGroupAssignmentInput["scopeType"], scopeId: string): ScopedGroupAssignmentInput => ({ scopeType, scopeId, permissions });
const keys = (...keys: string[]): GroupEntry[] => keys.map((key) => ({ key }));
const direct = (permissionKey: string, scopeType: ScopedDirectPermissionInput["scopeType"], scopeId: string, cardReadVisibility?: CardReadVisibility): ScopedDirectPermissionInput => ({
    permissionKey,
    scopeType,
    scopeId,
    ...(cardReadVisibility ? { cardReadVisibility } : {}),
});
const resolve = (over: Partial<ResolveEffectivePermissionsInput>) => resolveEffectivePermissions({
    ...
    it("[BR-037][properti] Owner granted SELURUH katalog + visibility ALL, termasuk dengan assignment kosong", () => {
        for (const groups of [[], [group(keys("card.read"), "project", PROJECT)], [group(keys("milestone.create"), "milestone", MS_B)]]) {
            ...
            for (const key of ALL_KEYS)
                expect(effective.grantedKeys.has(key)).toBe(true);
...
describe("hasPermission — helper call site (goal 4.1.2)", () => {
    it("[BR-043] true hanya untuk key yang di-grant; false untuk key lain dan set kosong", () => {
        ...
        const empty = resolve({});
        for (const key of ALL_KEYS)
            expect(hasPermission(empty, key)).toBe(false);
    });
...
describe("resolveCardVisibilityFilter — D.3/BR-047 (goal 4.1.2)", () => {
    ...
    it("[BR-047] ASSIGNED_TO_ME → union OR creator/assignee", () => {
        const filter = resolveCardVisibilityFilter(resolve({ directAssignments: [direct("card.read", "project", PROJECT, "ASSIGNED_TO_ME")] }), ME);
        expect(cards.filter(filter)).toEqual([cards[0], cards[1], cards[3]]);
    });
    it("[BR-048] ALL → semua Card tanpa kecuali (termasuk sumber Group)", () => {
        const filter = resolveCardVisibilityFilter(resolve({
