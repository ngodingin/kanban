...
import { groupPermissions, permissionGroups, permissions, projectDatabases, projectMemberships, projects, } from "../database/global-schema.ts";
import { BASELINE_GROUP_DESCRIPTIONS, BASELINE_GROUP_NAMES, PERMISSION_CATALOG, baselineGroupPermissionKeys, } from "../database/permission-catalog.ts";
import { activities, projectState } from "../database/project-schema.ts";
import { createDatabase, deleteDatabase, mintDatabaseToken, projectDatabaseName, type TursoEnv, } from "./turso.ts";
...
export async function registerProjectWithOwnerMembership(globalClient: Client, input: ProjectRegistrationInput): Promise<void> {
    ...
    await runInDrizzleWriteTransaction(db, async (tx) => {
        ...
        for (const name of BASELINE_GROUP_NAMES) {
            ...
            const rows = baselineGroupPermissionKeys(name).map((key) => ({
                ...
                cardReadVisibility: key === "card.read" ? ("CREATED_BY_ME" as const) : null,
                createdAt: input.now,
            }));
