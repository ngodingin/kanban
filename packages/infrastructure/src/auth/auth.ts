...
async function defaultSendMagicLink(data: SendMagicLinkData): Promise<void> {
    ...
    if (res.error)
        throw new Error(`Resend gagal: ${res.error.message}`);
}
function guardedSendMagicLink(impl: (data: SendMagicLinkData) => Promise<void>): (data: SendMagicLinkData) => Promise<void> {
    return async (data) => {
        try {
            await impl(data);
        }
        catch (error) {
            console.error(`[auth] sendMagicLink gagal (email tidak terkirim, request TETAP dianggap sukses ke client): ${error instanceof Error ? error.message : String(error)}`);
        }
    };
}
