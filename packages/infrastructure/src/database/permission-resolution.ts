...
import type { CardReadVisibility, GroupPermissionEntry, ScopedDirectPermissionInput, ScopedGroupAssignmentInput, } from "@kanban/domain";
...
export async function loadEffectivePermissionInputs(globalClient: Client, membershipId: string): Promise<EffectivePermissionInputs> {
    ...
    const groups = new Map<string, {
        scopeType: string;
        scopeId: string;
        permissions: ScopedGroupAssignmentInput["permissions"];
    }>();
    for (const row of groupRows.rows as unknown as GroupRow[]) {
        const mapKey = `${row.groupId} ${row.scopeType} ${row.scopeId}`;
