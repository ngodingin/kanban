...
import type { CreateMilestoneInput, LifecycleState, MilestoneLifecycleInput, MilestoneRecord, MilestoneRepository, UpdateMilestoneInput, } from "@kanban/domain";
import { AncestorNotActiveError, evaluateRestore, isEffectivelyOperational, resolveLifecycleState, MilestoneInvalidStateError, MilestoneNotFoundError, MilestoneValidationError, MilestoneVersionConflictError, } from "@kanban/domain";
...
export class DrizzleMilestoneRepository implements MilestoneRepository {
    ...
    async getMilestone(projectId: string, milestoneId: string): Promise<MilestoneRecord | undefined> {
        void projectId;
        const result = await this.client.execute("SELECT id, title, description, progress, start_date, due_date, created_at, updated_at, archived_at, deleted_at, version FROM milestones WHERE id = ?", [milestoneId]);
        return mapMilestoneRow(result.rows[0]);
    }
    async listMilestones(projectId: string): Promise<MilestoneRecord[]> {
        void projectId;
        const result = await this.client.execute("SELECT id, title, description, progress, start_date, due_date, created_at, updated_at, archived_at, deleted_at, version FROM milestones ORDER BY created_at, id");
    ...
    async createMilestone(projectId: string, input: CreateMilestoneInput): Promise<MilestoneRecord> {
        ...
        return runInWriteTransaction(this.client, async (tx) => {
            ...
            if (!project || !isEffectivelyOperational([projectState!])) {
                throw new AncestorNotActiveError("create", project
                    ? `Project dalam state ${projectState} — Milestone tidak dapat dibuat`
                    : `Project ${projectId} tidak ditemukan`);
            }
            await tx.execute("INSERT INTO milestones (id, title, description, progress, start_date, due_date, created_at, updated_at, version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)", [input.id, input.title, input.description, input.progress, input.startDate, input.dueDate, now, now]);
            await tx.execute("INSERT INTO activities (id, entity_type, entity_id, entity_version, actor_user_id, action, data, created_at) VALUES (?, 'milestone', ?, 1, ?, 'milestone.created', ?, ?)", [
                ulid(),
                input.id,
                input.actorUserId,
    ...
    private async commitMutation(projectId: string, input: MilestoneLifecycleInput | UpdateMilestoneInput, operation: LifecycleOperation): Promise<MilestoneRecord> {
        return runInWriteTransaction(this.client, async (tx) => {
            ...
            if (!isEffectivelyOperational([projectBefore])) {
                throw new AncestorNotActiveError(operation, `Project dalam state ${projectBefore} — Milestone tidak dapat menerima operasi ${operation} (INV-LIFE-001)`);
            }
            if (operation === "update") {
            ...
            await tx.execute("UPDATE milestones SET title = ?, description = ?, progress = ?, start_date = ?, due_date = ?, archived_at = ?, deleted_at = ?, updated_at = ?, version = ? WHERE id = ? AND version = ?", [
                next.title,
                next.description,
                next.progress,
                next.startDate,
            ...
            await tx.execute("INSERT INTO activities (id, entity_type, entity_id, entity_version, actor_user_id, action, data, created_at) VALUES (?, 'milestone', ?, ?, ?, ?, ?, ?)", [ulid(), input.milestoneId, nextVersion, input.actorUserId, action, JSON.stringify(data), now]);
...
async function loadMilestoneForUpdate(tx: Tx, milestoneId: string): Promise<LoadedMilestone> {
    const result = await tx.execute("SELECT id, title, description, progress, start_date, due_date, created_at, updated_at, archived_at, deleted_at, version FROM milestones WHERE id = ?", [milestoneId]);
...
function applyFieldChange<K extends "title" | "description" | "progress" | "startDate" | "dueDate">(target: MilestoneRecord, changes: Record<string, {
    before: unknown;
    after: unknown;
}>, key: K, value: MilestoneRecord[K] | undefined): void {
    if (value === undefined)
        return;
