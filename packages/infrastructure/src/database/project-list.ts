...
export async function listProjectSummaries(globalClient: Client, databaseResolver: ProjectDatabaseResolver, clientFactory: {
    create(databaseId: string): Client | Promise<Client>;
}, userId: string, statusFilter?: readonly ProjectStatus[]): Promise<ProjectSummary[]> {
    ...
    for (const membership of memberships) {
        ...
        if (!state) {
            throw new PipelineError("RESOURCE_NOT_FOUND", `project_state ${membership.projectId} tidak ditemukan di Project DB.`, 404);
        }
