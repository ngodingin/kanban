...
/** RAW token — HANYA sekali pada response create (BR-057). */
...
export async function revokePersonalAccessToken(
  ...
  // Self-managed (analog BR-034A): hanya pemilik — token orang lain 404.
...
export async function getPersonalAccessToken(
  ...
  userId: string,
  tokenId: string,
): Promise<PersonalAccessTokenSummary | null> {
