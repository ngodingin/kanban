...
import { groupPermissions, invitationGroupAssignments, invitations, membershipGroupAssignments, membershipPermissionAssignments, permissionGroups, permissions, projectMemberships, projects, users, } from "./global-schema.ts";
...
export async function assertAssigneeNotRevocationPending(globalClient: Client, projectId: string, userId: string): Promise<void> {
    ...
    if (result.rows[0]) {
        throw new PipelineError("INVALID_STATE", "User sedang dalam proses pencabutan Membership — tidak dapat dipilih sebagai assignee baru (BR-054C).", 409);
    }
}
...
async function attachPermissions(globalClient: Client, rows: Array<typeof permissionGroups.$inferSelect>): Promise<PermissionGroupSummary[]> {
    if (rows.length === 0)
        return [];
    const db = drizzle(globalClient);
...
export async function listPermissionGroups(globalClient: Client, projectId: string, opts: {
...
export async function createPermissionGroup(globalClient: Client, input: CreatePermissionGroupInput): Promise<PermissionGroupSummary> {
    ...
    const requested: Array<{
        permissionId: string;
        cardReadVisibility: CardReadVisibility | null;
...
export async function updatePermissionGroup(globalClient: Client, input: UpdatePermissionGroupInput): Promise<PermissionGroupSummary> {
    ...
    const now = new Date().toISOString();
    let requestedPermissions: Array<{
        permissionId: string;
        cardReadVisibility: CardReadVisibility | null;
    }> | undefined;
...
export async function createGroupAssignment(globalClient: Client, input: {
    projectId: string;
    membershipId: string;
    groupId: string;
    scopeType: string;
...
export async function revokeGroupAssignment(globalClient: Client, input: {
    projectId: string;
    membershipId: string;
    assignmentId: string;
}): Promise<GroupAssignmentSummary> {
    ...
    const claimed = await db.update(membershipGroupAssignments).set({ revokedAt: now })
        .where(and(eq(membershipGroupAssignments.id, input.assignmentId), isNull(membershipGroupAssignments.revokedAt)))
        .returning({
...
function parseCardReadVisibility(raw: string | null | undefined): "CREATED_BY_ME" | "ASSIGNED_TO_ME" | "ALL" {
    ...
    if (raw === "CREATED_BY_ME" || raw === "ASSIGNED_TO_ME" || raw === "ALL")
        return raw;
...
export async function createPermissionAssignment(globalClient: Client, input: {
    projectId: string;
    membershipId: string;
    permissionId: string;
    scopeType: string;
...
export async function revokePermissionAssignment(globalClient: Client, input: {
    projectId: string;
    membershipId: string;
    assignmentId: string;
}): Promise<PermissionAssignmentSummary> {
    ...
    const claimed = await db.update(membershipPermissionAssignments).set({ revokedAt: now })
        .where(and(eq(membershipPermissionAssignments.id, input.assignmentId), isNull(membershipPermissionAssignments.revokedAt)))
        .returning({
...
export async function createInvitation(globalClient: Client, input: {
    projectId: string;
    invitedByUserId: string;
    email: string;
    assignments: Array<{
...
export async function acceptInvitation(globalClient: Client, input: {
    invitationId: string;
    userId: string;
    userEmail: string;
    ...
    await runInDrizzleWriteTransaction(db, async (tx) => {
        ...
        if (existingMembership.length > 0) {
            ...
            await tx.update(membershipGroupAssignments).set({ revokedAt: now }).where(sql `${membershipGroupAssignments.membershipId} = ${membershipId} AND ${membershipGroupAssignments.revokedAt} IS NULL`).run();
            await tx.update(membershipPermissionAssignments).set({ revokedAt: now }).where(sql `${membershipPermissionAssignments.membershipId} = ${membershipId} AND ${membershipPermissionAssignments.revokedAt} IS NULL`).run();
        }
        else {
            membershipId = ulid();
...
export async function listProjectMembers(globalClient: Client, projectId: string, opts: {
...
export async function revokeMembership(globalClient: Client, input: {
    projectId: string;
    membershipId: string;
    actorUserId?: string;
}, projectDb?: Client | null): Promise<ProjectMemberSummary> {
    const db = drizzle(globalClient);
    const rows = await db.select().from(projectMemberships)
    ...
    if (membership.revokedAt === null) {
        await db.update(projectMemberships)
            ...
            .where(and(eq(projectMemberships.id, input.membershipId), isNull(projectMemberships.revokedAt), isNull(projectMemberships.revocationPendingAt)))
            .run();
        if (projectDb) {
            await cleanupAssigneesForRevokedMembership(projectDb, input.projectId, membership.userId, input.actorUserId ?? membership.userId);
        }
        const finalizedAt = new Date().toISOString();
        await db.update(projectMemberships)
            .set({ revokedAt: finalizedAt, revocationPendingAt: null })
            .where(and(eq(projectMemberships.id, input.membershipId), isNull(projectMemberships.revokedAt), isNotNull(projectMemberships.revocationPendingAt)))
            .run();
    }
    const freshRows = await db.select().from(projectMemberships)
        .where(eq(projectMemberships.id, input.membershipId)).run();
    const freshRevokedAt = (freshRows.rows[0] as unknown as {
...
export async function listProjectInvitations(globalClient: Client, projectId: string): Promise<InvitationListSummary[]> {
    const db = drizzle(globalClient);
...
export async function revokeInvitation(globalClient: Client, input: {
    projectId: string;
    invitationId: string;
    ...
    if (revokedAt === null) {
        const claimed = await db.update(invitations).set({ revokedAt: now })
            .where(and(eq(invitations.id, input.invitationId), isNull(invitations.revokedAt), isNull(invitations.acceptedAt)))
            .returning({ id: invitations.id });
        if (claimed.length === 0) {
            throw new PipelineError("INVALID_STATE", "Invitation sudah diterima dan tidak dapat di-revoke.", 409);
        }
...
export async function getMembershipInProject(globalClient: Client, projectId: string, membershipId: string): Promise<{
    id: string;
    userId: string;
} | null> {
    const result = await globalClient.execute({
...
export async function listMembershipAssignments(globalClient: Client, projectId: string, membershipId: string): Promise<MembershipAssignmentsList | null> {
...
export async function assertPermissionKey(globalClient: Client, projectId: string, requesterUserId: string, key: string): Promise<void> {
    const ownerId = await getProjectOwnerId(globalClient, projectId);
    if (ownerId === null) {
        throw new PipelineError("RESOURCE_NOT_FOUND", `Project ${projectId} tidak ditemukan.`, 404);
    }
