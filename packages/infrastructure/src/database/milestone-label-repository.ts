...
import type { CreateLabelInput, LabelLifecycleInput, LifecycleState, ListLabelsOptions, MilestoneLabelRecord, MilestoneLabelRepository, UpdateLabelInput, } from "@kanban/domain";
...
export class DrizzleMilestoneLabelRepository implements MilestoneLabelRepository {
    ...
    async listMilestoneLabels(projectId: string, milestoneId: string, opts?: ListLabelsOptions): Promise<MilestoneLabelRecord[]> {
        void projectId;
    ...
    async createMilestoneLabel(projectId: string, milestoneId: string, input: CreateLabelInput): Promise<MilestoneLabelRecord> {
        ...
        return runInWriteTransaction(this.client, async (tx) => {
            ...
            if (!isEffectivelyOperational(chain)) {
                throw new AncestorNotActiveError("create", `Ancestor tidak ACTIVE — Milestone Label tidak dapat dibuat di bawah Milestone ${milestoneId} (INV-LIFE-001)`);
            }
            await tx.execute("INSERT INTO milestone_labels (id, milestone_id, name, created_at, updated_at, version) VALUES (?, ?, ?, ?, ?, 1)", [input.id, milestoneId, input.name, now, now]);
    ...
    async updateMilestoneLabel(projectId: string, input: UpdateLabelInput): Promise<MilestoneLabelRecord> {
    ...
    async archiveMilestoneLabel(projectId: string, input: LabelLifecycleInput): Promise<MilestoneLabelRecord> {
        return this.commitMutation(input.labelId, input.expectedVersion, input.actorUserId, "archive");
    }
    async restoreMilestoneLabel(projectId: string, input: LabelLifecycleInput): Promise<MilestoneLabelRecord> {
        return this.commitMutation(input.labelId, input.expectedVersion, input.actorUserId, "restore");
    }
    async deleteMilestoneLabel(projectId: string, input: LabelLifecycleInput): Promise<MilestoneLabelRecord> {
        return this.commitMutation(input.labelId, input.expectedVersion, input.actorUserId, "delete");
    }
    private async commitMutation(labelId: string, expectedVersion: number, actorUserId: string, operation: LifecycleOperation, buildChanges?: (label: MilestoneLabelRecord) => Record<string, unknown>): Promise<MilestoneLabelRecord> {
        return runInWriteTransaction(this.client, async (tx) => {
            ...
            if (!isEffectivelyOperational(chain)) {
                throw new AncestorNotActiveError(operation, `Ancestor tidak ACTIVE — Milestone Label tidak dapat menerima operasi ${operation} (INV-LIFE-001)`);
            }
            ...
            await tx.execute("UPDATE milestone_labels SET name = ?, archived_at = ?, deleted_at = ?, updated_at = ?, version = ? WHERE id = ? AND version = ?", [next.name, next.archivedAt, next.deletedAt, now, nextVersion, labelId, expectedVersion]);
...
async function loadLabelForUpdate(tx: Tx, labelId: string): Promise<LoadedLabel> {
    const result = await tx.execute("SELECT id, milestone_id, name, created_at, updated_at, archived_at, deleted_at, version FROM milestone_labels WHERE id = ?", [labelId]);
    const row = result.rows[0];
    if (!row)
        throw new LabelNotFoundError(labelId, "milestone");
...
function insertActivity(tx: Tx, entityId: string, entityVersion: number, actorUserId: string, action: string, data: Record<string, unknown>, now: string): Promise<unknown> {
    return tx.execute("INSERT INTO activities (id, entity_type, entity_id, entity_version, actor_user_id, action, data, created_at) VALUES (?, 'milestone_label', ?, ?, ?, ?, ?, ?)", [ulid(), entityId, entityVersion, actorUserId, action, JSON.stringify(data), now]);
}
