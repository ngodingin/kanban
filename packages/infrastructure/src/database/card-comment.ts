...
import { AncestorNotActiveError, CardInvalidStateError, CardNotFoundError, CardValidationError, isEffectivelyOperational, resolveLifecycleState, type LifecycleState, } from "@kanban/domain";
...
async function loadCardAncestorContext(tx: Tx, cardId: string): Promise<CardAncestorContext> {
    ...
    const listRow = (await tx.execute("SELECT board_id, archived_at, deleted_at FROM lists WHERE id = ?", [String(cardRow.list_id)])).rows[0];
    ...
    if (listRow) {
        const boardRow = (await tx.execute("SELECT milestone_id, archived_at, deleted_at FROM boards WHERE id = ?", [
...
export async function addComment(client: Client, cardId: string, body: unknown, actorUserId: string): Promise<CardCommentRecord> {
    ...
    return runInWriteTransaction(client, async (tx) => {
        ...
        await tx.execute("INSERT INTO activities (id, entity_type, entity_id, entity_version, actor_user_id, action, data, created_at) VALUES (?, 'card', ?, ?, ?, 'comment.added', ?, ?)", [id, cardId, ctx.cardVersion, actorUserId, JSON.stringify({ body: validatedBody }), now]);
        return {
            id,
            cardId,
...
export async function editComment(client: Client, cardId: string, commentActivityId: string, newBody: unknown, actorUserId: string): Promise<EditCommentRecord> {
    const validatedBody = validateBody(newBody);
    return runInWriteTransaction(client, async (tx) => {
        const activityRow = (await tx.execute("SELECT id, entity_id, actor_user_id, action, data FROM activities WHERE id = ?", [
            commentActivityId,
        ])).rows[0];
        if (!activityRow ||
            String(activityRow.entity_id) !== cardId ||
            !COMMENT_ACTIONS.has(String(activityRow.action))) {
            throw new PipelineError("RESOURCE_NOT_FOUND", `Comment Activity ${commentActivityId} tidak ditemukan pada Card ${cardId}.`, 404);
        }
        if (String(activityRow.actor_user_id) !== actorUserId) {
            throw new PipelineError("PERMISSION_DENIED", "Hanya pemilik comment yang dapat mengedit comment ini (BR-034A).", 403);
        }
        ...
        const latestRow = (await tx.execute("SELECT action, data FROM activities WHERE entity_id = ? AND (id = ? OR json_extract(data, '$.commentActivityId') = ?) ORDER BY created_at DESC, id DESC LIMIT 1", [cardId, originalId, originalId])).rows[0]!;
        const latestData = (typeof latestRow.data === "string" ? JSON.parse(latestRow.data) : latestRow.data) as Record<string, unknown>;
        ...
        const id = ulid();
        const now = new Date().toISOString();
        await tx.execute("INSERT INTO activities (id, entity_type, entity_id, entity_version, actor_user_id, action, data, created_at) VALUES (?, 'card', ?, ?, ?, 'comment.edited', ?, ?)", [
            id,
            cardId,
            ctx.cardVersion,
            actorUserId,
