...
import type { BoardLabelRecord, BoardLabelRepository, CreateLabelInput, LabelLifecycleInput, LifecycleState, ListLabelsOptions, UpdateLabelInput, } from "@kanban/domain";
import { AncestorNotActiveError, evaluateRestore, isEffectivelyOperational, resolveLifecycleState, LabelInvalidStateError, LabelNotFoundError, LabelValidationError, LabelVersionConflictError, } from "@kanban/domain";
...
export class DrizzleBoardLabelRepository implements BoardLabelRepository {
    ...
    async listBoardLabels(projectId: string, boardId: string, opts?: ListLabelsOptions): Promise<BoardLabelRecord[]> {
        void projectId;
    ...
    async createBoardLabel(projectId: string, boardId: string, input: CreateLabelInput): Promise<BoardLabelRecord> {
        ...
        return runInWriteTransaction(this.client, async (tx) => {
            ...
            if (!isEffectivelyOperational(chain)) {
                throw new AncestorNotActiveError("create", `Ancestor tidak ACTIVE — Board Label tidak dapat dibuat di bawah Board ${boardId} (INV-LIFE-001)`);
            }
            await tx.execute("INSERT INTO board_labels (id, board_id, name, created_at, updated_at, version) VALUES (?, ?, ?, ?, ?, 1)", [input.id, boardId, input.name, now, now]);
    ...
    async updateBoardLabel(projectId: string, input: UpdateLabelInput): Promise<BoardLabelRecord> {
        void projectId;
        return this.commitMutation(input.labelId, input.expectedVersion, input.actorUserId, "update", (label) => {
            ...
            if (input.name !== undefined) {
                ...
                if (label.name !== input.name)
                    changes.name = { before: label.name, after: input.name };
    ...
    async archiveBoardLabel(projectId: string, input: LabelLifecycleInput): Promise<BoardLabelRecord> {
        void projectId;
        return this.commitMutation(input.labelId, input.expectedVersion, input.actorUserId, "archive");
    }
    async restoreBoardLabel(projectId: string, input: LabelLifecycleInput): Promise<BoardLabelRecord> {
        void projectId;
        return this.commitMutation(input.labelId, input.expectedVersion, input.actorUserId, "restore");
    }
    async deleteBoardLabel(projectId: string, input: LabelLifecycleInput): Promise<BoardLabelRecord> {
        void projectId;
        return this.commitMutation(input.labelId, input.expectedVersion, input.actorUserId, "delete");
    }
    private async commitMutation(labelId: string, expectedVersion: number, actorUserId: string, operation: LifecycleOperation, buildChanges?: (label: BoardLabelRecord) => Record<string, unknown>): Promise<BoardLabelRecord> {
        return runInWriteTransaction(this.client, async (tx) => {
            ...
            if (!isEffectivelyOperational(chain)) {
                throw new AncestorNotActiveError(operation, `Ancestor tidak ACTIVE — Board Label tidak dapat menerima operasi ${operation} (INV-LIFE-001)`);
            }
            ...
            if (operation === "update") {
                ...
                const projectRow = (await tx.execute("SELECT archived_at, deleted_at FROM project_state LIMIT 1")).rows[0];
            ...
            await tx.execute("UPDATE board_labels SET name = ?, archived_at = ?, deleted_at = ?, updated_at = ?, version = ? WHERE id = ? AND version = ?", [next.name, next.archivedAt, next.deletedAt, now, nextVersion, labelId, expectedVersion]);
...
async function loadAncestorStates(tx: Tx, boardId: string): Promise<LifecycleState[]> {
    const boardRow = (await tx.execute("SELECT archived_at, deleted_at FROM boards WHERE id = ?", [boardId])).rows[0];
    ...
    if (boardRow) {
        ...
        if (msIdRaw !== null && msIdRaw !== undefined) {
            msRow = (await tx.execute("SELECT archived_at, deleted_at FROM milestones WHERE id = ?", [String(msIdRaw)])).rows[0];
        }
    }
...
async function loadLabelForUpdate(tx: Tx, labelId: string): Promise<LoadedLabel> {
    const result = await tx.execute("SELECT id, board_id, name, created_at, updated_at, archived_at, deleted_at, version FROM board_labels WHERE id = ?", [labelId]);
    const row = result.rows[0];
    if (!row)
        throw new LabelNotFoundError(labelId, "board");
...
function insertActivity(tx: Tx, entityId: string, entityVersion: number, actorUserId: string, action: string, data: Record<string, unknown>, now: string): Promise<unknown> {
    return tx.execute("INSERT INTO activities (id, entity_type, entity_id, entity_version, actor_user_id, action, data, created_at) VALUES (?, 'board_label', ?, ?, ?, ?, ?, ?)", [ulid(), entityId, entityVersion, actorUserId, action, JSON.stringify(data), now]);
}
