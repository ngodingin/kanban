...
import type { ProjectLifecycleInput, ProjectLifecycleState, ProjectRepository, ProjectStateRecord, UpdateProjectNameInput, } from "@kanban/domain";
import { ProjectInvalidStateError, ProjectNotFoundError, ProjectVersionConflictError, resolveProjectLifecycle, } from "@kanban/domain";
...
export class DrizzleProjectRepository implements ProjectRepository {
    ...
    private async commitMutation(input: ProjectLifecycleInput, operation: LifecycleOperation, buildNext: (current: ProjectStateRecord, now: string, lifecycleBefore: ProjectLifecycleState) => NextProjectState): Promise<ProjectStateRecord> {
        ...
        return runInWriteTransaction(this.client, async (tx) => {
            ...
            await tx.execute("UPDATE project_state SET name = ?, updated_at = ?, archived_at = ?, deleted_at = ?, version = ? WHERE project_id = ? AND version = ?", [next.name, now, next.archivedAt, next.deletedAt, nextVersion, projectId, expectedVersion]);
            await tx.execute("INSERT INTO activities (id, entity_type, entity_id, entity_version, actor_user_id, action, data, created_at) VALUES (?, 'project', ?, ?, ?, ?, ?, ?)", [ulid(), projectId, nextVersion, actorUserId, next.activityAction, JSON.stringify(next.activityData), now]);
            return {
                projectId,
                name: next.name,
