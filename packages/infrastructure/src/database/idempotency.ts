...
/** C.3/TASK-0.21 — retensi minimum completed result (poin 8). */
export const IDEMPOTENCY_COMPLETED_TTL_MS = 24 * 60 * 60 * 1000;
/** Lease default claim IN_PROGRESS — cukup untuk mutation biasa, jauh lebih pendek dari completed TTL. */
...
export type IdempotencyClaimResult =
  ...
  | { status: "replay"; response: IdempotencyResponse }
  | { status: "conflict" }
  | { status: "in_progress" };
...
/**
 * IdempotencyStore state machine atomic claim (C.3/TASK-0.21, amandemen SOT
 * 4.0.0) — MENGGANTIKAN pola cache `get -> handler -> put` (0.16.2, dilarang
 * eksplisit 03-ENG: dua request in-flight bisa eksekusi side-effect ganda).
 *
 * Atomicity claim pertama TIDAK bergantung pada SELECT-lalu-INSERT (rentan
 * TOCTOU race antara dua request benar-benar konkuren) — melainkan INSERT
 * LANGSUNG dan menangani UNIQUE constraint violation sebagai sinyal "row
 * sudah ada", memanfaatkan atomicity SQLite di level storage engine
 * (persis prinsip yang sama dengan kenapa `mapUniqueViolation`/`isBusy()`
 * di `project-admin.ts`/`transaction.ts` mengecek `.code` terstruktur).
 */
export class DbIdempotencyStore {
  ...
  /** Claim key+scope atomik SEBELUM domain handler dijalankan (C.3 poin 3). */
  async claim(key: string, scope: string, fingerprint: string): Promise<IdempotencyClaimResult> {
    ...
    try {
      ...
      // Row sudah ada (baru dibuat request lain, ATAU baris lama) — evaluasi.
      return this.claimAgainstExisting(key, scope, fingerprint, claimToken, leaseExpiresAt, now);
    }
  }

  private async claimAgainstExisting(
    key: string,
    scope: string,
    fingerprint: string,
    ...
    if (!row) {
      // Row barusan dihapus (mis. release() request lain) sebelum kita baca —
      // coba claim ulang sekali (kasus langka, retry tunggal cukup).
    ...
    if (row.state === "COMPLETED") {
      ...
      if (row.request_fingerprint !== fingerprint) return { status: "conflict" };
      return {
        status: "replay",
  ...
  private async reclaimInProgress(
    key: string,
    scope: string,
    fingerprint: string,
    newClaimToken: string,
    ...
    // Reclaim MERANTASI token (C.3 poin 3) — kondisional atas lease_expires_at
    // supaya dua reclaimer konkuren tidak sama-sama menang.
    ...
    if (result.rowsAffected === 0) {
      // Reclaimer lain menang race — evaluasi ulang state terbaru.
  ...
  private async reclaimCompleted(
    key: string,
    scope: string,
    fingerprint: string,
    newClaimToken: string,
    ...
    // Completed result kadaluarsa (>24 jam) -> diproses sebagai request baru
    // (C.3 poin 8): reset ke IN_PROGRESS, bersihkan result lama.
    const result = await this.globalClient.execute({
      sql: `UPDATE idempotency_keys
  ...
  /**
   * Tandai claim selesai sukses (HANYA dipanggil untuk respons 2xx — C.3
   * poin 7). Token TIDAK cocok (stale owner setelah reclaim) -> no-op,
   * dikembalikan sebagai `false` (bukan throw — pemanggil tetap harus
   * mengembalikan respons ke client, cuma tidak tersimpan sebagai replay).
   */
  ...
  /**
   * Lepas claim (handler gagal/throw/non-2xx — C.3 poin 7) — row DIHAPUS
   * total (bukan ditandai) supaya retry berikutnya diproses sebagai request
   * baru sepenuhnya. Token tidak cocok -> no-op silent (stale owner).
   */
