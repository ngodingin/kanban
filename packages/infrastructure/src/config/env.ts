...
export function loadAppConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
    ...
    if (!parsed.success) {
        ...
        throw new Error(`App config tidak lengkap (kurang: ${missing.join(", ") || "nilai tidak valid"})`);
    }
    ...
    if (canonical !== null && parsed.data.BETTER_AUTH_URL !== canonical) {
        if (parsed.data.AUTH_ALLOW_NON_CANONICAL === "1") {
            console.warn(`[config] AUTH_ALLOW_NON_CANONICAL=1: BETTER_AUTH_URL=${parsed.data.BETTER_AUTH_URL} dipakai walau bukan canonical ${canonical} (preview/CL-21; jangan dipakai production)`);
        }
        else {
            throw new Error(`BETTER_AUTH_URL untuk env ${appEnv} wajib ${canonical} (D.7), dapat: ${parsed.data.BETTER_AUTH_URL}`);
        }
    }
