...
expectError(() => parseGlobalDbEnv({ GLOBAL_DB_URL: dbUrl! } as NodeJS.ProcessEnv), "negatif: GLOBAL_DB_TOKEN hilang -> throw");
expectError(() => createProjectClient({ url: "file:local.db", authToken: "x" }), "negatif: url non-libsql:// -> throw");
expectError(() => createProjectClient({ url: `libsql://${dbUrl!}`, authToken: "" }), "negatif: authToken kosong -> throw");
const globalClient = createGlobalClient({
    GLOBAL_DB_URL: dbUrl!,
    GLOBAL_DB_TOKEN: dbToken!,
} as NodeJS.ProcessEnv);
const g = await globalClient.execute("SELECT 1 AS one");
if (Number(g.rows[0]?.one) !== 1)
    throw new Error("global SELECT 1 gagal");
...
if (Number(p.rows[0]?.one) !== 1)
    throw new Error("project SELECT 1 gagal");
