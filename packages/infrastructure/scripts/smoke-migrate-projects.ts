...
try {
    ...
    if (result.total < 1)
        fail("total", `total mapping harus >=1, dapat ${result.total}`);
    else
        console.log(`PASS: fan-out melihat ${result.total} mapping (termasuk project uji)`);
    if (result.failed.length > 0)
        fail("gagal", result.failed.join("; "));
    else
        console.log("PASS: tidak ada kegagalan fan-out untuk mapping manapun");
    if (result.ok < 1)
        fail("ok", `ok harus >=1, dapat ${result.ok}`);
    ...
    try {
        const tables = await projectClient.execute("SELECT COUNT(*) AS n FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name NOT LIKE '__drizzle%'");
        if (Number(tables.rows[0]?.n) !== 10)
            fail("migrasi", `10 tabel Project diharapkan, dapat ${tables.rows[0]?.n}`);
        else
            console.log("PASS: fan-out benar-benar menerapkan migration (10 tabel Project DB terpasang di DB nyata)");
    }
    finally {
