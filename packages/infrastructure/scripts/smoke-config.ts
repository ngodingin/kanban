...
expectThrow({ ...BASE, BETTER_AUTH_URL: "http://localhost:5173", VERCEL_ENV: "preview" } as NodeJS.ProcessEnv, "negatif: staging + BETTER_AUTH_URL bukan kanban-ngodingin.vercel.app -> throw");
...
if (prod.env !== "production" || prod.canonicalOrigin !== "https://kanban.ngodingin.xyz")
    throw new Error("production config salah");
...
if (stag.env !== "staging" || stag.canonicalOrigin !== "https://kanban-ngodingin.vercel.app")
    throw new Error("staging config salah");
...
if (dev.env !== "development" || dev.MAIL_FROM !== "noreply@kanban.ngodingin.xyz")
    throw new Error("dev config salah");
...
if (new Set(origins).size !== origins.length)
    throw new Error("canonical origin antar env tidak unik");
