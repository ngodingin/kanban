...
try {
    await client.execute("CREATE TABLE project_databases (project_id TEXT PRIMARY KEY, database_id TEXT NOT NULL, created_at TEXT NOT NULL)");
    ...
    if (unknown !== null)
        throw new Error(`project_id tak dikenal seharusnya null: ${JSON.stringify(unknown)}`);
    console.log("PASS negatif: resolve project_id tak dikenal -> null (tidak menyentuh DB lain)");
    const empty = await resolver.resolve("");
    if (empty !== null)
        throw new Error("project_id kosong seharusnya null");
