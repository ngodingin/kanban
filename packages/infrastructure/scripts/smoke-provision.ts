...
try {
    ...
    const states = await client.execute("SELECT project_id, name, version, archived_at, deleted_at FROM project_state WHERE project_id = ?", [projectId]);
    if (states.rows.length !== 1)
        fail("state", "project_state harus tepat satu");
    else {
        const s = states.rows[0] as unknown as {
            version: number;
            archived_at: null;
            deleted_at: null;
            name: string;
        ...
        if (s.version !== 1 || s.archived_at !== null || s.deleted_at !== null || s.name !== "Smoke Provisioning") {
            ...
            console.log("PASS: project_state tepat satu, ACTIVE (version=1, archived_at/deleted_at NULL)");
    }
    const acts = await client.execute("SELECT id, entity_type, entity_id, entity_version, actor_user_id, action, data FROM activities WHERE entity_type='project' AND entity_id = ?", [projectId]);
    if (acts.rows.length !== 1)
        fail("activity", "Activity project.created harus tepat satu");
    else {
        const a = acts.rows[0] as unknown as {
            id: string;
            action: string;
            entity_version: number;
            data: string;
        };
        const data = JSON.parse(a.data) as {
            snapshot?: {
                name?: string;
            };
        };
        if (a.action !== "project.created" || a.entity_version !== 1 || data.snapshot?.name !== "Smoke Provisioning") {
            ...
            console.log("PASS: Activity project.created tunggal, entity_version=1, data B.5 snapshot.name");
        if (!isValidUlid(a.id))
            fail("activity", `id activity bukan ULID (A.13): ${a.id}`);
        else
            console.log("PASS: Activity project.created id memakai ULID (A.13)");
    }
    ...
    try {
        ...
        if (Number(after.rows[0]?.n) !== 1)
            fail("atomic", "tx gagal meninggalkan sisa (orphan activity)");
        else
            console.log("PASS: seed atomik — tx gagal tidak meninggalkan activity yatim (F.2)");
    }
}
catch (e) {
    fail("provision", e);
    if (!(e instanceof ProjectProvisioningError))
        fail("provision", "harus ProjectProvisioningError");
}
finally {
    await client?.close();
    if (dbName) {
