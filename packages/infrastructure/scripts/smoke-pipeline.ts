...
try {
  ...
  await expectReject(
    pipeline.run(new Request("http://localhost:3000/api/v1/projects/x"), projectA),
    "TOKEN_EXPIRED", 401, "pipe-anonymous", "tanpa identitas -> TOKEN_EXPIRED 401",
  );
  await expectReject(
    pipeline.run(new Request("http://localhost:3000/api/v1/projects/x", { headers: { cookie: "kanban.session_token=bad" } }), projectA),
    "TOKEN_EXPIRED", 401, "pipe-invalid-cookie", "cookie invalid -> TOKEN_EXPIRED 401",
  );
  await expectReject(
    pipeline.run(new Request("http://localhost:3000/api/v1/projects/x", { headers: { cookie: cookieB } }), projectA),
    "PROJECT_ACCESS_DENIED", 403, "pipe-no-membership", "user B ke Project A tanpa membership -> PROJECT_ACCESS_DENIED 403",
  );
  await expectReject(
    pipeline.run(new Request("http://localhost:3000/api/v1/projects/x", { headers: { cookie: cookieA } }), projectB),
    "PROJECT_ACCESS_DENIED", 403, "pipe-cross-project", "user A ke Project B (bukan anggota) -> PROJECT_ACCESS_DENIED 403",
  );
  ...
  await expectReject(
    pipeline.run(new Request("http://localhost:3000/api/v1/projects/x", { headers: { cookie: cookieA } }), projectUnknown),
    "RESOURCE_NOT_FOUND", 404, "pipe-unknown-project", "project tidak dikenal -> RESOURCE_NOT_FOUND 404",
  );
  await expectReject(
    pipeline.run(new Request("http://localhost:3000/api/v1/projects/x", { headers: { cookie: cookieX } }), projectA),
    "PROJECT_ACCESS_DENIED", 403, "pipe-revoked-membership", "membership revoked -> PROJECT_ACCESS_DENIED 403",
  );
  ...
  async function expectReject(promise: Promise<unknown>, code: string, status: number, name: string, msg: string) {
    try {
      ...
      if (e instanceof PipelineError && e.code === code && e.httpStatus === status) pass(name, msg);
      else fail(name, `${msg} (dapat: ${String(e)})`);
    }
  }
} catch (e) {
