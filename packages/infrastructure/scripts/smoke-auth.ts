...
try {
  ...
  if (!isValidUlid(id) || !/^[a-z0-9]+$/.test(id)) fail("ulid", `id bukan ULID lowercase: ${id}`);
  else console.log("PASS: generateId ULID (custom advanced.generateId)");

  const rows = await client.execute(
    "SELECT id, email, email_verified, name FROM users WHERE id = ?",
    [id],
  );
  if (rows.rows.length !== 1) fail("mapping", "user tidak tersimpan di Global DB users");
  else {
    const r = rows.rows[0] as unknown as { email: string; email_verified: number; name: string };
    if (r.email !== email || r.email_verified !== 0 || r.name !== "Auth Smoke") fail("mapping", "field mapping salah");
    else console.log("PASS: adapter menulis ke tabel users dengan kolom snake_case (mapping B.2)");
  }
