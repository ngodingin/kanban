...
try {
  ...
  const history = await client.execute(
    "SELECT COUNT(*) AS n FROM card_board_labels WHERE card_id = 'cd_1' AND label_id = 'bl_1'",
  );
  if (Number(history.rows[0]?.n) !== 2) throw new Error("riwayat junction harus tetap 2 baris (append-only, bukan hapus)");
  ...
  if (!read) throw new Error("activity tidak terbaca");
  if (read.action !== "card.moved" || read.entityVersion !== 2) throw new Error("kolom activity salah");
  const data = read.data as { from: { list_title: string } };
  if (data.from.list_title !== "Todo") throw new Error("data JSON tidak round-trip");
  ...
  if (Number(count.rows[0]?.n) !== 1) throw new Error("query polymorphic gagal");
