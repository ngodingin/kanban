...
if (!migrationFile) throw new Error("tidak ada file migration di drizzle/migrations");
...
for (const name of EXPECTED_TABLES) {
  if (!ddlTables.has(name)) throw new Error(`migration tidak membuat tabel ${name}`);
}
...
try {
  ...
  for (const name of EXPECTED_TABLES) {
    if (!created.has(name)) throw new Error(`tabel ${name} tidak ada di DB hasil migrasi`);
  }
  console.log(`PASS: migrasi menghasilkan ${created.size} tabel di DB (semua 16 ada)`);

  const indexNames = await client.execute(
  ...
  for (const expected of [
    ...
    if (!idx.has(expected)) throw new Error(`index ${expected} tidak ada`);
  }
