...
try {
    ...
    if (Number(tables.rows[0]?.n) !== 16)
        fail("migrate", "Global DB harus 16 tabel");
    else
        console.log("PASS: Global DB nyata (Turso) siap — 16 tabel via applyGlobalMigrations");
    ...
    if (!read || read.databaseId !== databaseId)
        fail("mapping", "mapping tidak terbaca");
    else
        console.log("PASS: mapping tercatat di project_databases (A.4/B.1) dan terbaca ulang");
    try {
        await recordProjectDatabaseMapping(client, { projectId, databaseId, now });
        ...
        if (e instanceof MappingAlreadyExistsError)
            console.log("PASS: mapping duplikat ditolak (satu Project = satu database)");
        else
            fail("duplicate", e);
    }
