...
try {
    ...
    const requestLink = (email: string, name?: string) => auth.handler(new Request(`${baseUrl}/api/auth/sign-in/magic-link`, {
        method: "POST",
    ...
    assert(((await resKnown.json()) as {
        status?: boolean;
    }).status === true, "request-link-body", "body {status:true} (tanpa kebocoran)");
    ...
    assert(((await resUnknown.json()) as {
        status?: boolean;
    }).status === true, "enumeration-body", "email tak dikenal -> body identik {status:true}");
    const verify = (t: string) => auth.handler(new Request(`${baseUrl}/api/auth/magic-link/verify?token=${encodeURIComponent(t)}`, {
    ...
    const getSes = await auth.handler(new Request(`${baseUrl}/api/auth/get-session`, {
        headers: { cookie: `${cookieName}=${sessionToken}` },
    }));
    const getSesJson = (await getSes.json()) as {
        user?: {
            email?: string;
        };
    } | null;
    ...
    const emailPasswordRes = await auth.handler(new Request(`${baseUrl}/api/auth/sign-in/email`, {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    const socialRes = await auth.handler(new Request(`${baseUrl}/api/auth/sign-in/google`, { method: "POST" }));
    ...
    await prod.handler(new Request("https://kanban.ngodingin.xyz/api/auth/sign-in/magic-link", {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    for (const email of [`${cleanupEmails}@smoke.local`, `${cleanupEmails}-unknown@smoke.local`, `${cleanupEmails}-expired@smoke.local`, `${cleanupEmails}-conc@smoke.local`, `${cleanupEmails}-prod@smoke.local`]) {
        ...
        for (const u of uRows.rows as unknown as Array<{
            id: string;
        }>) {
