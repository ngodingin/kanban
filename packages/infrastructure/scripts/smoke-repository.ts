...
try {
    ...
    if (!state || state.name !== "P1" || state.version !== 1)
        fail("getProjectState", "state tidak terbaca");
    else
        console.log("PASS: getProjectState membaca data domain (bukan objek Drizzle)");
    const raw = await client.execute("SELECT name FROM project_state");
    if (raw.rows.length !== 1)
        fail("raw", "verifikasi data mentah gagal");
    else
        console.log("PASS: data di DB cocok dengan hasil repository (sumber tunggal)");
    const db = drizzle(client);
    const stateRow = await db.select().from(projectState).where(eq(projectState.projectId, "proj_1")).get();
    if (stateRow?.name !== "P1")
        fail("state-db", "state DB tidak cocok");
    else
        console.log("PASS: domain type tidak bocor ke implementasi (interface + impl terpisah)");
}
catch (e) {
    fail("exception", e);
}
