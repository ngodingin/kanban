...
try {
    ...
    await expectUniqueViolation(() => client.execute({
        sql: "INSERT INTO users (id, email, email_verified, name, created_at, updated_at) VALUES (?, ?, 0, ?, ?, ?)",
    ...
    if (!gcols.includes("scope_type"))
        throw new Error("kolom scope_type hilang");
    if (!gcols.includes("revoked_at"))
        throw new Error("kolom revoked_at hilang");
    ...
    if (!keyCols.includes("key_hash") || keyCols.includes("key"))
        throw new Error("api_keys wajib key_hash, tanpa kolom key plaintext");
    if (!patCols.includes("token_hash") || patCols.includes("token"))
        throw new Error("pat wajib token_hash, tanpa kolom token plaintext");
    ...
    for (const c of [...userNeed, ...sessionNeed, ...accountNeed]) {
        ...
        if (!table.includes(c))
            throw new Error(`kolom Better Auth ${c} hilang`);
    }
