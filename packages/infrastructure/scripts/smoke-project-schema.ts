...
if (!migrationFile) throw new Error("tidak ada migration project");
const ddl = readFileSync(join(migrationsDir, migrationFile), "utf8");
const ddlTables = new Set(
  [...ddl.matchAll(/CREATE TABLE [`"]?(\w+)[`"]?/g)].map((m) => m[1]!).filter((n) => !n.startsWith("__drizzle")),
);
for (const name of EXPECTED_TABLES) {
  if (!ddlTables.has(name)) throw new Error(`migration project tidak membuat tabel ${name}`);
}
...
try {
  ...
  for (const t of versioned) {
    ...
    if (!cols.includes("version")) throw new Error(`${t} tidak punya version`);
  ...
  if (!stateCols.includes("project_id")) throw new Error("project_state harus punya project_id");
  ...
  for (const t of junction) {
    const info = await client.execute(`PRAGMA table_info(${t})`);
    const cols = info.rows.map((r) => String(r.name));
    if (!cols.includes("removed_at")) throw new Error(`${t} tidak punya removed_at`);
  }
  ...
  if (Number(journal.rows[0]?.n) !== 1) throw new Error("migration project tidak idempotent");
