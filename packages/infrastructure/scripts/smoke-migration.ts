...
try {
    ...
    const actualTables = (await client.execute("SELECT name FROM sqlite_master WHERE type = 'table' AND name NOT LIKE 'sqlite_%' AND name NOT LIKE '__drizzle%' ORDER BY name")).rows.map((r) => String(r.name));
    if (JSON.stringify(actualTables) !== JSON.stringify(expectedTableNames)) {
        throw new Error(`daftar tabel tidak cocok — hilang: ${expectedTableNames.filter((t) => !actualTables.includes(t)).join(",") || "-"}; ekstra: ${actualTables.filter((t) => !expectedTableNames.includes(t)).join(",") || "-"}`);
    }
    await client.execute("INSERT INTO permissions (id, key) VALUES ('perm_idem', 'card.move')");
    await migrate(db, { migrationsFolder });
    const afterData = await appliedCount();
    if (afterData !== migrationFileCount)
        throw new Error("apply ulang dengan data ada harus tetap no-op");
    const perm = await client.execute("SELECT id FROM permissions WHERE id = 'perm_idem'");
    if (perm.rows.length !== 1)
        throw new Error("data hilang setelah apply ulang");
