...
import {
  ...
  resolveOrThrow,
} from "../src/database/project-resolver.ts";
...
try {
  ...
  if (mapping.databaseId !== "db_projA") throw new Error("mapping proj_A salah");
  ...
  if (!threw) throw new Error("project_id tak dikenal harus throw ProjectDatabaseNotFoundError");
  ...
  if (rows.rows.length !== 1 || String(rows.rows[0]!.id) !== "hanya-A") throw new Error("project DB berubah");
