...
describe("moveCard auto-orphan Board Label (goal 3.7.2)", () => {
    it("[WAJIB] move lintas-Board → Board Label ter-orphan (removed_at terisi) + Activity label.removed, dalam transaksi/Activity list yang sama dengan card.moved", async () => {
        ...
        const bdRow = await db.client.execute("SELECT removed_at FROM card_board_labels WHERE card_id = 'c_1' AND label_id = 'bl_1'");
        expect(bdRow.rows[0]!.removed_at).not.toBeNull();
        const activities = await db.client.execute("SELECT action, entity_version, data FROM activities WHERE entity_id = 'c_1' ORDER BY created_at, action");
    ...
    it("[Invariant #5/BR-018] Milestone Label pada Card yang sama TIDAK ter-orphan oleh move lintas-Board yang sama", async () => {
        ...
        const msRow = await db.client.execute("SELECT removed_at FROM card_milestone_labels WHERE card_id = 'c_2' AND label_id = 'ml_1'");
        expect(msRow.rows[0]!.removed_at).toBeNull();
        const activities = await db.client.execute("SELECT action FROM activities WHERE entity_id = 'c_2' AND action = 'label.removed'");
    ...
    it("[Guard presisi] move List→List DALAM Board yang sama TIDAK meng-orphan Board Label apa pun", async () => {
        ...
        const bdRow = await db.client.execute("SELECT removed_at FROM card_board_labels WHERE card_id = 'c_3' AND label_id = 'bl_1'");
        expect(bdRow.rows[0]!.removed_at).toBeNull();
        const activities = await db.client.execute("SELECT action FROM activities WHERE entity_id = 'c_3' AND action = 'label.removed'");
    ...
    it("[Tanpa Board Label] move lintas-Board pada Card tanpa Board Label apa pun → tidak error, tidak ada Activity label.removed", async () => {
        ...
        expect(moved.version).toBe(4);
        const activities = await db.client.execute("SELECT action FROM activities WHERE entity_id = 'c_4' AND action = 'label.removed'");
        expect(activities.rows).toHaveLength(0);
    });
});
