...
import { AncestorNotActiveError, CardInvalidStateError, CardNotFoundError, CardValidationError, CardVersionConflictError, ListNotFoundError, } from "@kanban/domain";
...
beforeAll(async () => {
    ...
    await db.client.execute("CREATE TABLE active_members_test (project_id TEXT NOT NULL, user_id TEXT NOT NULL)");
...
async function seedCard(id: string, opts: {
...
describe("createCard — INV-LIFE-001 chain 4 level / FR-024–026 / 03-ENG A.5 (goal 2.8.1)", () => {
    ...
    it("[INV-LIFE-001] negatif: masing-masing ancestor ARCHIVED (List/Board/Milestone/Project) → ditolak", async () => {
        ...
        for (const [i, testCase] of cases.entries()) {
            ...
            await expect(repo.createCard(PROJECT, {
                id: `cd_case${i}`,
                listId: `ls_case${i}`,
                title: "X",
                subtitle: null,
...
describe("update/archive/restore/delete Card — A.3/AC-020/BR-045A/C.15 (goal 2.8.2)", () => {
    it("positif: update field + validasi assignee baru + Activity changes", async () => {
        ...
        const activity = await db.client.execute("SELECT data FROM activities WHERE entity_id = 'cd_u' AND action = 'card.updated'");
        expect(JSON.parse(String(activity.rows[0]!.data))).toEqual({
            changes: {
    ...
    it("[BR-061/062][C.15] updateCard tidak menyentuh list_id — record tetap; input tak punya field list", async () => {
        ...
        expect(updated.listId).toBe("ls_c");
    ...
    it("[AC-020] expected_version salah → VERSION_CONFLICT tanpa perubahan/activity", async () => {
        await seedChain();
        await seedCard("cd_v");
        await expect(repo.updateCard(PROJECT, { cardId: "cd_v", expectedVersion: 99, actorUserId: OWNER, title: "Tabrak" })).rejects.toBeInstanceOf(CardVersionConflictError);
    ...
    it("[Review-CL-02][INV-LIFE-001] negatif: Project ARCHIVED walau Card local ACTIVE → update/archive/delete DITOLAK semua", async () => {
        ...
        await expect(repo.updateCard(PROJECT, { cardId: "cd_pa", expectedVersion: 1, actorUserId: OWNER, title: "Gagal" })).rejects.toBeInstanceOf(AncestorNotActiveError);
        await expect(repo.archiveCard(PROJECT, { cardId: "cd_pa", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
        await expect(repo.deleteCard(PROJECT, { cardId: "cd_pa", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    ...
    it("[A.3] update dari ARCHIVED ditolak; archive sukses lalu diulang ditolak", async () => {
        ...
        await expect(repo.updateCard(PROJECT, { cardId: "cd_a", expectedVersion: 1, actorUserId: OWNER, title: "Gagal" })).rejects.toBeInstanceOf(CardInvalidStateError);
        ...
        await expect(repo.archiveCard(PROJECT, { cardId: "cd_arc", expectedVersion: 2, actorUserId: OWNER })).rejects.toBeInstanceOf(CardInvalidStateError);
    });
    it("[BR-045A] restore blanket: Card di-archive User A berhasil di-restore User B", async () => {
        ...
        const activity = await db.client.execute("SELECT data, actor_user_id FROM activities WHERE entity_id = 'cd_bl' AND action = 'card.restored'");
    ...
    it("[INV-LIFE-002] restore Card saat ancestor Board ARCHIVED → ditolak", async () => {
        ...
        await expect(repo.restoreCard(PROJECT, { cardId: "cd_rb", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    });
    it("[A.3] delete dari ARCHIVED diizinkan previous_state ARCHIVED; delete dari DELETED ditolak", async () => {
        ...
        await expect(repo.deleteCard(PROJECT, { cardId: "cd_term", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(CardInvalidStateError);
    });
    it("negatif: card tidak ada → RESOURCE_NOT_FOUND", async () => {
        await seedChain();
        await expect(repo.archiveCard(PROJECT, { cardId: "cd_none", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(CardNotFoundError);
    });
});
