...
describe("runInWriteTransaction (integration, local Project DB)", () => {
    ...
    it("INV-9/INV-MOVE-004: rollback membatalkan mutation saat fn throw", async () => {
        await expect(runInWriteTransaction(client, async (tx) => {
    ...
    it("mutation + activity atomik: activity invalid membatalkan mutation (INV #9)", async () => {
        await expect(runInWriteTransaction(client, async (tx) => {
            ...
            await tx.execute("INSERT INTO activities (id, entity_type, entity_id, entity_version, actor_user_id, action, data, created_at) VALUES ('act_3', 'gadget', 'ms_3', 1, 'u1', 'x', '{}', ?)", [now]);
        })).rejects.toThrow();
