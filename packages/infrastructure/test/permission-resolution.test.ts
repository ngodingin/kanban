...
async function insertGroupAssignment(id: string, membershipId: string, groupId: string, opts: {
...
async function insertDirectAssignment(id: string, membershipId: string, key: string, opts: {
    revokedAt?: string | null;
    visibility?: string | null;
} = {}): Promise<void> {
    seq += 1;
    await client.execute({ sql: "INSERT OR IGNORE INTO permissions (id, key) VALUES (?, ?)", args: [`perm_${seq}`, key] });
    const actual = await client.execute({ sql: "SELECT id FROM permissions WHERE key = ?", args: [key] });
    const pid = String(actual.rows[0]!.id);
    await client.execute({
...
describe("loadEffectivePermissionInputs — goal 4.2.1", () => {
    ...
    it("[DoD] Scoped ke SATU membership — assignment Membership lain tidak bocor", async () => {
        ...
        expect(mine.groupAssignments).toHaveLength(1);
