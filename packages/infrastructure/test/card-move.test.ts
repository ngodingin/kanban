...
async function seedFullBoard(opts: { boardArchivedAt?: string | null; projectArchivedAt?: string | null } = {}): Promise<void> {
  ...
  // Milestone sama: ms_1 → dua Board; Milestone beda: ms_2 → satu Board
...
async function seedCard(id: string, listId: string, opts: { archivedAt?: string | null; deletedAt?: string | null } = {}): Promise<void> {
  await db.client.execute({
...
async function movedActivities(cardId: string): Promise<Array<{ action: string; data: string }>> {
...
describe("moveCard — C.8 / INV-MOVE-001–004 / BR-017/018 (goal 2.10.1)", () => {
  ...
  it("[BR-018] negatif: move ke Board di Milestone BEDA → INVALID_DESTINATION walau permission penuh", async () => {
    ...
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_x", destinationListId: "ls_m2", expectedVersion: 1, actorUserId: OWNER }),
    ...
    expect(row.rows[0]).toMatchObject({ list_id: "ls_s", version: 1 }); // rollback total
  });
  ...
  it("[INV-MOVE-001][Project-boundary] move ke List Project lain → ditolak INVALID_DESTINATION tanpa menyentuh DB lain", async () => {
    ...
    // isolation DB-per-project: list Project lain tidak ada di DB ini → jalur yang sama dengan tidak ada
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_p", destinationListId: "ls_project_lain", expectedVersion: 1, actorUserId: OWNER }),
  ...
  it("[INV-MOVE-002] negatif: ancestor destination tidak ACTIVE → INVALID_DESTINATION", async () => {
    ...
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_i", destinationListId: "ls_m2", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(InvalidDestinationError);
  });
  ...
  it("[AC-020][BR-021] expected_version salah → VERSION_CONFLICT duluan; INV-MOVE tidak dievaluasi; tanpa perubahan/activity", async () => {
    ...
    // tujuan bahkan cross-milestone — tetap harus VERSION_CONFLICT karena version dicek lebih dulu
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_v", destinationListId: "ls_m2", expectedVersion: 99, actorUserId: OWNER }),
  ...
  it("[konkurensi][AC-020] mover kedua beroperasi pada snapshot stale → satu sukses, satu VERSION_CONFLICT", async () => {
    ...
    // Mover pertama commit penuh (menang race).
    ...
    // Mover kedua masih memegang snapshot lama (version 1) — persis kondisi
    // interleaving konkuren: harus ditolak VERSION_CONFLICT tanpa efek samping.
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_c", destinationListId: "ls_d2", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(CardVersionConflictError);

    const row = await db.client.execute("SELECT list_id, version FROM cards WHERE id = 'cd_c'");
    expect(row.rows[0]).toMatchObject({ list_id: "ls_d1", version: 2 });
    expect(await movedActivities("cd_c")).toHaveLength(1); // bukan dua activity
  });
  ...
  it("[INV-LIFE-003/004] negatif: move dari Card ARCHIVED dan DELETED → ditolak", async () => {
    ...
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_ar", destinationListId: "ls_d1", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(CardInvalidStateError);

    await seedCard("cd_de", "ls_s", { deletedAt: T0 });
    // DELETED dicek sebelum version — pun dengan version salah tetap INVALID_STATE
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_de", destinationListId: "ls_d1", expectedVersion: 99, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(CardInvalidStateError);
  });
  ...
  it("[Review-CL-02][INV-LIFE-001] negatif: source chain non-operational (Board source ARCHIVED) → AncestorNotActiveError", async () => {
    ...
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_so", destinationListId: "ls_m2", expectedVersion: 1, actorUserId: OWNER }),
  ...
  it("negatif: card tidak ada → RESOURCE_NOT_FOUND; destination tidak ada → INVALID_DESTINATION", async () => {
    ...
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_none", destinationListId: "ls_d1", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(CardNotFoundError);
    await expect(
      repo.moveCard(PROJECT, { cardId: "cd_ok", destinationListId: "ls_none", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(InvalidDestinationError);
  });
});
