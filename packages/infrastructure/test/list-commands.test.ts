...
import { AncestorNotActiveError, BoardNotFoundError, ListInvalidStateError, ListNotFoundError, ListValidationError, ListVersionConflictError, } from "@kanban/domain";
...
async function seedMilestone(id: string, opts: {
    archivedAt?: string | null;
    deletedAt?: string | null;
} = {}): Promise<void> {
    await db.client.execute({
...
async function seedBoard(id: string, milestoneId: string, opts: {
    archivedAt?: string | null;
    deletedAt?: string | null;
} = {}): Promise<void> {
    await db.client.execute({
...
async function seedList(id: string, boardId: string, opts: {
    archivedAt?: string | null;
    deletedAt?: string | null;
    title?: string;
} = {}): Promise<void> {
...
describe("createList — INV-LIFE-001 chain 3 level / FR-021 (goal 2.6.1)", () => {
    ...
    it("[INV-LIFE-001] negatif: Board ARCHIVED (Milestone+Project ACTIVE) → ditolak", async () => {
        ...
        await expect(repo.createList(PROJECT, { id: "ls_x", boardId: "bd_a", title: "X", actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    });
    it("[INV-LIFE-001] negatif: Milestone ARCHIVED (Board local ACTIVE) → ditolak", async () => {
        ...
        await expect(repo.createList(PROJECT, { id: "ls_x", boardId: "bd_m", title: "X", actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    });
    it("[INV-LIFE-001] negatif: Project DELETED (Board+Milestone ACTIVE) → ditolak", async () => {
        ...
        await expect(repo.createList(PROJECT, { id: "ls_x", boardId: "bd_p", title: "X", actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    ...
    it("negatif: board tidak ada → RESOURCE_NOT_FOUND (BoardNotFoundError)", async () => {
        await seedProject();
        await expect(repo.createList(PROJECT, { id: "ls_z", boardId: "bd_none", title: "Z", actorUserId: OWNER })).rejects.toBeInstanceOf(BoardNotFoundError);
    });
    it("[FR-021] negatif: title kosong → VALIDATION_ERROR", async () => {
        await seedProject();
        await seedMilestone("ms_t");
        await seedBoard("bd_t", "ms_t");
        await expect(repo.createList(PROJECT, { id: "ls_t", boardId: "bd_t", title: "  ", actorUserId: OWNER })).rejects.toBeInstanceOf(ListValidationError);
    });
});
describe("update/archive/delete List — A.3 / AC-020 / FR-023 (goal 2.6.1)", () => {
    it("positif: update title dari ACTIVE + Activity changes {before, after}", async () => {
        ...
        const activity = await db.client.execute("SELECT data FROM activities WHERE entity_id = 'ls_u' AND action = 'list.updated'");
    ...
    it("[FR-023] struktur record tanpa field status/wip/dll", async () => {
        ...
        expect(Object.keys(record!).sort()).toEqual(["archivedAt", "boardId", "createdAt", "deletedAt", "id", "title", "updatedAt", "version"].sort());
    });
    it("[AC-020] negatif: expected_version salah → VERSION_CONFLICT tanpa perubahan/activity", async () => {
        ...
        await expect(repo.updateList(PROJECT, { listId: "ls_v", expectedVersion: 99, actorUserId: OWNER, title: "Tabrak" })).rejects.toBeInstanceOf(ListVersionConflictError);
    ...
    it("[A.3] negatif: update dari ARCHIVED ditolak; restore dari DELETED tidak lewat state machine", async () => {
        ...
        await expect(repo.updateList(PROJECT, { listId: "ls_s", expectedVersion: 1, actorUserId: OWNER, title: "Gagal" })).rejects.toBeInstanceOf(ListInvalidStateError);
    });
    it("[INV-LIFE-002] negatif: restore List saat Board masih ARCHIVED → ditolak tanpa perubahan row", async () => {
        ...
        await expect(repo.restoreList(PROJECT, { listId: "ls_r", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    ...
    it("[INV-LIFE-002] positif: restore List saat chain 3 level ACTIVE semua → archivedAt null + Activity", async () => {
        ...
        const activity = await db.client.execute("SELECT data FROM activities WHERE entity_id = 'ls_ok' AND action = 'list.restored'");
    ...
    it("[A.3] delete dari ARCHIVED diizinkan dengan previous_state ARCHIVED", async () => {
        ...
        const activity = await db.client.execute("SELECT data, entity_version FROM activities WHERE entity_id = 'ls_d' AND action = 'list.deleted'");
        expect(JSON.parse(String(activity.rows[0]!.data))).toEqual({ previousState: "ARCHIVED" });
    ...
    it("[Review-CL-02][INV-LIFE-001] negatif: Board di-archive walau List local ACTIVE → update/archive/delete DITOLAK semua", async () => {
        ...
        await expect(repo.updateList(PROJECT, { listId: "ls_pa", expectedVersion: 1, actorUserId: OWNER, title: "Gagal" })).rejects.toBeInstanceOf(AncestorNotActiveError);
        await expect(repo.archiveList(PROJECT, { listId: "ls_pa", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
        await expect(repo.deleteList(PROJECT, { listId: "ls_pa", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    ...
    it("negatif: list tidak ada → RESOURCE_NOT_FOUND", async () => {
        await seedProject();
        await expect(repo.archiveList(PROJECT, { listId: "ls_none", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(ListNotFoundError);
    });
});
describe("[FR-022][BR-013] archive/delete List TIDAK cascade ke Card descendant (goal 2.6.1)", () => {
    it("archive List: row Card identik sebelum vs sesudah (state/version/list_id/parent utuh)", async () => {
        ...
        const activities = await db.client.execute("SELECT DISTINCT entity_type FROM activities WHERE action != 'list.created'");
