...
import { applyGlobalMigrations, createApiKey, hashApiKeySecret, listApiKeys, revokeApiKey, ApiKeyIdentityResolver, CompositeIdentityResolver, RequestPipeline, SqliteProjectDatabaseResolver, PipelineError, } from "../src/index.ts";
...
describe("API Key domain commands — goal 4.7.1", () => {
    ...
    it("[negatif] expiresAt masa lalu → VALIDATION_ERROR", async () => {
        await expect(createApiKey(globalClient, {
            projectId: PROJ_A,
            createdByUserId: USER,
            name: "Lama",
    ...
    it("[boundary] revoke key milik Project lain → RESOURCE_NOT_FOUND", async () => {
        ...
        await expect(revokeApiKey(globalClient, { projectId: PROJ_A, keyId: second.id })).rejects.toMatchObject({ code: "RESOURCE_NOT_FOUND" });
    });
});
describe("ApiKeyIdentityResolver + Composite — goal 4.7.2", () => {
    ...
    it("[AC-023/AC-024] revoked / expired / token tak dikenal → null", async () => {
        const resolver = new ApiKeyIdentityResolver(globalClient);
        expect(await resolver.resolveIdentity(fakeRequest(rawSecret))).toBeNull();
