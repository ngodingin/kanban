...
async function seedLabel(id: string, opts: { archivedAt?: string | null; deletedAt?: string | null } = {}): Promise<void> {
  await db.client.execute({
...
describe("Board Label domain commands — chain 3 level (goal 3.5.1)", () => {
  ...
  it("[Review-CL-02][WAJIB] archive Board dulu → update/archive/delete Board Label local-ACTIVE DITOLAK semua", async () => {
    ...
    await expect(
      repo.updateBoardLabel(PROJECT, { labelId: "bl_r2", expectedVersion: 1, actorUserId: OWNER, name: "X" }),
    ).rejects.toBeInstanceOf(AncestorNotActiveError);
    await expect(
      repo.archiveBoardLabel(PROJECT, { labelId: "bl_r2", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(AncestorNotActiveError);
    await expect(
      repo.deleteBoardLabel(PROJECT, { labelId: "bl_r2", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(AncestorNotActiveError);
  ...
  it("[transitive] archive Milestone SAJA (Board tetap ACTIVE) → mutasi Board Label tetap DITOLAK", async () => {
    ...
    for (const op of ["update", "archive", "delete"] as const) {
      ...
      const method = (repo as unknown as Record<string, (projectId: string, i: unknown) => Promise<unknown>>)[
        `${op}BoardLabel`
      ]!;
  ...
  it("[INV-LIFE-001] negatif: Project ARCHIVED walau Board+Milestone ACTIVE → create ditolak", async () => {
    ...
    await expect(
      repo.createBoardLabel(PROJECT, "bd_l", { id: "bl_x", name: "X", actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(AncestorNotActiveError);
  });
  ...
  it("[A.3][AC-020] update dari ARCHIVED ditolak; expected_version salah → VERSION_CONFLICT tanpa efek", async () => {
    ...
    await expect(
      repo.updateBoardLabel(PROJECT, { labelId: "bl_ar", expectedVersion: 1, actorUserId: OWNER, name: "Gagal" }),
    ).rejects.toBeInstanceOf(LabelInvalidStateError);

    await seedLabel("bl_v");
    await expect(
      repo.updateBoardLabel(PROJECT, { labelId: "bl_v", expectedVersion: 99, actorUserId: OWNER, name: "Tabrak" }),
  ...
  it("[INV-LIFE-002] restore ARCHIVED saat chain ACTIVE → sukses; saat Milestone ARCHIVED → ditolak", async () => {
    ...
    await expect(
      repo.restoreBoardLabel(PROJECT, { labelId: "bl_blk", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(AncestorNotActiveError);
  });
  ...
  it("[FR-034/B.5] title kosong ditolak; list exclude deleted; delete dari ARCHIVED sukses previous_state; tidak ada → NOT_FOUND", async () => {
    await seedChain();
    await expect(
      repo.createBoardLabel(PROJECT, "bd_l", { id: "bl_t", name: "  ", actorUserId: OWNER }),
    ...
    await expect(
      repo.archiveBoardLabel(PROJECT, { labelId: "bl_none", expectedVersion: 1, actorUserId: OWNER }),
