...
import { applyGlobalMigrations, createPersonalAccessToken, hashPatToken, listPersonalAccessTokens, revokePersonalAccessToken, PersonalAccessTokenIdentityResolver, CompositeIdentityResolver, } from "../src/index.ts";
...
let rawToken = "";
let tokenId = "";
const fakeRequest = (token?: string): Request => new Request("http://localhost/x", {
...
describe("PAT domain commands — goal 4.8.1", () => {
    ...
    it("[negatif] expiresAt masa lalu → VALIDATION_ERROR", async () => {
        await expect(createPersonalAccessToken(globalClient, {
            userId: USER_A,
            name: "Lama",
    ...
    it("[self-managed] User A TIDAK bisa revoke PAT milik User B → 404", async () => {
        ...
        await expect(revokePersonalAccessToken(globalClient, { userId: USER_A, tokenId: b.id })).rejects.toMatchObject({ code: "RESOURCE_NOT_FOUND" });
...
describe("PersonalAccessTokenIdentityResolver — goal 4.8.2", () => {
    ...
    it("[AC-023/AC-024] expired / revoked / unknown → null", async () => {
        ...
        await revokePersonalAccessToken(globalClient, { userId: USER_A, tokenId });
        expect(await resolver.resolveIdentity(fakeRequest(rawToken))).toBeNull();
