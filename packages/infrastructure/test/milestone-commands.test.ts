...
import { AncestorNotActiveError, MilestoneInvalidStateError, MilestoneNotFoundError, MilestoneValidationError, MilestoneVersionConflictError, } from "@kanban/domain";
...
async function seedProject(id: string = PROJECT, opts: {
    name?: string;
...
describe("createMilestone — BR-013/INV-LIFE-001/FR-014 (goal 2.2.1)", () => {
    ...
    it("[BR-013] negatif: Project ARCHIVED → create ditolak AncestorNotActiveError tanpa row/activity", async () => {
        ...
        await expect(repo.createMilestone(PROJECT, {
            id: "ms_x",
            title: "X",
            description: null,
            progress: 0,
...
describe("updateMilestone — AC-020/A.3/B.5 (goal 2.2.1)", () => {
    ...
    it("negatif: update dari ARCHIVED ditolak (A.3 update:[ACTIVE], INV-LIFE-003)", async () => {
        ...
        await expect(repo.updateMilestone(PROJECT, {
            milestoneId: "ms_a",
            expectedVersion: 1,
            actorUserId: OWNER,
            title: "Gagal",
    ...
    it("[Review-CL-02][INV-LIFE-001] negatif: Project di-archive lalu update/archive/delete Milestone (local ACTIVE) → DITOLAK semua", async () => {
        ...
        await expect(repo.archiveMilestone(PROJECT, { milestoneId: "ms_rev", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
        await expect(repo.deleteMilestone(PROJECT, { milestoneId: "ms_rev", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
        ...
        const msActivities = await db.client.execute("SELECT COUNT(*) AS n FROM activities WHERE entity_type = 'milestone'");
...
describe("archive/restore/delete — state machine A.3 (goal 2.2.1)", () => {
    ...
    it("[A.3] negatif: archive dari ARCHIVED ditolak; delete dari DELETED ditolak (terminal)", async () => {
        ...
        await expect(repo.archiveMilestone(PROJECT, { milestoneId: "ms_ar", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(MilestoneInvalidStateError);
        await seedMilestone("ms_del", { deletedAt: T0 });
        await expect(repo.deleteMilestone(PROJECT, { milestoneId: "ms_del", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(MilestoneInvalidStateError);
    });
    it("[INV-LIFE-002] negatif: restore ARCHIVED saat Project ARCHIVED ditolak — urutan benar: pulihkan Project dulu", async () => {
        ...
        await expect(repo.restoreMilestone(PROJECT, { milestoneId: "ms_r", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    ...
    it("[INV-LIFE-004] negatif: restore entity DELETED selalu ditolak walau ancestor ACTIVE", async () => {
        ...
        await expect(repo.restoreMilestone(PROJECT, { milestoneId: "ms_dead", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(MilestoneInvalidStateError);
    });
