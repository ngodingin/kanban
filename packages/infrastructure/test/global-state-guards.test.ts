...
describe("Guard current-state Global DB — BR-019/invariant #7 (goal 5.5.1 remediasi QA-CL-07)", () => {
  ...
  it("[revokeGroupAssignment] double-revoke → idempoten dengan revokedAt AKHIR yang sama (bukan timestamp lokal)", async () => {
    ...
    expect(r2.revokedAt).toBe(r1.revokedAt); // state pemenang, bukan now lokal
  });
  ...
  it("[QA-CL-07 skenario] accept ditahan pra-tx; revoke commit duluan → accept INVALID_STATE, tanpa accepted+revoked", async () => {
    ...
    // acceptInvitation pada koneksi terpisah yang DITAHAN sebelum tx dibuka
    ...
    const acceptPromise = (async () => {
      // replika alur acceptInvitation tetapi dengan gerbang pra-transaksi,
      // memakai fungsi produksi yang sama via parameterisasi manual:
      aStarted();
      await releaseAP;
    ...
    // Beri waktu accept melewati pre-check luar & sampai gerbang
    await startedP;

    // revoke commit SELAMA accept tertahan (persis skenario QA-CL-07)
  ...
  it("[arah kebalikan] revoke ditahan; accept commit duluan → revoke INVALID_STATE, accepted tetap tunggal", async () => {
    ...
    // accept berjalan TUNTUS lebih dulu
    const admin = await import("../src/database/project-admin.ts");
    ...
    // revoke setelahnya → INVALID_STATE, tidak mengubah apa pun
