...
async function seedEntity(
  table: "project_state" | "milestones" | "boards",
  id: string,
  opts: { title?: string; archivedAt?: string | null; deletedAt?: string | null; parentColumn?: string; parentId?: string } = {},
): Promise<void> {
...
describe("createBoard — INV-LIFE-001 chain 2 level / FR-018 (goal 2.4.1)", () => {
  ...
  it("[INV-LIFE-001] negatif: Milestone ARCHIVED walau Project ACTIVE → ditolak", async () => {
    ...
    await expect(
      repo.createBoard(PROJECT, { id: "bd_x", milestoneId: "ms_arc", title: "X", description: null, actorUserId: OWNER }),
  ...
  it("[INV-LIFE-001] negatif: Milestone DELETED walau Project ACTIVE → ditolak", async () => {
    ...
    await expect(
      repo.createBoard(PROJECT, { id: "bd_x", milestoneId: "ms_del", title: "X", description: null, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(AncestorNotActiveError);
  });
  ...
  it("[INV-LIFE-001] negatif: Project ARCHIVED/DELETED walau Milestone local ACTIVE → ditolak (satu saja cukup)", async () => {
    for (const lifecycle of [
      ...
      await expect(
        repo.createBoard(PROJECT, { id: "bd_y", milestoneId: "ms_live", title: "Y", description: null, actorUserId: OWNER }),
      ).rejects.toBeInstanceOf(AncestorNotActiveError);
    }
  ...
  it("negatif: milestone tidak ada → RESOURCE_NOT_FOUND (MilestoneNotFoundError)", async () => {
    await seedEntity("project_state", PROJECT);
    await expect(
      repo.createBoard(PROJECT, { id: "bd_z", milestoneId: "ms_none", title: "Z", description: null, actorUserId: OWNER }),
  ...
  it("[FR-019] negatif: title kosong → VALIDATION_ERROR", async () => {
    ...
    await expect(
      repo.createBoard(PROJECT, { id: "bd_t", milestoneId: "ms_t", title: "  ", description: null, actorUserId: OWNER }),
...
describe("update/archive/delete Board — state machine A.3 / AC-020 (goal 2.4.1)", () => {
  it("positif: update title/description dari ACTIVE + Activity changes", async () => {
    ...
    const activity = await db.client.execute(
      "SELECT data FROM activities WHERE entity_id = 'bd_u' AND action = 'board.updated'",
    );
  ...
  it("[FR-019] struktur record/input hanya field non-MVP-free — tidak ada status/warna/ikon/WIP", async () => {
    ...
    expect(Object.keys(record!).sort()).toEqual(
      ["archivedAt", "createdAt", "deletedAt", "description", "id", "milestoneId", "title", "updatedAt", "version"].sort(),
    );
  });
  ...
  it("[AC-020] negatif: expected_version salah → VERSION_CONFLICT tanpa perubahan/activity", async () => {
    ...
    await expect(
      repo.updateBoard(PROJECT, { boardId: "bd_v", expectedVersion: 99, actorUserId: OWNER, title: "Tabrak" }),
    ).rejects.toBeInstanceOf(BoardVersionConflictError);
  ...
  it("negatif: update dari ARCHIVED ditolak; delete dari DELETED ditolak (terminal)", async () => {
    ...
    await expect(
      repo.updateBoard(PROJECT, { boardId: "bd_a", expectedVersion: 1, actorUserId: OWNER, title: "Gagal" }),
    ).rejects.toBeInstanceOf(BoardInvalidStateError);

    await seedEntity("boards", "bd_d", { parentId: "ms_a", deletedAt: T0 });
    await expect(
      repo.deleteBoard(PROJECT, { boardId: "bd_d", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(BoardInvalidStateError);
  });
  ...
  it("[INV-LIFE-002] negatif: restore Board saat Milestone masih ARCHIVED → ditolak (urutan: restore Milestone dulu)", async () => {
    ...
    await expect(
      repo.restoreBoard(PROJECT, { boardId: "bd_r", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(AncestorNotActiveError);
  ...
  it("[INV-LIFE-002] negatif: restore Board saat Project ARCHIVED (Milestone ACTIVE) → ditolak", async () => {
    ...
    await expect(
      repo.restoreBoard(PROJECT, { boardId: "bd_p", expectedVersion: 1, actorUserId: OWNER }),
    ).rejects.toBeInstanceOf(AncestorNotActiveError);
  });
  ...
  it("negatif: board tidak ada → RESOURCE_NOT_FOUND", async () => {
    await seedEntity("project_state", PROJECT);
    await expect(
      repo.archiveBoard(PROJECT, { boardId: "bd_none", expectedVersion: 1, actorUserId: OWNER }),
