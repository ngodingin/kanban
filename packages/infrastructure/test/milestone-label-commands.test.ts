...
async function labelActivities(labelId?: string): Promise<Array<{
    entity_type: string;
    action: string;
    data: string;
}>> {
...
function project_client_query(sql: string, args: unknown[]): Promise<{
    rows: Record<string, unknown>[];
}> {
    return db.client.execute({ sql, args: args as never[] }) as Promise<{
        rows: Record<string, unknown>[];
    }>;
}
beforeAll(async () => {
...
describe("Milestone Label domain commands — C.11/INV-LIFE (goal 3.3.1)", () => {
    it("[FR-031][positif] create pada chain ACTIVE → version 1 + Activity entity_type milestone_label", async () => {
        ...
        expect(activity!.entity_type).toBe("milestone_label");
    ...
    it("[Review-CL-02][WAJIB] archive Milestone dulu → update/archive/delete Label local-ACTIVE DITOLAK semua", async () => {
        ...
        const milestoneRepo = new (await import("../src/database/milestone-repository.ts")).DrizzleMilestoneRepository(db.client);
        await milestoneRepo.archiveMilestone(PROJECT, { milestoneId: "ms_l", expectedVersion: 1, actorUserId: OWNER });
        await expect(repo.updateMilestoneLabel(PROJECT, { labelId: "ml_r2", expectedVersion: 1, actorUserId: OWNER, name: "X" })).rejects.toBeInstanceOf(AncestorNotActiveError);
        await expect(repo.archiveMilestoneLabel(PROJECT, { labelId: "ml_r2", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
        await expect(repo.deleteMilestoneLabel(PROJECT, { labelId: "ml_r2", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
        ...
        const labelActs = await db.client.execute("SELECT COUNT(*) AS n FROM activities WHERE entity_type = 'milestone_label'");
        expect(Number(labelActs.rows[0]?.n)).toBe(0);
        const msActs = await db.client.execute("SELECT COUNT(*) AS n FROM activities WHERE entity_type = 'milestone' AND action = 'milestone.archived'");
        expect(Number(msActs.rows[0]?.n)).toBe(1);
    });
    it("[INV-LIFE-001] negatif: Project ARCHIVED walau Milestone ACTIVE → create ditolak", async () => {
        await seedChain({ projectArchivedAt: T0 });
        await expect(repo.createMilestoneLabel(PROJECT, "ms_l", { id: "ml_x", name: "X", actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    });
    it("[A.3] update dari ARCHIVED ditolak; delete dari DELETED ditolak (terminal)", async () => {
        await seedChain();
        await seedLabel("ml_ar", { archivedAt: T0 });
        await expect(repo.updateMilestoneLabel(PROJECT, { labelId: "ml_ar", expectedVersion: 1, actorUserId: OWNER, name: "Gagal" })).rejects.toBeInstanceOf(LabelInvalidStateError);
        await seedLabel("ml_de", { deletedAt: T0 });
        await expect(repo.deleteMilestoneLabel(PROJECT, { labelId: "ml_de", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(LabelInvalidStateError);
    });
    it("[AC-020] expected_version salah → VERSION_CONFLICT tanpa perubahan/activity", async () => {
        await seedChain();
        await seedLabel("ml_v");
        await expect(repo.updateMilestoneLabel(PROJECT, { labelId: "ml_v", expectedVersion: 99, actorUserId: OWNER, name: "Tabrak" })).rejects.toBeInstanceOf(LabelVersionConflictError);
    ...
    it("[INV-LIFE-002] restore ARCHIVED saat ancestor ACTIVE → sukses; saat Project ARCHIVED → ditolak", async () => {
        ...
        await expect(repo.restoreMilestoneLabel(PROJECT, { labelId: "ml_blk", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(AncestorNotActiveError);
    });
    it("[B.5] archive/delete menyimpan previous_state; patch kosong → VALIDATION_ERROR; label tidak ada → NOT_FOUND", async () => {
        ...
        await expect(repo.updateMilestoneLabel(PROJECT, { labelId: "ml_e", expectedVersion: 1, actorUserId: OWNER, name: "L ml_e" })).rejects.toBeInstanceOf(LabelValidationError);
        await expect(repo.archiveMilestoneLabel(PROJECT, { labelId: "ml_none", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(LabelNotFoundError);
    });
});
