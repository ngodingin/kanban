...
/** Fixture LENGKAP satu rantai + label dua scope + junction + Activity/Comment. */
async function seedFullChain(client: Client, p: string, deletedAt: string | null): Promise<ChainIds> {
  ...
  // Activity untuk SETIAP level + Comment (action card.comment) pada Card.
...
/**
 * Verifikasi no-orphan ROW-LEVEL: setiap id yang wajib hilang dicek eksplisit
 * di tabelnya; survivor juga dicek masih ada.
 */
async function verifyNoOrphans(
  client: Client,
  pruned: ChainIds,
  survivors?: Partial<ChainIds>,
): Promise<void> {
...
describe("No-orphan integrity — property-style matriks subtree (goal 5.2.2)", () => {
  ...
  for (const shape of shapes) {
    it(`[BR-016] ${shape.name}: seluruh descendant ikut, NOL orphan row-level`, async () => {
      ...
      // Tandai level root DELETED 31 hari (level lain tetap ACTIVE).
      ...
      if (shape.rootLevel !== "milestone") survivors.ms = ids.ms;
      if (shape.rootLevel === "list" || shape.rootLevel === "card") survivors.bd = ids.bd;
      if (shape.rootLevel === "card") survivors.ls = ids.ls;
      // Label milik ancestor yang SELAMAT juga selamat
      if (shape.rootLevel !== "milestone") survivors.ml = ids.ml;
      if (shape.rootLevel !== "milestone" && shape.rootLevel !== "board") survivors.bl = ids.bl;

      // Pruned subtree = rantai DARI level root ke bawah; label milik ancestor selamat.
      ...
      // Untuk level non-root, cek "harus hilang" hanya berlaku utk level <= root;
      // placeholder id tidak ada di DB sehingga COUNT=0 otomatis terpenuhi.
  ...
  it("[campuran] chain eligible + Card independen eligible dalam SATU Project → keduanya bersih tanpa saling ganggu", async () => {
    const client = await makeProjectDb();
    const a = await seedFullChain(client, "aa", daysAgoIso(31)); // seluruh chain A eligible via ms
    const b = await seedFullChain(client, "bb", null); // chain B hidup
    await client.execute({
      sql: "UPDATE cards SET deleted_at = ? WHERE id = ?",
      args: [daysAgoIso(31), b.cd], // card B independen eligible
    });

    const result = await pruneDescendantSubtrees(client, NOW);
    expect(result.milestones).toBe(1);
    expect(result.cards).toBe(2); // a.cd (cascade) + b.cd (independen)

    await verifyNoOrphans(client, a, {});
    // Chain B: struktur selamat KECUALI card b.cd + junction/activity-nya
    ...
    // Activity non-card chain B tetap ada
