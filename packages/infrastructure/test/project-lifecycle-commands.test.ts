...
import { ProjectInvalidStateError, ProjectNotFoundError, ProjectVersionConflictError, } from "@kanban/domain";
...
async function seedProject(id: string, opts: {
    name?: string;
...
describe("DrizzleProjectRepository — project lifecycle domain commands (goal 1.1.1)", () => {
    ...
    describe("negatif — optimistic locking (BR-021, pola AC-020)", () => {
        ...
        it("penulis kedua yang masih memegang version lama ditolak — last valid write wins (BR-022)", async () => {
            ...
            await expect(repo.updateProjectName({ projectId: "proj_1", expectedVersion: 1, actorUserId: OWNER, name: "Kedua" })).rejects.toBeInstanceOf(ProjectVersionConflictError);
            const state = await repo.getProjectState("proj_1");
    ...
    describe("negatif — validasi current state sebelum commit (INV-LIFE-003, INV-LIFE-004)", () => {
        it("updateProjectName pada Project ARCHIVED ditolak (INV-LIFE-003)", async () => {
            await seedProject("proj_1", { archivedAt: T0, version: 2 });
            await expect(repo.updateProjectName({ projectId: "proj_1", expectedVersion: 2, actorUserId: OWNER, name: "X" })).rejects.toBeInstanceOf(ProjectInvalidStateError);
            expect(await activityCount()).toBe(0);
            const state = await repo.getProjectState("proj_1");
        ...
        it("archive pada Project DELETED ditolak (INV-LIFE-004)", async () => {
            ...
            await expect(repo.archiveProject({ projectId: "proj_1", expectedVersion: 3, actorUserId: OWNER })).rejects.toBeInstanceOf(ProjectInvalidStateError);
            expect(await activityCount()).toBe(0);
        });
        it("restore pada Project DELETED ditolak — DELETED terminal (INV-LIFE-004)", async () => {
            await seedProject("proj_1", { deletedAt: T0, version: 2 });
            await expect(repo.restoreProject({ projectId: "proj_1", expectedVersion: 2, actorUserId: OWNER })).rejects.toBeInstanceOf(ProjectInvalidStateError);
            const state = await repo.getProjectState("proj_1");
        ...
        it("restore pada Project ACTIVE bukan transisi valid (state machine A.3)", async () => {
            await seedProject("proj_1");
            await expect(repo.restoreProject({ projectId: "proj_1", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(ProjectInvalidStateError);
        });
        it("delete pada Project DELETED ditolak — tidak ada double-delete (INV-LIFE-004)", async () => {
            await seedProject("proj_1", { deletedAt: T0, version: 2 });
            await expect(repo.deleteProject({ projectId: "proj_1", expectedVersion: 2, actorUserId: OWNER })).rejects.toBeInstanceOf(ProjectInvalidStateError);
        });
    });
    describe("error lain", () => {
        it("command pada Project yang tidak ada → RESOURCE_NOT_FOUND tanpa Activity (BR-007 boundary)", async () => {
            await expect(repo.updateProjectName({ projectId: "proj_hilang", expectedVersion: 1, actorUserId: OWNER, name: "X" })).rejects.toMatchObject({ code: "RESOURCE_NOT_FOUND" });
            await expect(repo.archiveProject({ projectId: "proj_hilang", expectedVersion: 1, actorUserId: OWNER })).rejects.toBeInstanceOf(ProjectNotFoundError);
            expect(await activityCount()).toBe(0);
        });
    });
});
