...
import { PERMISSION_CATALOG, baselineGroupPermissionKeys, permissionCatalogKeys, seedPermissionCatalog, } from "../src/database/permission-catalog.ts";
let dir: string;
let client: Client;
beforeAll(async () => {
...
describe("seedPermissionCatalog — katalog permission D.1 (goal 1.5.1)", () => {
    ...
    it("[D.1] re-run seed dua kali berturut-turut idempotent: jumlah row sama, tidak ada duplikat, tidak error", async () => {
        ...
        const dupes = await client.execute("SELECT key, COUNT(*) AS n FROM permissions GROUP BY key HAVING n > 1");
...
describe("[DoD 3.1.1] provisioning Project baru menghasilkan baseline Manager dengan Label lifecycle penuh", () => {
    it("[D.2][end-to-end] group_permissions Manager memuat 12 key Label setelah registerProjectWithOwnerMembership", async () => {
        ...
        const managerRows = await client.execute(`SELECT p.key FROM group_permissions gp
       JOIN permission_groups g ON g.id = gp.group_id
       JOIN permissions p ON p.id = gp.permission_id
       WHERE g.project_id = ? AND g.name = 'Manager'`, [projectId]);
        ...
        const contributorRows = await client.execute(`SELECT p.key FROM group_permissions gp
       JOIN permission_groups g ON g.id = gp.group_id
       JOIN permissions p ON p.id = gp.permission_id
       WHERE g.project_id = ? AND g.name = 'Contributor'`, [projectId]);
