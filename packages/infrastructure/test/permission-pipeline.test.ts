...
const fakeRequest = (userId: string): Request =>
  new Request("http://localhost/x", { headers: { "x-test-user": userId } }) as unknown as Request;
...
beforeAll(async () => {
  ...
  // Assignment scope Project utk member: card.move via Group + direct member.invite ALL
  await globalClient.execute({
...
describe("RequestPipeline + RealPermissionResolver — goal 4.3.1", () => {
  it("[BR-038] Member dengan assignment scope Project → grantedKeys berisi key di-grant + visibility terluas", async () => {
    ...
    expect(ctx.permission.cardReadVisibility).toBe("ASSIGNED_TO_ME"); // widest card.read applicable
  });
  ...
  it("[BR-037] Owner → seluruh key katalog granted terlepas assignment", async () => {
    ...
    expect(ctx.permission.grantedKeys.size).toBeGreaterThan(40); // katalog penuh
    expect(ctx.permission.grantedKeys.has("card.move")).toBe(true);
