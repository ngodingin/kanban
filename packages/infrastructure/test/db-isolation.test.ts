...
describe("DB test terisolasi per suite (04-DEL B.5)", () => {
    ...
    it("AC-001/BR-007: suite memakai DB file terpisah dengan 10 tabel termigrasi", async () => {
    ...
    it("AC-001/BR-007: rollback antar test — data test sebelumnya tidak terlihat (truncate per test)", async () => {
    ...
    it("AC-001/BR-007: test lain dimulai bersih (tidak ada sisa proj_a)", async () => {
        const rows = await db.client.execute("SELECT COUNT(*) n FROM project_state");
        expect(Number(rows.rows[0]?.n)).toBe(0);
    });
});
describe("isolasi antar suite: DB berbeda, state tidak bocor", () => {
    it("AC-001/BR-007: suite kedua punya DB kosong sendiri (state tidak bocor lintas suite)", async () => {
