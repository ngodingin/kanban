...
import { applyGlobalMigrations, cleanupAssigneesForRevokedMembership, newProjectId, registerProjectWithOwnerMembership, revokeMembership, unassignCardFromRevokedMember, } from "../src/index.ts";
...
describe("unassignCardFromRevokedMember — atomik per Card (goal 2.12.1)", () => {
    it("[FR-026][BR-054] positif: assignee NULL + Activity card.unassigned payload B.5 + creator utuh", async () => {
        ...
        const card = await project.client.execute("SELECT assignee_user_id, creator_user_id, version FROM cards WHERE id = 'cd_1'");
        expect(card.rows[0]).toMatchObject({ assignee_user_id: null, creator_user_id: "creator-awal", version: 2 });
        const activity = await project.client.execute("SELECT action, data, actor_user_id FROM activities WHERE entity_id = 'cd_1'");
    ...
    it("[atomik per-Card] UPDATE gagal → tidak ada Activity yatim tanpa mutation yang sesuai", async () => {
        ...
        const failing = {
            transaction: async () => {
                ...
                return {
                    execute: async (sqlOrOpts: string | {
                        sql: string;
                        args?: InArgs;
                    }, maybeArgs?: InArgs) => {
        ...
        await expect(unassignCardFromRevokedMember(failing, { cardId: failId, revokedUserId: "user-x", actorUserId: OWNER_ACTOR })).rejects.toThrow();
        expect(txCount).toBeGreaterThan(0);
        const orphanActivity = await project.client.execute("SELECT COUNT(*) AS n FROM activities WHERE entity_id = 'cd_2' AND action = 'card.unassigned'");
...
describe("cleanupAssigneesForRevokedMembership — best-effort per Card (goal 2.12.1)", () => {
    it("[FR-026] 3 Card user-x dibersihkan masing-masing; Card lain & creator tak tersentuh", async () => {
        await seedCards();
        const result = await cleanupAssigneesForRevokedMembership(project.client, projectIdValue, "user-x", OWNER_ACTOR);
        expect(result.cleaned).toBe(3);
        expect(result.skipped).toBe(0);
        for (const id of ["cd_1", "cd_2", "cd_3"]) {
            const card = await project.client.execute("SELECT assignee_user_id, creator_user_id, version FROM cards WHERE id = ?", [id]);
            expect(card.rows[0]).toMatchObject({ assignee_user_id: null, creator_user_id: "creator-awal", version: 2 });
        }
        for (const [id, assignee] of [
            ["cd_other", "user-y"],
        ...
        const activities = await project.client.execute("SELECT entity_id, data FROM activities WHERE action = 'card.unassigned' ORDER BY entity_id");
        expect(activities.rows).toHaveLength(3);
    ...
    it("[best-effort] kegagalan di tengah loop → card lain tetap dibersihkan, card gagal tak tersentuh & tanpa Activity", async () => {
        ...
        const orphanActivity = await project.client.execute("SELECT COUNT(*) AS n FROM activities WHERE entity_id = 'cd_2'");
        expect(Number(orphanActivity.rows[0]?.n)).toBe(0);
    });
});
describe("revokeMembership + cleanup — integrasi lintas-DB nyata (goal 2.12.1)", () => {
    it("[DoD] Global revoke → Project DB Card assignee ikut NULL; idempoten", async () => {
        await seedCards();
        const summary = await revokeMembership(globalClient, { projectId: projectIdValue, membershipId: MEMBERSHIP_X, actorUserId: OWNER_ACTOR }, project.client);
        expect(summary.revokedAt).not.toBeNull();
        const xCards = await project.client.execute("SELECT COUNT(*) AS n FROM cards WHERE assignee_user_id = 'user-x'");
        expect(Number(xCards.rows[0]?.n)).toBe(0);
        const activities = await project.client.execute("SELECT COUNT(*) AS n FROM activities WHERE action = 'card.unassigned'");
        expect(Number(activities.rows[0]?.n)).toBe(3);
        await revokeMembership(globalClient, { projectId: projectIdValue, membershipId: MEMBERSHIP_X, actorUserId: OWNER_ACTOR }, project.client);
        const after = await project.client.execute("SELECT COUNT(*) AS n FROM activities WHERE action = 'card.unassigned'");
        expect(Number(after.rows[0]?.n)).toBe(3);
    });
    it("[kompatibilitas] revokeMembership tanpa projectDb tetap jalan (perilaku Phase 1)", async () => {
        await seedCards();
        const summary = await revokeMembership(globalClient, { projectId: projectIdValue, membershipId: MEMBERSHIP_Y }, null);
        ...
        expect(other.rows[0]).toMatchObject({ assignee_user_id: "user-y" });
    });
});
