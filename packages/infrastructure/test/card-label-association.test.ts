...
// Goal 3.7.1 (02-SPEC C.11, FR-032/033/034) — assign/remove Label ke Card,
// server menentukan scope, histori dipertahankan (insert baris baru, bukan
// reuse baris removed_at lama).
...
describe("assignLabelToCard/removeLabelFromCard (goal 3.7.1)", () => {
  it("[positif] assign Milestone Label & Board Label milik posisi Card saat ini → sukses, Activity label.added", async () => {
    ...
    const msRow = await db.client.execute(
      "SELECT card_id, label_id, removed_at FROM card_milestone_labels WHERE card_id = 'c_1'",
    );
    expect(msRow.rows[0]).toMatchObject({ card_id: "c_1", label_id: "ml_1", removed_at: null });
    const bdRow = await db.client.execute(
      "SELECT card_id, label_id, removed_at FROM card_board_labels WHERE card_id = 'c_1'",
    );
    expect(bdRow.rows[0]).toMatchObject({ card_id: "c_1", label_id: "bl_1", removed_at: null });

    const activities = await db.client.execute(
      "SELECT action, entity_version, data FROM activities WHERE entity_id = 'c_1' ORDER BY created_at",
    );
  ...
  it("[FR-033] remove lalu assign ulang → baris BARU (histori utuh, bukan reuse baris removed_at lama)", async () => {
    ...
    const afterRemove = await db.client.execute(
      "SELECT COUNT(*) n FROM card_milestone_labels WHERE card_id = 'c_1' AND label_id = 'ml_1'",
    );
    expect(Number(afterRemove.rows[0]!.n)).toBe(1);

    await assignLabelToCard(db.client, "c_1", "ml_1", OWNER);
    const afterReassign = await db.client.execute(
      "SELECT removed_at FROM card_milestone_labels WHERE card_id = 'c_1' AND label_id = 'ml_1' ORDER BY created_at",
    );
    ...
    const activities = await db.client.execute(
      "SELECT action FROM activities WHERE entity_id = 'c_1' ORDER BY created_at",
    );
