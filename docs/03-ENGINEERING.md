...
- **Resend webhook (email deliverability, Phase 6 scope — belum diimplementasi):** saat diimplementasikan, endpoint webhook MUST menangani minimal `email.bounced` dan `email.complained` (sinyal kesehatan pengiriman Magic Link — bounce berarti user tidak bisa login sama sekali, complaint berisiko domain pengirim diblokir provider lain); `email.delivered`/`email.delivery_delayed` MAY ditambahkan untuk visibilitas dukungan (mis. menjawab keluhan "email tidak sampai"). `email.sent` tidak perlu (sudah diketahui dari response API sinkron). **Keamanan:** open/click tracking Resend MUST NOT diaktifkan untuk email Magic Link — tracking membungkus URL token sekali-pakai dengan redirect pihak ketiga milik Resend, melemahkan properti single-use/hashed/short-expiring yang dikunci A.14; karenanya `email.opened`/`email.clicked` tidak relevan ditrack.
