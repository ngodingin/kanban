import { acceptInvitation, assertProjectOwner, createCachedProjectDbClientFactory, createGroupAssignment, createInvitation, createPermissionAssignment, createPermissionGroup, DbIdempotencyStore, deletePermissionGroup, listPermissionGroups, listProjectInvitations, listProjectMembers, listProjectSummaries, listMembershipAssignments, assertPermissionKey, createEntityPermissionResolver, newProjectId, provisionProjectWithMapping, RequestPipeline, requireActiveMember, revokeGroupAssignment, assertAssigneeNotRevocationPending, revokeMembership, revokeInvitation, revokePermissionAssignment, SqliteProjectDatabaseResolver, updatePermissionGroup, type IdentityResolver, type TursoEnv, } from "@kanban/infrastructure";
...
export function buildProjectRoutesDeps(input: BuildProjectRoutesDepsInput): ProjectRoutesDeps {
    ...
    return {
        ...
        createProject: async (projInput) => {
            if (!turso) {
                throw new Error("Provisioning Project baru membutuhkan TURSO_API_TOKEN/TURSO_GROUP (database Turso nyata dibuat per Project).");
            }
        ...
        listProjects: (userId, statusFilter) => listProjectSummaries(globalClient, databaseResolver, projectClientFactory, userId, statusFilter),
...
export function buildCardRoutesDeps(input: BuildCardRoutesDepsInput): CardRoutesDeps {
    ...
    return {
        ...
        assertAssigneeActiveMember: (projectId, userId) => assertAssigneeNotRevocationPending(globalClient, projectId, userId),
    };
}
...
function buildProjectContextDeps(identityResolver: IdentityResolver, globalClient: Client, turso: TursoEnv | null): {
...
export function buildProjectAdminDeps(input: {
    ...
    return {
        ...
        createPermissionGroup: async (projectId, payload) => createPermissionGroup(globalClient, {
            projectId,
            name: payload.name,
            description: payload.description,
            permissions: payload.permissions,
        }),
        updatePermissionGroup: async (projectId, groupId, payload) => updatePermissionGroup(globalClient, {
            projectId,
            groupId,
        ...
        createGroupAssignment: async (projectId, membershipId, input) => createGroupAssignment(globalClient, {
            projectId,
            membershipId,
        ...
        revokeGroupAssignment: (projectId, membershipId, assignmentId) => revokeGroupAssignment(globalClient, { projectId, membershipId, assignmentId }),
        createPermissionAssignment: async (projectId, membershipId, input) => createPermissionAssignment(globalClient, {
            projectId,
            membershipId,
        ...
        revokePermissionAssignment: (projectId, membershipId, assignmentId) => revokePermissionAssignment(globalClient, { projectId, membershipId, assignmentId }),
        createInvitation: async (projectId, invitedByUserId, input) => createInvitation(globalClient, {
            projectId,
            invitedByUserId,
            email: input.email,
        ...
        listMembershipAssignments: (projectId, membershipId) => listMembershipAssignments(globalClient, projectId, membershipId),
        idempotencyStore,
