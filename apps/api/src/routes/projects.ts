...
import { apiError, extractIdempotencyKey, ok, toErrorResponse, type ErrorEnvelope, } from "@kanban/contracts";
import { DrizzleProjectRepository, PipelineError, ResolveIdentityStep, type ProjectStateRecord, type ProjectStatus, type ProjectSummary, type ResolvedIdentity, } from "@kanban/infrastructure";
import type { Client } from "@libsql/client";
import type { EffectivePermissions } from "@kanban/domain";
import { hasPermission, loadEntityHierarchy, type RouteEntityType, } from "@kanban/infrastructure";
...
export async function authorize(ctx: OpenProjectContext, key: string, projectId: string, entity?: {
    ...
    if (entity) {
        ...
        if (path)
            effective = await ctx.effectiveFor(path);
        else
            effective = await ctx.effectiveFor(entity.type === "milestone"
...
export interface IdempotencyStoreLike {
    ...
    complete(key: string, scope: string, claimToken: string, response: {
        status: number;
        body: unknown;
    }): Promise<boolean>;
...
function readStatusFilter(raw: string | undefined): ProjectStatus[] | undefined {
    ...
    for (const value of values) {
        if (!(PROJECT_STATUS_VALUES as readonly string[]).includes(value)) {
            throw new PipelineError("VALIDATION_ERROR", `Nilai status '${value}' tidak dikenal; gunakan subset ACTIVE,ARCHIVED,DELETED.`, 400);
        }
...
export function toApiErrorResponse(error: unknown): {
    ...
    if (error instanceof Error && typeof (error as {
        code?: unknown;
    }).code === "string") {
        return toErrorResponse({ code: (error as {
                code?: unknown;
            }).code as string, message: error.message });
    }
...
export class ValidationCollector {
    private readonly details: Array<{
        field: string;
        reason: string;
    }> = [];
...
type LifecycleCommand = (repository: DrizzleProjectRepository, input: {
    projectId: string;
    expectedVersion: number;
    actorUserId: string;
}) => Promise<ProjectStateRecord>;
async function handleLifecycle(c: Context, deps: ProjectRoutesDeps, projectId: string, command: LifecycleCommand): Promise<{
    project: ReturnType<typeof projectStatePayload>;
}> {
...
export async function withIdempotentHandling<T>(c: Context, deps: {
    resolveIdentity(request: Request): Promise<{
        userId: string;
    } | null>;
}, handler: () => Promise<T>, successStatus: ContentfulStatusCode = 200, idempotencyStore?: IdempotencyStoreLike): Promise<Response> {
    if (!idempotencyStore)
        return withErrorHandling(c, handler, successStatus);
    ...
    if (claim.status === "conflict") {
        return c.json(apiError("IDEMPOTENCY_CONFLICT", "Idempotency-Key ini sudah dipakai untuk request dengan payload berbeda."), 409);
    }
    if (claim.status === "in_progress") {
        return c.json(apiError("IDEMPOTENCY_IN_PROGRESS", "Request dengan Idempotency-Key ini masih diproses eksekusi pertama."), 409);
    }
...
export function createProjectsRouter(getDeps: () => ProjectRoutesDeps): Hono {
    ...
    router.get("/v1/projects/:project_id", async (c) => {
        return withErrorHandling(c, async () => {
            ...
            if (!state) {
                throw new PipelineError("RESOURCE_NOT_FOUND", `project_state ${c.req.param("project_id")} tidak ditemukan.`, 404);
            }
            return {
    ...
    router.patch("/v1/projects/:project_id", async (c) => {
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            const ctx = await deps.openProjectContext(c.req.raw, projectId);
            if (ctx.ownerUserId !== ctx.userId) {
                throw new PipelineError("PERMISSION_DENIED", "Hanya Owner Project yang dapat mengubah nama (interim Phase 1).", 403);
            }
    ...
    router.post("/v1/projects/:project_id/archive", async (c) => {
        const deps = getDeps();
        return withIdempotentHandling(c, deps, () => handleLifecycle(c, deps, c.req.param("project_id"), (repository, input) => repository.archiveProject(input)), 200, deps.idempotencyStore);
    });
    router.post("/v1/projects/:project_id/restore", async (c) => {
        const deps = getDeps();
        return withIdempotentHandling(c, deps, () => handleLifecycle(c, deps, c.req.param("project_id"), (repository, input) => repository.restoreProject(input)), 200, deps.idempotencyStore);
    });
    router.post("/v1/projects/:project_id/delete", async (c) => {
        const deps = getDeps();
        return withIdempotentHandling(c, deps, () => handleLifecycle(c, deps, c.req.param("project_id"), (repository, input) => repository.deleteProject(input)), 200, deps.idempotencyStore);
    });
    return router;
}
