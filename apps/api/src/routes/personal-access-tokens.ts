...
import { PipelineError, ResolveIdentityStep, type PersonalAccessTokenSummary, type ResolvedIdentity, } from "@kanban/infrastructure";
...
async function withErrorHandling<T>(c: import("hono").Context, handler: () => Promise<T>, successStatus: ContentfulStatusCode = 200): Promise<Response> {
    try {
...
export function createPersonalAccessTokensRouter(getDeps: () => PersonalAccessTokensRoutesDeps): Hono {
    ...
    router.post("/v1/me/personal-access-tokens", async (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
    ...
    router.get("/v1/me/personal-access-tokens", async (c) => withErrorHandling(c, async () => {
        const deps = getDeps();
        const identity = await new ResolveIdentityStep({ resolveIdentity: deps.resolveIdentity }).run(c.req.raw);
        const tokens = await deps.listPersonalAccessTokens(identity.userId);
        return { personalAccessTokens: tokens };
    }));
    router.post("/v1/me/personal-access-tokens/:token_id/revoke", async (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
        const identity = await new ResolveIdentityStep({ resolveIdentity: deps.resolveIdentity }).run(c.req.raw);
