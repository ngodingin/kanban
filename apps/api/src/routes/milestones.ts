...
import { DrizzleMilestoneRepository, PipelineError, type MilestoneRecord, type ResolvedIdentity, } from "@kanban/infrastructure";
...
export function createMilestonesRouter(getDeps: () => MilestoneRoutesDeps): Hono {
    ...
    router.get("/v1/projects/:project_id/milestones/:milestone_id", async (c) => {
        return withErrorHandling(c, async () => {
            ...
            if (!record) {
                throw new PipelineError("RESOURCE_NOT_FOUND", `Milestone ${c.req.param("milestone_id")} tidak ditemukan.`, 404);
            }
        ...
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            for (const key of Object.keys(body)) {
                if (!(allowedFields as readonly string[]).includes(key) && key !== "expectedVersion") {
                    throw new PipelineError("VALIDATION_ERROR", `Field '${key}' tidak dapat diubah via PATCH Milestone (C.15).`, 400);
                }
            }
            const repository = new DrizzleMilestoneRepository(ctx.database);
    ...
    const lifecycleCommands = {
        archive: (repository: DrizzleMilestoneRepository, projectId: string, input: {
            milestoneId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.archiveMilestone(projectId, input),
        restore: (repository: DrizzleMilestoneRepository, projectId: string, input: {
            milestoneId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.restoreMilestone(projectId, input),
        delete: (repository: DrizzleMilestoneRepository, projectId: string, input: {
            milestoneId: string;
            expectedVersion: number;
            actorUserId: string;
