...
import { DrizzleCardRepository, listCardLabels, listCardLabelsForCards, PipelineError, type CardLabelSummary, type CardRecord, type ResolvedIdentity, } from "@kanban/infrastructure";
import { authorize, readExpectedVersionField, readJsonObject, toApiErrorResponse, ValidationCollector, withIdempotentHandling, type IdempotencyStoreLike, type OpenProjectContext, } from "./projects.ts";
...
export function createCardsRouter(getDeps: () => CardRoutesDeps): Hono {
    ...
    router.get("/v1/projects/:project_id/cards/:card_id", async (c) => {
        return withErrorHandling(c, async () => {
            ...
            if (!record) {
                throw new PipelineError("RESOURCE_NOT_FOUND", `Card ${c.req.param("card_id")} tidak ditemukan.`, 404);
            }
        ...
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            for (const key of Object.keys(body)) {
                if (!(allowedFields as readonly string[]).includes(key) && key !== "expectedVersion") {
                    throw new PipelineError("VALIDATION_ERROR", `Field '${key}' tidak dapat diubah via PATCH Card${key === "listId" ? " — move wajib via /cards/:id/move (BR-017)" : " (C.15)"}.`, 400);
                }
            }
            const repository = new DrizzleCardRepository(ctx.database, {
                assertAssigneeActiveMember: deps.assertAssigneeActiveMember,
        ...
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            for (const key of Object.keys(body)) {
                if (key !== "destinationListId" && key !== "expectedVersion") {
                    throw new PipelineError("VALIDATION_ERROR", `Field '${key}' tidak dikenal pada payload move (C.8).`, 400);
                }
            }
            const collector = new ValidationCollector();
    ...
    const lifecycleCommands = {
        archive: (repository: DrizzleCardRepository, projectId: string, input: {
            cardId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.archiveCard(projectId, input),
        restore: (repository: DrizzleCardRepository, projectId: string, input: {
            cardId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.restoreCard(projectId, input),
        delete: (repository: DrizzleCardRepository, projectId: string, input: {
            cardId: string;
            expectedVersion: number;
            actorUserId: string;
