...
import { PipelineError, ResolveIdentityStep, type AcceptInvitationResult, type GroupAssignmentSummary, type MembershipAssignmentsList, type InvitationSummary, type InvitationListSummary, type PermissionAssignmentSummary, type PermissionGroupSummary, type ProjectMemberSummary, type ResolvedIdentity, } from "@kanban/infrastructure";
...
export interface ProjectAdminRoutesDeps {
    ...
    listPermissionGroups(projectId: string, requesterUserId: string, opts: {
    ...
    updatePermissionGroup(projectId: string, groupId: string, input: UpdatePermissionGroupPayload): Promise<PermissionGroupSummary>;
    deletePermissionGroup(projectId: string, groupId: string): Promise<PermissionGroupSummary>;
    createGroupAssignment(projectId: string, membershipId: string, input: {
        groupId: string;
        scopeType: string;
        scopeId: string;
    }): Promise<GroupAssignmentSummary>;
    revokeGroupAssignment(projectId: string, membershipId: string, assignmentId: string): Promise<GroupAssignmentSummary>;
    createPermissionAssignment(projectId: string, membershipId: string, input: {
        permissionId: string;
        scopeType: string;
        scopeId: string;
        cardReadVisibility?: string | null;
    }): Promise<PermissionAssignmentSummary>;
    revokePermissionAssignment(projectId: string, membershipId: string, assignmentId: string): Promise<PermissionAssignmentSummary>;
    createInvitation(projectId: string, invitedByUserId: string, input: {
        email: string;
        assignments: Array<{
            groupId: string;
            scopeType: string;
    ...
    listMembers(projectId: string, requesterUserId: string, opts: {
    ...
    listMembershipAssignments(projectId: string, membershipId: string): Promise<MembershipAssignmentsList | null>;
...
function readPermissionEntries(rawPermissions: unknown): Array<{
    permissionId: string;
    cardReadVisibility?: string | null;
}> {
    if (!Array.isArray(rawPermissions))
        throw new PipelineError("VALIDATION_ERROR", "Field permissions wajib array.", 400);
...
export function createProjectAdminRouter(getDeps: () => ProjectAdminRoutesDeps): Hono {
    ...
    router.get("/v1/projects/:project_id/permission-groups", (c) => withErrorHandling(c, async () => {
        const deps = getDeps();
    ...
    router.post("/v1/projects/:project_id/permission-groups", (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
        const projectId = c.req.param("project_id");
        const identity = await new ResolveIdentityStep({
            resolveIdentity: deps.resolveIdentity,
    ...
    router.patch("/v1/projects/:project_id/permission-groups/:group_id", (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
        const projectId = c.req.param("project_id");
    ...
    router.post("/v1/projects/:project_id/permission-groups/:group_id/delete", (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
        const projectId = c.req.param("project_id");
        const groupId = c.req.param("group_id");
        const identity = await new ResolveIdentityStep({
    ...
    router.post("/v1/projects/:project_id/members/:membership_id/group-assignments", (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
        const projectId = c.req.param("project_id");
    ...
    router.post("/v1/projects/:project_id/members/:membership_id/group-assignments/:assignment_id/revoke", (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
        const projectId = c.req.param("project_id");
        const identity = await new ResolveIdentityStep({
            resolveIdentity: deps.resolveIdentity,
        ...
        await deps.assertProjectOwner(projectId, identity.userId);
        const assignment = await deps.revokeGroupAssignment(projectId, c.req.param("membership_id"), c.req.param("assignment_id"));
        return { assignment };
    }, 200, getDeps().idempotencyStore));
    ...
    router.post("/v1/projects/:project_id/members/:membership_id/permission-assignments/:assignment_id/revoke", async (c) => {
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            await deps.assertProjectOwner(projectId, identity.userId);
            const assignment = await deps.revokePermissionAssignment(projectId, c.req.param("membership_id"), c.req.param("assignment_id"));
            return { assignment };
        });
    });
    ...
    router.post("/v1/invitations/:invitation_id/accept", (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
        const identity = await new ResolveIdentityStep({
            resolveIdentity: deps.resolveIdentity,
        }).run(c.req.raw);
