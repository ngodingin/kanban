...
import { PipelineError, ResolveIdentityStep, type ApiKeySummary, type ResolvedIdentity, } from "@kanban/infrastructure";
...
export function createApiKeysRouter(getDeps: () => ApiKeysRoutesDeps): Hono {
    ...
    router.post("/v1/projects/:project_id/api-keys", async (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
    ...
    router.get("/v1/projects/:project_id/api-keys", async (c) => withErrorHandling(c, async () => {
        const deps = getDeps();
        const projectId = c.req.param("project_id");
        const identity = await new ResolveIdentityStep({ resolveIdentity: deps.resolveIdentity }).run(c.req.raw);
    ...
    router.post("/v1/projects/:project_id/api-keys/:key_id/revoke", async (c) => withIdempotentHandling(c, getDeps(), async () => {
        const deps = getDeps();
        const projectId = c.req.param("project_id");
