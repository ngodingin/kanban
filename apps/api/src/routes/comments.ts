...
import { addComment, editComment, PipelineError, type CardCommentRecord, type EditCommentRecord, type ResolvedIdentity, } from "@kanban/infrastructure";
...
export function createCommentsRouter(getDeps: () => CommentRoutesDeps): Hono {
    ...
    router.patch("/v1/projects/:project_id/cards/:card_id/comments/:activity_id", async (c) => {
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            const body = collector.collect("body", () => readBodyField(rawBody));
            collector.throwIfAny();
            const edited = await editComment(ctx.database, c.req.param("card_id"), c.req.param("activity_id"), body!, ctx.userId);
