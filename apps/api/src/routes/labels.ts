...
import { DrizzleBoardLabelRepository, DrizzleMilestoneLabelRepository, PipelineError, type BoardLabelRecord, type MilestoneLabelRecord, type ResolvedIdentity, } from "@kanban/infrastructure";
import { authorize, readExpectedVersionField, readJsonObject, toApiErrorResponse, ValidationCollector, type OpenProjectContext, withIdempotentHandling, type IdempotencyStoreLike, } from "./projects.ts";
...
export function createMilestoneLabelsRouter(getDeps: () => MilestoneLabelRoutesDeps): Hono {
    ...
    router.post("/v1/projects/:project_id/milestones/:milestone_id/labels", async (c) => {
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            for (const key of Object.keys(body)) {
                if (key !== "name") {
                    throw new PipelineError("VALIDATION_ERROR", `Field '${key}' tidak dikenal pada payload create Label (C.11).`, 400);
                }
            }
            const repository = new DrizzleMilestoneLabelRepository(ctx.database);
        ...
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            for (const key of Object.keys(body)) {
                if (key !== "name" && key !== "expectedVersion") {
                    throw new PipelineError("VALIDATION_ERROR", `Field '${key}' tidak dapat diubah via PATCH Label (C.15).`, 400);
                }
            }
            const repository = new DrizzleMilestoneLabelRepository(ctx.database);
    ...
    const lifecycleCommands = {
        archive: (repository: DrizzleMilestoneLabelRepository, projectId: string, input: {
            labelId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.archiveMilestoneLabel(projectId, input),
        restore: (repository: DrizzleMilestoneLabelRepository, projectId: string, input: {
            labelId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.restoreMilestoneLabel(projectId, input),
        delete: (repository: DrizzleMilestoneLabelRepository, projectId: string, input: {
            labelId: string;
            expectedVersion: number;
            actorUserId: string;
...
export function createBoardLabelsRouter(getDeps: () => BoardLabelRoutesDeps): Hono {
    ...
    const lifecycleCommands = {
        archive: (repository: DrizzleBoardLabelRepository, projectId: string, input: {
            labelId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.archiveBoardLabel(projectId, input),
        restore: (repository: DrizzleBoardLabelRepository, projectId: string, input: {
            labelId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.restoreBoardLabel(projectId, input),
        delete: (repository: DrizzleBoardLabelRepository, projectId: string, input: {
            labelId: string;
            expectedVersion: number;
            actorUserId: string;
