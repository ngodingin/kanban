...
import { DrizzleBoardRepository, PipelineError, type BoardRecord, type ResolvedIdentity, } from "@kanban/infrastructure";
...
export function createBoardsRouter(getDeps: () => BoardRoutesDeps): Hono {
    ...
    router.get("/v1/projects/:project_id/boards/:board_id", async (c) => {
        return withErrorHandling(c, async () => {
            ...
            if (!record) {
                throw new PipelineError("RESOURCE_NOT_FOUND", `Board ${c.req.param("board_id")} tidak ditemukan.`, 404);
            }
        ...
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            for (const key of Object.keys(body)) {
                if (!(allowedFields as readonly string[]).includes(key) && key !== "expectedVersion") {
                    throw new PipelineError("VALIDATION_ERROR", `Field '${key}' tidak dapat diubah via PATCH Board (C.15).`, 400);
                }
            }
            const repository = new DrizzleBoardRepository(ctx.database);
    ...
    const lifecycleCommands = {
        archive: (repository: DrizzleBoardRepository, projectId: string, input: {
            boardId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.archiveBoard(projectId, input),
        restore: (repository: DrizzleBoardRepository, projectId: string, input: {
            boardId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.restoreBoard(projectId, input),
        delete: (repository: DrizzleBoardRepository, projectId: string, input: {
            boardId: string;
            expectedVersion: number;
            actorUserId: string;
