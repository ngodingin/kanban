...
import { DrizzleListRepository, PipelineError, type ListRecord, type ResolvedIdentity, } from "@kanban/infrastructure";
...
export function createListsRouter(getDeps: () => ListRoutesDeps): Hono {
    ...
    router.get("/v1/projects/:project_id/lists/:list_id", async (c) => {
        return withErrorHandling(c, async () => {
            ...
            if (!record) {
                throw new PipelineError("RESOURCE_NOT_FOUND", `List ${c.req.param("list_id")} tidak ditemukan.`, 404);
            }
        ...
        return withIdempotentHandling(c, getDeps(), async () => {
            ...
            for (const key of Object.keys(body)) {
                if (key !== "title" && key !== "expectedVersion") {
                    throw new PipelineError("VALIDATION_ERROR", `Field '${key}' tidak dapat diubah via PATCH List (C.15/FR-023).`, 400);
                }
            }
            const repository = new DrizzleListRepository(ctx.database);
    ...
    const lifecycleCommands = {
        archive: (repository: DrizzleListRepository, projectId: string, input: {
            listId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.archiveList(projectId, input),
        restore: (repository: DrizzleListRepository, projectId: string, input: {
            listId: string;
            expectedVersion: number;
            actorUserId: string;
        }) => repository.restoreList(projectId, input),
        delete: (repository: DrizzleListRepository, projectId: string, input: {
            listId: string;
            expectedVersion: number;
            actorUserId: string;
