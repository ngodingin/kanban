...
import { BetterAuthIdentityResolver, CompositeIdentityResolver, DbIdempotencyStore, assertPermissionKey, pruneAllRegisteredProjects, createApiKey, createPersonalAccessToken, listPersonalAccessTokens, revokePersonalAccessToken, listApiKeys, revokeApiKey, createAuth, createGlobalClient, loadAppConfig, readTursoEnvFromProcess, type SendMagicLinkData, } from "@kanban/infrastructure";
...
export function createApiApp(opts: {
    ...
    const getPatDeps = (): PersonalAccessTokensRoutesDeps => {
        ...
        if (!deps) {
            ...
            deps = {
                ...
                createPersonalAccessToken: (input) => createPersonalAccessToken(r.globalClient, input),
                revokePersonalAccessToken: (userId, tokenId) => revokePersonalAccessToken(r.globalClient, { userId, tokenId }),
    ...
    const getApiKeysDeps = (): ApiKeysRoutesDeps => {
        ...
        if (!deps) {
            ...
            deps = {
                resolveIdentity: (request) => identityResolver.resolveIdentity(request),
                assertPermissionKey: (projectId, requesterUserId, key) => assertPermissionKey(r.globalClient, projectId, requesterUserId, key),
