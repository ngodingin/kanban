...
beforeAll(async () => {
    ...
    const provision = async (projectId: string, projectName: string, ownerUserId: string, lifecycle: {
...
describe("GET /api/v1/projects/:project_id/milestones/:milestone_id — goal 2.3.1", () => {
    ...
    it("[C.5][C.2] member membaca milestone via pipeline", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${seeded.projectId}/milestones/${seeded.milestoneId}`, { headers: { "x-test-user": "user-a" } });
    ...
    it("[INV-04] Project-boundary: milestone Project lain tidak bocor — akses non-member ditolak", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${seeded.projectId}/milestones/${seeded.milestoneId}`, { headers: { "x-test-user": "user-b" } });
        expect(res.status).toBe(403);
        expect((await res.json()).error?.code).toBe("PROJECT_ACCESS_DENIED");
    });
    ...
    it("[C.2] GET tanpa identitas → TOKEN_EXPIRED 401", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${seeded.projectId}/milestones/${seeded.milestoneId}`);
        expect(res.status).toBe(401);
        expect((await res.json()).error?.code).toBe("TOKEN_EXPIRED");
    });
});
