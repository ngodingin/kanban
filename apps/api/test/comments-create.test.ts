...
describe("POST /api/v1/projects/:project_id/cards/:card_id/comments — goal 3.11.1", () => {
    it("[C.10][BR-030] Owner comment pada Card ACTIVE → 201, Activity comment.added dengan entity_version = versi Card TERKINI", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/cards/c_active/comments`, {
            method: "POST",
        ...
        try {
            const row = (await projectDb.execute("SELECT action, entity_type, entity_version, data FROM activities WHERE id = ?", [
    ...
    it("[BR-033] Comment pada Card ARCHIVED ditolak — INVALID_STATE 409", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/cards/c_archived/comments`, {
            method: "POST",
            headers: { "x-test-user": "user-a", "content-type": "application/json" },
    ...
    it("[BR-033] Comment pada Card DELETED ditolak — INVALID_STATE 409", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/cards/c_deleted/comments`, {
            method: "POST",
            headers: { "x-test-user": "user-a", "content-type": "application/json" },
            body: JSON.stringify({ body: "X" }),
        });
    ...
    it("[Validasi + Authz] body kosong 400; non-Owner 403; tanpa identitas 401; Card tidak ada 404", async () => {
        const emptyBody = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/cards/c_active/comments`, {
            method: "POST",
            headers: { "x-test-user": "user-a", "content-type": "application/json" },
        ...
        const denied = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/cards/c_active/comments`, {
            method: "POST",
        ...
        const noIdentity = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/cards/c_active/comments`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ body: "X" }),
        });
        expect(noIdentity.status).toBe(401);
        const notFound = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/cards/c_missing/comments`, {
            method: "POST",
            headers: { "x-test-user": "user-a", "content-type": "application/json" },
            body: JSON.stringify({ body: "X" }),
        });
