...
import { applyGlobalMigrations, applyProjectMigrations, DrizzleProjectRepository, listProjectSummaries, newProjectId, registerProjectWithOwnerMembership, SqliteProjectDatabaseResolver, } from "@kanban/infrastructure";
...
beforeAll(async () => {
    ...
    ctx = {
        ...
        deps: {
            ...
            listProjects: (userId, statusFilter) => listProjectSummaries(globalClient, new SqliteProjectDatabaseResolver(globalClient), {
...
describe("GET /api/v1/projects — list Project bermembership aktif (goal 1.3.2)", () => {
    it("[C.4][A.4] list berisi Project dengan membership aktif + status ringkas benar (ACTIVE dan ARCHIVED), tanpa transaksi lintas-DB", async () => {
        ...
        const projects = json.data.projects as Array<{
            id: string;
            name: string;
            status: string;
        }>;
        ...
        if (byStatus["Proj A1"] !== "ACTIVE")
            throw new Error(`Proj A1 harusnya ACTIVE: ${JSON.stringify(byStatus)}`);
        if (byStatus["Proj A2"] !== "ARCHIVED")
            throw new Error(`Proj A2 harusnya ARCHIVED (baca state terkini dari Project DB): ${JSON.stringify(byStatus)}`);
        for (const p of projects) {
            if (typeof p.id !== "string" || p.id.length !== 29)
                throw new Error(`id tidak sesuai: ${p.id}`);
        }
    });
    it("[INV-04][C.4] boundary: Project milik User lain tidak muncul, dan membership revoked tidak menghasilkan entri", async () => {
        ...
        const json = await res.json();
        const ids = (json.data.projects as Array<{
            id: string;
        }>).map((p) => p.id);
        ...
        const projectsB = jsonB.data.projects as Array<{
            id: string;
            name: string;
        }>;
        ...
        if (Number(revokedProbe.rows[0]!.n) !== 1)
            throw new Error("fixture membership revoked hilang");
...
describe("GET /api/v1/projects?status= — filter subset status (goal 1.3.5)", () => {
    it("[C.4] positif: ?status=ACTIVE hanya mengembalikan project ACTIVE", async () => {
        ...
        if (res.status !== 200)
            throw new Error(`status ${res.status}: ${await res.text()}`);
        const projects = (await res.json()).data.projects as Array<{
            name: string;
            status: string;
        }>;
    ...
    it("[C.4] positif: ?status=ARCHIVED / DELETED / kombinasi comma-separated", async () => {
        ...
        if (deleted.length !== 0)
            throw new Error(`hasil filter DELETED harusnya kosong: ${JSON.stringify(deleted)}`);
        ...
        if (both.length !== 2)
            throw new Error(`hasil filter gabungan salah: ${JSON.stringify(both)}`);
    });
    it("[C.12] negatif: nilai status tidak dikenal → VALIDATION_ERROR 400", async () => {
        for (const query of ["?status=bogus", "?status=ACTIVE,archived"]) {
            ...
            if (res.status !== 400)
                throw new Error(`${query}: status ${res.status}, harusnya 400`);
            const json = await res.json();
            if (json.error?.code !== "VALIDATION_ERROR")
                throw new Error(`${query}: code ${json.error?.code}`);
        }
    });
    it("[C.4] tanpa param tetap mengembalikan semua status (default tidak berubah)", async () => {
        ...
        if (projects.length !== 2)
            throw new Error(`default berubah: ${JSON.stringify(projects)}`);
    });
});
