...
// Goal 1.7.4 — POST /api/v1/projects/:project_id/permission-groups/:group_id/delete
// (C.12 amandemen 2.1.0): soft-delete set deleted_at; BR-041 — member kehilangan
// grant tanpa penghapusan riwayat membership_group_assignments; Owner-only interim.
...
describe("POST /api/v1/projects/:project_id/permission-groups/:group_id/delete (goal 1.7.4)", () => {
  ...
  beforeAll(async () => {
    ...
    groupId = created.id;
    // Assignment aktif — bukti BR-041 bahwa delete tidak menyentuhnya.
  ...
  it("[BR-041][C.12] Owner soft-delete: deleted_at ter-set, hilang dari default list, riwayat assignment utuh", async () => {
    ...
    if (json.data.group.deletedAt === null) throw new Error("deleted_at tidak ter-set");

    const active = await ctx.deps.listPermissionGroups(ctx.projectIdA, "user-a", { includeDeleted: false });
    if (active.some((g) => g.id === groupId)) throw new Error("group terhapus masih di default list");
    ...
    if (!deleted || deleted.deletedAt === null) throw new Error("group tidak muncul di include_deleted");
  ...
  it("[Rule-3] negatif: non-Owner ditolak 403 PERMISSION_DENIED", async () => {
    ...
    if (check.rows[0]!.deleted_at !== null) throw new Error("group ikut terhapus pada percobaan non-Owner");
  });
  ...
  it("[INV-04] negatif: group milik Project lain lewat path Project ini → 404, group B utuh", async () => {
    ...
    if (check.rows[0]!.deleted_at !== null) throw new Error("group Project B ikut terhapus — boundary bocor");
  });
