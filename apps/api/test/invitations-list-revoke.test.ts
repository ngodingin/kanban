...
describe("GET /projects/:project_id/invitations & POST /invitations/:id/revoke (goals 1.9.3 & 1.9.4)", () => {
    it("[C.13][FR-006] Positif 1.9.3: list mengembalikan seluruh invitation project", async () => {
        ...
        await ctx.globalClient.execute({
            ...
            args: [new Date(Date.now() - 60000).toISOString(), inv3],
        });
        ...
        if (invitations.length !== 3)
            throw new Error(`expected 3 invitations, got ${invitations.length}`);
        ...
        if (!accepted)
            throw new Error("accepted invitation not found in list");
        if (!revoked)
            throw new Error("revoked invitation not found in list");
        if (!pending)
            throw new Error("pending (expired) invitation not found in list");
    });
    it("[C.13][FR-006] Negatif 1.9.4: revoke pending → revokedAt ter-set", async () => {
        ...
        if (res.status !== 200)
            throw new Error(`revoke pending failed: ${res.status}`);
        const json = await res.json();
        if (json.data.invitation.revokedAt === null)
            throw new Error("revokedAt tidak ter-set");
    });
    it("[C.13][FR-006] Negatif 1.9.4: revoke accepted → INVALID_STATE", async () => {
        ...
        if (revokeRes.status !== 409)
            throw new Error(`expected 409, got ${revokeRes.status}`);
        const json = await revokeRes.json();
        if (json.error.code !== "INVALID_STATE")
            throw new Error(`expected INVALID_STATE, got ${json.error.code}`);
    });
    it("[C.13][BR-007/BR-009] Project boundary: list invitations tidak bocor lintas Project", async () => {
        ...
        if (listRes.status !== 200)
            throw new Error(`list projectB failed: ${listRes.status}`);
        const json = await listRes.json();
        if (json.data.invitations.length !== 0)
            throw new Error(`projectB should have 0 invitations, got ${json.data.invitations.length}`);
    });
});
