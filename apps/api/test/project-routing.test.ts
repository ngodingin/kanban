...
import { applyGlobalMigrations, applyProjectMigrations, DrizzleProjectRepository, newProjectId, PipelineError, registerProjectWithOwnerMembership, type ResolvedIdentity, } from "@kanban/infrastructure";
...
describe("routing Project DB lewat wiring produksi (regression QA-CL-04..09)", () => {
    it("[INV-04][BR-007][A.4] listProjects memetakan tiap project ke DB-nya sendiri — nama per id tidak tertukar", async () => {
        ...
        if (summaries.length !== 2)
            throw new Error(`jumlah salah: ${JSON.stringify(summaries)}`);
        const byId = Object.fromEntries(summaries.map((s) => [s.id, s.name]));
        if (byId[ctx.idA] !== "Alpha Routing")
            throw new Error(`pairing A salah: ${JSON.stringify(byId)}`);
        if (byId[ctx.idB] !== "Beta Routing")
            throw new Error(`pairing B salah: ${JSON.stringify(byId)}`);
    ...
    it("[A.4] negatif: projectId tak dikenal ditolak RESOURCE_NOT_FOUND 404 oleh pipeline produksi", async () => {
        ...
        if (!(caught instanceof PipelineError))
            throw new Error(`harusnya PipelineError: ${String(caught)}`);
    ...
    it("[BR-007][C.4] endpoint GET detail lewat router+wiring produksi membaca DB yang benar", async () => {
        ...
        if (resA.status !== 200)
            throw new Error(`status A ${resA.status}: ${await resA.text()}`);
        const jsonA = await resA.json();
        if (jsonA.data.project.name !== "Alpha Renamed")
            throw new Error(`nama A salah: ${JSON.stringify(jsonA)}`);
        const resB = await app.request(`http://localhost/v1/projects/${ctx.idB}`, {
            headers: { "x-test-user": "user-b" },
        });
        const jsonB = await resB.json();
        if (jsonB.data.project.name !== "Beta Renamed")
            throw new Error(`nama B salah: ${JSON.stringify(jsonB)}`);
    });
    ...
    it("[C.4] negatif: request tanpa identitas ditolak sebelum operasi apa pun", async () => {
        ...
        if (res.status !== 401)
            throw new Error(`harusnya 401, dapat ${res.status}: ${await res.text()}`);
    });
});
