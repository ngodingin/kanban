...
import { applyGlobalMigrations, createPersonalAccessToken, listPersonalAccessTokens, revokePersonalAccessToken, } from "@kanban/infrastructure";
import type { ResolvedIdentity } from "@kanban/infrastructure";
import { createPersonalAccessTokensRouter, type PersonalAccessTokensRoutesDeps, } from "../src/routes/personal-access-tokens.ts";
...
describe("POST/GET/revoke /me/personal-access-tokens — goal 4.8.1/4.8.2 (self-managed, tanpa permission key)", () => {
    ...
    it("[isolasi] list hanya milik identity yang resolve, bukan seluruh User", async () => {
        ...
        expect(jsonA.data.personalAccessTokens.map((t: {
            name: string;
        }) => t.name)).toEqual(["CLI"]);
        ...
        expect(jsonB.data.personalAccessTokens.map((t: {
            name: string;
        }) => t.name)).toEqual(["B token"]);
    });
    it("[boundary] User A revoke token milik User B → RESOURCE_NOT_FOUND (bukan 403 — analog BR-034A)", async () => {
        ...
        const denied = await app().request(`http://localhost/v1/me/personal-access-tokens/${tokenId}/revoke`, { method: "POST", headers: { "x-test-user": "user-a" } });
        expect(denied.status).toBe(404);
        expect((await denied.json()).error?.code).toBe("RESOURCE_NOT_FOUND");
        const ok = await app().request(`http://localhost/v1/me/personal-access-tokens/${tokenId}/revoke`, { method: "POST", headers: { "x-test-user": "user-b" } });
