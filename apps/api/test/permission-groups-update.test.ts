...
describe("PATCH /api/v1/projects/:project_id/permission-groups/:group_id (goal 1.7.3)", () => {
    ...
    it("[BR-040][C.12] REPLACE permission set berlaku langsung; riwayat assignment member utuh", async () => {
        ...
        if (res.status !== 200)
            throw new Error(`status ${res.status}: ${await res.text()}`);
        const keys = (await res.json()).data.group.permissions.map((p: {
            key: string;
        }) => p.key).sort();
    ...
    it("[INV-04] negatif: PATCH group milik Project lain lewat path Project ini → RESOURCE_NOT_FOUND", async () => {
        ...
        if (res.status !== 404)
            throw new Error(`harusnya 404, dapat ${res.status}: ${await res.text()}`);
    ...
    it("[C.12] negatif: permission tak dikenal / duplikat / field asing / tanpa field → VALIDATION_ERROR 400", async () => {
        ...
        for (const body of [
            ...
            if (res.status !== 400)
                throw new Error(`body ${JSON.stringify(body)} harusnya 400, dapat ${res.status}`);
            const json = await res.json();
            if (json.error.code !== "VALIDATION_ERROR")
                throw new Error(`kode salah untuk ${JSON.stringify(body)}: ${JSON.stringify(json)}`);
        }
    });
});
