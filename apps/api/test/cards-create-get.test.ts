...
beforeAll(async () => {
    ...
    ctx = {
        ...
        deps: {
            ...
            assertAssigneeActiveMember: (projectId, userId) => requireActive(globalClient, projectId, userId),
        },
    };
});
...
describe("POST /api/v1/projects/:project_id/lists/:list_id/cards — goal 2.9.1", () => {
    it("[FR-024][FR-025][C.8] Owner membuat card dengan assignee → 201 + creator=actor + Activity", async () => {
        ...
        try {
            const activity = await projectDb.execute("SELECT action FROM activities WHERE entity_type = 'card'");
