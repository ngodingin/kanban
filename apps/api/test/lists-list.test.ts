...
describe("GET /api/v1/projects/:project_id/boards/:board_id/lists — goal 2.7.4", () => {
    it("[C.7] mengembalikan seluruh List Board tsb (termasuk ARCHIVED), List Board LAIN tidak muncul", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/boards/bd_1/lists`, { headers: { "x-test-user": "user-a" } });
        expect(res.status).toBe(200);
        const json = await res.json();
        const ids = json.data.lists.map((l: {
            id: string;
        }) => l.id).sort();
        expect(ids).toEqual(["l_1", "l_2"]);
    });
    it("[Boundary/Authz] tanpa membership → PROJECT_ACCESS_DENIED; tanpa identitas → 401", async () => {
        const denied = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/boards/bd_1/lists`, { headers: { "x-test-user": "user-stranger" } });
        expect(denied.status).toBe(403);
        const noIdentity = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/boards/bd_1/lists`);
