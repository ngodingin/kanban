...
describe("GET /api/v1/projects/:project_id/activities — goal 3.10.1", () => {
    it("[C.9] generic tanpa filter mengembalikan seluruh 5 Activity Project ini, terurut created_at ASC", async () => {
        ...
        expect(json.data.activities.map((a: {
            id: string;
        }) => a.id)).toEqual(["act_1", "act_2", "act_3", "act_4", "act_5"]);
    });
    it("[C.9] filter entity_type+entity_id mengembalikan HANYA Activity entity tsb", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/activities?entity_type=card&entity_id=c_1`, { headers: { "x-test-user": "user-a" } });
        const json = await res.json();
        expect(json.data.activities).toHaveLength(2);
        expect(json.data.activities.every((a: {
            entityType: string;
        }) => a.entityType === "card")).toBe(true);
    });
    it("[C.9] filter actor+action+from/to menyempitkan hasil dengan benar", async () => {
        const byActor = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/activities?actor=user-b`, { headers: { "x-test-user": "user-a" } });
        expect((await byActor.json()).data.activities).toHaveLength(1);
        const byAction = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/activities?action=card.created`, { headers: { "x-test-user": "user-a" } });
        expect((await byAction.json()).data.activities).toHaveLength(1);
        const byRange = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/activities?from=2026-08-01T00:00:02.000Z&to=2026-08-01T00:00:03.000Z`, { headers: { "x-test-user": "user-a" } });
        expect((await byRange.json()).data.activities.map((a: {
            id: string;
        }) => a.id)).toEqual(["act_2", "act_3"]);
    });
    it("[Convenience routes] card/milestone/board/list mengembalikan subset identik dengan generic+filter manual", async () => {
        ...
        for (const [path, entityType] of routes) {
            ...
            expect(json.data.activities.every((a: {
                entityType: string;
            }) => a.entityType === entityType)).toBe(true);
        }
    });
    it("[Authz + boundary] tanpa identitas 401; non-member 403; Activity Project lain tidak pernah muncul", async () => {
        ...
        const ids = (await projA.json()).data.activities.map((a: {
            id: string;
        }) => a.id);
