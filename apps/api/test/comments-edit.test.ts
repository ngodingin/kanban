...
// Goal 3.12.1 (02-SPEC C.10, BR-031/032/034A) — PATCH edit comment: Activity
// baru comment.edited, lama tidak pernah berubah; :activity_id selalu
// merujuk comment.added original.
...
beforeAll(async () => {
  ...
  // Comment original oleh Owner (user-a).
  await projectClient.execute({
  ...
  // Comment "milik user lain" (disisipkan langsung, BR-034A belum reachable
  // via addComment interim — dokumentasi invariant utk Phase 4).
  await projectClient.execute({
  ...
  // Activity non-comment, milik Card yang sama — harus ditolak sebagai target edit.
  await projectClient.execute({
...
describe("PATCH .../comments/:activity_id — goal 3.12.1", () => {
  it("[BR-031/032] edit comment milik sendiri → 200, Activity BARU comment.edited, Activity comment.added lama TIDAK berubah", async () => {
    ...
    try {
      const original = (
        await projectDb.execute("SELECT action, data FROM activities WHERE id = 'act_orig'")
      ).rows[0];
      expect(original).toMatchObject({ action: "comment.added" });
      expect(JSON.parse(String(original!.data))).toEqual({ body: "Versi pertama" });

      const edited = (
        await projectDb.execute("SELECT action, entity_type, data FROM activities WHERE id = ?", [json.data.comment.id])
      ).rows[0];
