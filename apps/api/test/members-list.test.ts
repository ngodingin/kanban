...
describe("GET /members (goal 1.10.1)", () => {
    it("[FR-008][C.12] Tanpa status → keduanya (aktif 3 + revoked 1), bentuk payload benar", async () => {
        ...
        if (members.length !== 4)
            throw new Error(`harusnya 4 membership, dapat ${members.length}`);
        const revoked = members.find((m) => m.membershipId === revokedMembershipId);
        if (!revoked || typeof revoked.revokedAt !== "string")
            throw new Error("membership revoked tidak muncul tanpa filter");
    ...
    it("[C.12][2.4.0] status=active hanya aktif; status=revoked hanya revoked; kombinasi keduanya", async () => {
        ...
        if (both.length !== 4)
            throw new Error(`kombinasi harusnya 4, dapat ${both.length}`);
    });
    ...
    it("[Rule-3][INV-04] negatif: non-member Project ini → 403; boundary antar Project terjaga", async () => {
        const res = await listMembers("user-x");
        ...
        if (outsider.status !== 403)
            throw new Error(`list lintas Project harusnya 403, dapat ${outsider.status}`);
