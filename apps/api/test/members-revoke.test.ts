...
import { applyGlobalMigrations, newProjectId, registerProjectWithOwnerMembership, applyProjectMigrations, } from "@kanban/infrastructure";
...
function revoke(projectId: string, membershipId: string, user: string) {
    return makeRouter().then((router) => router.request(`http://localhost/v1/projects/${projectId}/members/${membershipId}/revoke`, {
        method: "POST",
...
describe("POST /members/:membership_id/revoke (goal 1.10.2)", () => {
    it("[BR-053][C.12] Positif: revoke set revoked_at; row & assignment riwayat tetap utuh", async () => {
        ...
        if ((await res.json()).data.membership.revokedAt === null)
        ...
        if (membership.rows[0]!.revoked_at === null)
            throw new Error("DB revoked_at tidak ter-set");
    ...
    it("[C.12] Revoke ulang → idempotent, timestamp pertama dipertahankan", async () => {
        ...
        if (first.status !== 200)
            throw new Error(`revoke pertama gagal: ${first.status}`);
        const again = await revoke(ctx.projectIdA, `m-user-b-${ctx.projectIdA}`, "user-a");
        if (again.status !== 200)
            throw new Error(`revoke kedua harusnya idempotent 200, dapat ${again.status}`);
        const firstAt = (await first.json()).data.membership.revokedAt;
        if ((await again.json()).data.membership.revokedAt !== firstAt)
            throw new Error("timestamp berubah saat re-revoke");
    });
    it("[FR-002] negatif: Owner Membership ditolak INVALID_STATE dan Owner tetap aktif", async () => {
        ...
        if (Number(owners.rows[0]!.n) < 1)
            throw new Error("Project kehilangan seluruh membership aktif");
    });
