...
describe("GET+POST /api/v1/projects/:project_id/boards/:board_id/labels — goal 3.6.1", () => {
    it("[C.11] Owner membuat label → 201 envelope data.label + Activity board_label.created", async () => {
        ...
        try {
            const activity = await projectDb.execute("SELECT entity_type, action FROM activities WHERE entity_id = ?", [json.data.label.id]);
...
describe("PATCH .../boards/:board_id/labels/:label_id — goal 3.6.2", () => {
    it("[C.15] Owner update name → 200; field asing ditolak; non-Owner 403", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/boards/bd_1/labels/bl_patch`, {
            method: "PATCH",
            headers: { "x-test-user": "user-a", "content-type": "application/json" },
        ...
        const foreignField = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/boards/bd_1/labels/bl_patch`, {
            method: "PATCH",
            headers: { "x-test-user": "user-a", "content-type": "application/json" },
        ...
        const denied = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/boards/bd_1/labels/bl_patch`, {
            method: "PATCH",
            headers: { "x-test-user": "user-b", "content-type": "application/json" },
...
describe("POST .../boards/:board_id/labels/:label_id/{archive,restore,delete} — goal 3.6.3", () => {
    function post(action: string, labelId: string, body: unknown, user = "user-a"): Promise<Response> {
        return makeApp().request(`http://localhost/v1/projects/${projectIdValue}/boards/bd_1/labels/${labelId}/${action}`, {
            method: "POST",
