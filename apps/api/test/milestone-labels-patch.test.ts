...
function patch(body: unknown, user = "user-a"): Promise<Response> {
    return makeApp().request(`http://localhost/v1/projects/${projectIdValue}/milestones/ms_l/labels/ml_patch`, {
        method: "PATCH",
...
describe("PATCH .../milestones/:milestone_id/labels/:label_id — goal 3.4.2", () => {
    it("[C.11][B.5] Owner update name → 200 + Activity milestone_label.updated changes", async () => {
        ...
        try {
            const activity = await projectDb.execute("SELECT data FROM activities WHERE entity_id = 'ml_patch' AND action = 'milestone_label.updated'");
    ...
    it("[Authz interim] non-Owner member → PERMISSION_DENIED 403; label tidak ada → RESOURCE_NOT_FOUND 404", async () => {
        ...
        const missing = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/milestones/ms_l/labels/ml_none`, {
            method: "PATCH",
