...
const patchMs = (mid: string, user: string): Promise<Response> => new Hono().route("/", createMilestonesRouter(() => ctx.msDeps)).request(`http://localhost/v1/projects/${projectIdValue}/milestones/${mid}`, {
    method: "PATCH",
    headers: { "x-test-user": user, "content-type": "application/json" },
    body: JSON.stringify({ expectedVersion: 1, title: `${mid} by ${user}` }),
});
const patchBd = (bid: string, user: string): Promise<Response> => new Hono().route("/", createBoardsRouter(() => ctx.bdDeps)).request(`http://localhost/v1/projects/${projectIdValue}/boards/${bid}`, {
    method: "PATCH",
    headers: { "x-test-user": user, "content-type": "application/json" },
