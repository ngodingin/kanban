...
describe("permission-assignments endpoints (goal 1.8.2)", () => {
    ...
    it("[B.2][C.12] Positif: card.read + visibility eksplisit ALL tersimpan", async () => {
        ...
        if (res.status !== 201)
            throw new Error(`status ${res.status}: ${await res.text()}`);
        if ((await res.json()).data.assignment.cardReadVisibility !== "ALL")
            throw new Error("visibility ALL tidak tersimpan");
    });
    it("[B.2] negatif: visibility pada non-card.read → VALIDATION_ERROR 400", async () => {
        for (const key of ["board.read", "list.read"]) {
            ...
            const res = await assignPerm(ctx.projectIdA, membershipIdB, { permissionId: permId, scopeType: "project", scopeId: ctx.projectIdA, cardReadVisibility: "ALL" }, "user-a");
            if (res.status !== 400)
                throw new Error(`${key} harusnya 400, dapat ${res.status}`);
            if ((await res.json()).error.code !== "VALIDATION_ERROR")
                throw new Error(`kode salah untuk ${key}`);
        }
    });
    it("[UNIQUE] negatif: duplikat aktif → INVALID_STATE 409", async () => {
        ...
        if (first.status !== 201)
            throw new Error(`setup pertama gagal: ${first.status}`);
    ...
    it("[BR-042B][INV-04] negatif: scope salah / permission tak ada / membership lintas-Project ditolak", async () => {
        ...
        for (const body of [
            ...
            if (res.status !== expected)
                throw new Error(`body ${JSON.stringify(body)} harusnya ${expected}, dapat ${res.status}`);
        }
    ...
    it("[C.12] revoke: revoked_at ter-set, row tetap; re-revoke idempotent; re-assign bebas", async () => {
        ...
        if (reassign.status !== 201)
            throw new Error(`re-assign setelah revoke gagal: ${await reassign.text()}`);
    });
