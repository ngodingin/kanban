...
describe("GET /api/v1/projects/:project_id/milestones/:milestone_id/boards — goal 2.5.4", () => {
    it("[C.6] mengembalikan seluruh Board Milestone tsb (termasuk ARCHIVED), Board Milestone LAIN tidak muncul", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/milestones/ms_1/boards`, { headers: { "x-test-user": "user-a" } });
        expect(res.status).toBe(200);
        const json = await res.json();
        const ids = json.data.boards.map((b: {
            id: string;
        }) => b.id).sort();
        expect(ids).toEqual(["bd_1", "bd_2"]);
    });
    it("[Boundary/Authz] tanpa membership → PROJECT_ACCESS_DENIED; tanpa identitas → 401", async () => {
        const denied = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/milestones/ms_1/boards`, { headers: { "x-test-user": "user-stranger" } });
        expect(denied.status).toBe(403);
        const noIdentity = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/milestones/ms_1/boards`);
