...
describe("POST /api/v1/projects/:project_id/permission-groups (goal 1.7.2)", () => {
    it("[C.12][FR-010][FR-011] Owner membuat custom group + permission set; card.read tanpa visibility → CREATED_BY_ME", async () => {
        ...
        const res = await makeApp().request(`http://localhost/v1/projects/${ctx.projectIdA}/permission-groups`, {
            ...
            body: JSON.stringify({
                ...
                permissions: [
                    { permissionId: cardRead },
                    { permissionId: projectRead, cardReadVisibility: null },
                ],
            }),
        });
        if (res.status !== 201)
        ...
        if (json.data.group.name !== "Release Ops")
        ...
        if (!created)
            throw new Error("group tidak persisten");
        const byKey = Object.fromEntries(created.permissions.map((p) => [p.key, p.cardReadVisibility]));
        if (byKey["card.read"] !== "CREATED_BY_ME")
            throw new Error(`default visibility salah: ${JSON.stringify(byKey)}`);
        ...
        if (created.permissions.length !== 2)
            throw new Error(`jumlah permission salah: ${JSON.stringify(created.permissions)}`);
    });
    it("[BR-048][B.2] cardReadVisibility eksplisit pada card.read disimpan apa adanya", async () => {
        const cardRead = await permissionIdByKey(ctx.globalClient, "card.read");
        const res = await makeApp().request(`http://localhost/v1/projects/${ctx.projectIdA}/permission-groups`, {
            method: "POST",
            headers: { "content-type": "application/json", "x-test-user": "user-a" },
            body: JSON.stringify({
                name: "Wide Read",
                permissions: [{ permissionId: cardRead, cardReadVisibility: "ASSIGNED_TO_ME" }],
            }),
        });
        if (res.status !== 201)
            throw new Error(`status ${res.status}: ${await res.text()}`);
    ...
    it("[B.2] negatif: cardReadVisibility utk permission selain card.read ditolak VALIDATION_ERROR", async () => {
        const projectRead = await permissionIdByKey(ctx.globalClient, "project.read");
        const res = await makeApp().request(`http://localhost/v1/projects/${ctx.projectIdA}/permission-groups`, {
            method: "POST",
            headers: { "content-type": "application/json", "x-test-user": "user-a" },
            body: JSON.stringify({
                ...
                permissions: [{ permissionId: projectRead, cardReadVisibility: "ALL" }],
            }),
        });
        if (res.status !== 400)
            throw new Error(`harusnya 400, dapat ${res.status}: ${await res.text()}`);
        const json = await res.json();
    ...
    it("[C.12] negatif: permissionId tidak dikenal ditolak; nama kosong ditolak", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${ctx.projectIdA}/permission-groups`, {
            method: "POST",
            headers: { "content-type": "application/json", "x-test-user": "user-a" },
            body: JSON.stringify({ name: "X", permissions: [{ permissionId: "perm-tidak-ada" }] }),
        });
        if (res.status !== 400)
            throw new Error(`harusnya 400, dapat ${res.status}`);
        const resName = await makeApp().request(`http://localhost/v1/projects/${ctx.projectIdA}/permission-groups`, {
            method: "POST",
            headers: { "content-type": "application/json", "x-test-user": "user-a" },
            body: JSON.stringify({ name: "   " }),
        });
        if (resName.status !== 400)
            throw new Error(`nama kosong harusnya 400, dapat ${resName.status}`);
    });
    it("[INV-04][BR-039] boundary: group baru hanya muncul di Project pembuatnya", async () => {
        ...
        if (!groupsA.some((g) => g.name === "Release Ops"))
            throw new Error("group hilang dari Project A");
        const groupsB = await ctx.deps.listPermissionGroups(projectIdB, "user-b", { includeDeleted: false });
        if (groupsB.some((g) => g.name === "Release Ops"))
            throw new Error("group Project A bocor ke Project B");
    });
});
