...
// TASK-0.17.6 (02-SPEC C.2 amandemen 3.0.0) — rollout pola collect-all
// (ValidationCollector, TASK-0.17.4) ke parser request TERSISA: Label,
// Permission Group/Membership/Invitation (project-admin.ts), API Key.
...
const fields = (json: { error?: { details?: Array<{ field: string }> } }): string[] =>
  (json.error?.details ?? []).map((d) => d.field).sort();

describe("[TASK-0.17.6] VALIDATION_ERROR collect-all — Label", () => {
  const labelApp = (): Hono =>
    new Hono().route(
      "/",
      createMilestoneLabelsRouter(() => buildMilestoneLabelRoutesDeps({ identityResolver: fakeIdentityResolver, globalClient, turso: null })),
    );
...
describe("[TASK-0.17.6] VALIDATION_ERROR collect-all — Permission Group/Membership/Invitation", () => {
  const adminApp = (): Hono =>
    new Hono().route(
      "/",
      createProjectAdminRouter(() => buildProjectAdminDeps({ identityResolver: fakeIdentityResolver, globalClient, turso: null })),
    );
