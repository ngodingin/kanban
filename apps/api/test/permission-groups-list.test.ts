...
describe("GET /api/v1/projects/:project_id/permission-groups (goal 1.7.1)", () => {
    it("[C.12][FR-009][BR-039] Owner melihat baseline + custom group beserta permission-nya", async () => {
        ...
        if (!names.includes("Custom Ops"))
            throw new Error(`custom group hilang: ${JSON.stringify(names)}`);
        if (groups.length !== 4)
            throw new Error(`jumlah salah (Viewer harusnya tersembunyi): ${JSON.stringify(names)}`);
        ...
        for (const g of groups) {
            if (g.deletedAt !== null)
                throw new Error(`soft-deleted bocor di default list: ${g.name}`);
            if (typeof g.id !== "string" || !Array.isArray(g.permissions))
                throw new Error(`bentuk group salah: ${JSON.stringify(g)}`);
        }
    });
    it("[C.12] include_deleted=true menampilkan kembali grup yang soft-deleted", async () => {
        ...
        if (all.length !== 5)
            throw new Error(`jumlah salah: ${JSON.stringify(all.map((g) => g.name))}`);
        const viewer = all.find((g) => g.name === "Viewer")!;
        if (viewer.deletedAt === null)
            throw new Error("Viewer harusnya bertanda deleted_at");
    });
    it("[INV-04] boundary: list Project lain hanya berisi grup milik Project itu", async () => {
        ...
        if (groupsB.length !== 4)
            throw new Error(`Project B harusnya punya 4 baseline: ${JSON.stringify(groupsB.map((g) => g.name))}`);
        const idsA = new Set(groupsA.map((g) => g.id));
        for (const g of groupsB) {
            if (idsA.has(g.id))
                throw new Error(`group Project A bocor ke list Project B: ${g.name}`);
        }
    });
    it("[Rule-3][C.12] negatif: bukan member → 403; project tak dikenal → 404; tanpa identitas → 401", async () => {
        ...
        if (err === null || !(err instanceof Error) || !("httpStatus" in err) || (err as {
            httpStatus?: number;
        }).httpStatus !== 403) {
        ...
        if (resolved !== null)
            throw new Error("tanpa header identitas harusnya null");
    });
});
