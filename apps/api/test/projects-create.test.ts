...
beforeAll(async () => {
  ...
  const fakeProvision = async (input: { projectId: string; projectName: string; creatorUserId: string }) => {
...
describe("POST /api/v1/projects — endpoint provisioning (goal 1.3.1)", () => {
  it("[C.4][F.2] POST valid oleh user teridentifikasi menghasilkan registry READY + Owner Membership + Project DB ACTIVE + Activity project.created", async () => {
    ...
    if (typeof data.id !== "string" || data.id.length !== 26) throw new Error(`project_id bukan ULID: ${data.id}`);
    if (data.name !== "Proyek Uji Endpoint") throw new Error(`name tidak di-trim: ${data.name}`);
    if (data.status !== "ACTIVE" || data.version !== 1) throw new Error(`status/version salah: ${JSON.stringify(data)}`);

    const reg = await ctx.globalClient.execute({
      sql: "SELECT owner_user_id, provisioning_state FROM projects WHERE id = ?",
      args: [data.id],
    });
    if (reg.rows.length !== 1) throw new Error("registry tidak tercatat");
    ...
    if (mapping.rows.length !== 1) throw new Error("mapping DB tidak tercatat");
