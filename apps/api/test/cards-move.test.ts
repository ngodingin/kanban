...
describe("POST /api/v1/projects/:project_id/cards/:card_id/move — goal 2.11.1", () => {
  ...
  it("[AC-020][regresi 2.10] version mismatch → VERSION_CONFLICT tanpa perubahan/activity baru", async () => {
    ...
    try {
      ...
      const movedCount = await projectDb.execute(
        "SELECT COUNT(*) AS n FROM activities WHERE entity_id = 'cd_move' AND action = 'card.moved'",
      );
      expect(Number(movedCount.rows[0]?.n)).toBe(2); // hanya dari test pertama (bolak-balik)
    } finally {
      await projectDb.close();
    }
  });
