...
describe("POST .../cards/:card_id/labels/:label_id/remove — goal 3.8.1", () => {
    it("[C.11] Owner remove asosiasi aktif → 200, removed_at terisi", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/cards/c_1/labels/bl_1/remove`, { method: "POST", headers: { "x-test-user": "user-a" } });
        ...
        try {
            const row = await projectDb.execute("SELECT removed_at FROM card_board_labels WHERE card_id = 'c_1' AND label_id = 'bl_1'");
    ...
    it("[Authz] non-Owner member → PERMISSION_DENIED", async () => {
        const res = await makeApp().request(`http://localhost/v1/projects/${projectIdValue}/cards/c_1/labels/bl_1/remove`, { method: "POST", headers: { "x-test-user": "user-b" } });
