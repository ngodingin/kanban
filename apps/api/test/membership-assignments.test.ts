...
function buildDeps(gc: Client): ProjectAdminRoutesDeps {
    ...
    return {
        ...
        listMembers: lazy,
        assertPermissionKey: (projectId, requesterUserId, key) => import("@kanban/infrastructure").then((m) => m.assertPermissionKey(gc, projectId, requesterUserId, key)),
        listMembershipAssignments: (projectId, membershipId) => import("@kanban/infrastructure").then((m) => m.listMembershipAssignments(gc, projectId, membershipId)),
...
const getAssignments = (mid: string, user: string): Promise<Response> => app().request(`http://localhost/v1/projects/${projectIdValue}/members/${mid}/assignments`, {
...
describe("GET .../members/:membership_id/assignments — goal 4.6.1", () => {
    ...
    it("[boundary] membership_id milik Project lain → RESOURCE_NOT_FOUND 404", async () => {
        const res = await app().request(`http://localhost/v1/projects/${otherProjectIdValue}/members/m-reader/assignments`, { headers: { "x-test-user": "user-plain" } });
