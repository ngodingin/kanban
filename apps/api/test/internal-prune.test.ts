...
const call = (secret?: string): Promise<Response> => app().request("/internal/prune", {
    method: "POST",
...
describe("TASK-5.4 rework — trigger memproses journal existing (SOT 4.1.0)", () => {
    it("[recovery] job PENDING existing → diproses trigger; DATABASE_DELETED tanpa buka DB", async () => {
        ...
        expect(res.status).toBe(200);
        const json = await res.json();
        expect(json.data.jobsRecovered).toBe(1);
        expect(json.data.prunedProjects).toBe(1);
        ...
        const jobs = await globalClient.execute("SELECT state FROM project_deprovision_jobs WHERE project_id IN ('pa','pb') ORDER BY project_id");
    ...
    it("[isolasi kegagalan] satu job gagal provider TIDAK menghentikan project lain", async () => {
        ...
        expect(res.status).toBe(200);
        expect(await existsProject("pc-fail")).toBe(true);
        expect(await existsProject("pd-ok")).toBe(false);
