...
// Goal 1.9.2 — POST /api/v1/invitations/:invitation_id/accept (C.13/FR-007)
// Validasi state (INVITATION_EXPIRED / INVITATION_ALREADY_USED / revoked /
// sudah member), lalu atomik: membership + group assignments + accepted_at
// dalam satu transaksi Global DB.
...
describe("POST /invitations/:invitation_id/accept (goal 1.9.2)", () => {
  it("[FR-007][C.13] Positif: accept atomik — membership + group assignments + accepted_at", async () => {
    ...
    if (accepted.rows[0]!.accepted_at === null) throw new Error("accepted_at tidak ter-set");
  });
  ...
  it("[C.13] negatif: invitation expired → INVITATION_EXPIRED dan tidak ada efek samping", async () => {
    ...
    if (Number(memberships.rows[0]!.n) !== 0) throw new Error("accept expired membuat membership — efek samping!");
  });
  ...
  it("[INV-04][UNIQUE] negatif: pemanggil sudah member Project ini → INVALID_STATE tanpa duplikat", async () => {
    ...
    if (Number(memberships.rows[0]!.n) !== 1) throw new Error("membership Owner terduplikasi");
  });
});
