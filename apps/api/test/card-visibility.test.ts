...
const listCards = (user: string): Promise<Response> => app().request(`http://localhost/v1/projects/${projectIdValue}/lists/ls_v/cards`, {
    headers: { "x-test-user": user },
});
const getCard = (cid: string, user: string): Promise<Response> => app().request(`http://localhost/v1/projects/${projectIdValue}/cards/${cid}`, {
    headers: { "x-test-user": user },
});
...
describe("Card visibility scope — GET endpoints (goal 4.5.1)", () => {
    it("[BR-047][D.3 default] tanpa grant visibility eksplisit → CREATED_BY_ME: hanya cd_own di list; cd lain → 404 (bukan 403)", async () => {
        ...
        const ids = (await listRes.json()).data.cards.map((c: {
            id: string;
        }) => c.id);
    ...
    it("[BR-047 union OR] ASSIGNED_TO_ME → cd_own ATAU cd_assigned terlihat; Owner → ALL tetap lihat semua", async () => {
        ...
        const ids = (await (await listCards("user-vis")).json()).data.cards.map((c: {
            id: string;
        }) => c.id);
        expect(ids.sort()).toEqual(["cd_assigned", "cd_own"].sort());
        const ownerIds = (await (await listCards("user-owner")).json()).data.cards.map((c: {
            id: string;
        }) => c.id);
