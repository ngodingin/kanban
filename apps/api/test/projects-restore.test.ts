...
import { applyGlobalMigrations, applyProjectMigrations, DrizzleProjectRepository, newProjectId, registerProjectWithOwnerMembership, RequestPipeline, SqliteProjectDatabaseResolver, createEntityPermissionResolver, } from "@kanban/infrastructure";
...
describe("POST /api/v1/projects/:project_id/restore — lifecycle ARCHIVED→ACTIVE (goal 1.4.2)", () => {
    it("[INV-LIFE-002][C.4] restore pada project ACTIVE ditolak INVALID_STATE 409 (hanya valid dari ARCHIVED)", async () => {
        ...
        if (version !== 1)
            throw new Error(`fixture harus masih ACTIVE v1, dapat v${version}`);
    ...
    it("[A.3][C.4][B.5] owner restore dari ARCHIVED → 200, archived_at null kembali, Activity project.restored dengan previous_state ARCHIVED", async () => {
        ...
        if (p.archivedAt !== null || p.version !== fixtureVersion + 1)
    ...
    it("[C.4][interim-authz][C.2] non-owner → PERMISSION_DENIED 403; tanpa identitas → TOKEN_EXPIRED 401; tanpa expected_version → VALIDATION_ERROR", async () => {
        ...
        if (noVersion.status !== 400)
            throw new Error(`status ${noVersion.status}, harusnya 400`);
    });
});
