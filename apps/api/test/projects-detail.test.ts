...
describe("GET /api/v1/projects/:project_id — detail Project (goal 1.3.3)", () => {
  ...
  it("[INV-04][C.4] non-member menerima PROJECT_ACCESS_DENIED 403 tanpa isi project terungkap", async () => {
    ...
    const json = await res.json();
    if (json.error?.code !== "PROJECT_ACCESS_DENIED") throw new Error(`code ${json.error?.code}`);
    if (typeof json.data?.project !== "undefined") throw new Error("data project tidak boleh terungkap ke non-member");
  });
  ...
  it("[C.2][C.4] project yang tidak ada di registry → RESOURCE_NOT_FOUND 404", async () => {
    ...
    const json = await res.json();
    if (json.error?.code !== "RESOURCE_NOT_FOUND") throw new Error(`code ${json.error?.code}`);
  });
