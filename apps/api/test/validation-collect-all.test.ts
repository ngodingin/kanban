...
import { buildBoardRoutesDeps, buildCardRoutesDeps, buildListRoutesDeps, buildMilestoneRoutesDeps, buildProjectRoutesDeps, } from "../src/project-deps.ts";
...
const projectApp = (): Hono => new Hono().route("/", createProjectsRouter(() => buildProjectRoutesDeps({ identityResolver: fakeIdentityResolver, globalClient, turso: null })));
...
const boardApp = (): Hono => new Hono().route("/", createBoardsRouter(() => buildBoardRoutesDeps({ identityResolver: fakeIdentityResolver, globalClient, turso: null })));
const listApp = (): Hono => new Hono().route("/", createListsRouter(() => buildListRoutesDeps({ identityResolver: fakeIdentityResolver, globalClient, turso: null })));
const cardApp = (): Hono => new Hono().route("/", createCardsRouter(() => buildCardRoutesDeps({ identityResolver: fakeIdentityResolver, globalClient, turso: null })));
...
describe("[TASK-0.17.4] VALIDATION_ERROR collect-all (02-SPEC C.2 amandemen 3.0.0)", () => {
    it("[PATCH Project] name kosong + expectedVersion bukan integer -> KEDUA field muncul sekaligus di details", async () => {
        ...
        const fields = (json.error?.details ?? []).map((d: {
            field: string;
        }) => d.field).sort();
