...
**Test:** tiap goal wajib memverifikasi respons API nyata dan observasi web nyata pada data test yang sama; 7.16.1 MUST mengambil email melalui Resend Receiving API berdasarkan penerima unik + `receivedAfter`, memverifikasi sender/subject/link sebelum membuka callback browser, dan menolak pesan lama/salah penerima. Test negatif yang dirujuk wajib dijalankan, dan `test:e2e:staging` MUST fail-fast bila target bukan staging, konfigurasi Resend QA tidak lengkap, atau session tidak terbentuk.
