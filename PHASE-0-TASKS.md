...
| 0.19.2 | ⬜️ | [Review-CL-21](#review-cl-21) | 0 | P1 | `POST /invitations/:id/accept`, `GET /invitations`, `POST /invitations/:id/revoke` (`apps/api/src/routes/project-admin.ts:480-513`) me-return objek/array mentah langsung sebagai `data`, TIDAK dibungkus key — inkonsisten dengan `POST /invitations` (create, baris 411: `{ invitation }`) di FILE YANG SAMA, dan dengan konvensi seluruh codebase lain (`GET /milestones` → `{ milestones }`, `GET /labels` → `{ labels }`, `GET /members` di file ini sendiri → `{ members }`). Bungkus ketiganya: `{ invitation }` (accept), `{ invitations }` (list), `{ invitation }` (revoke) — konsisten dengan pola create. | [02-SPEC C.2.1](docs/02-SPEC.md), C.13 | — |

**Test:** `GET /projects/:id/members/:userId/assignments` → response `{ groupAssignments: [...], permissionAssignments: [...] }` (camelCase, bukan lagi `group_assignments`/`permission_assignments`). `POST /invitations/:id/accept`, `GET /invitations`, `POST /invitations/:id/revoke` → masing-masing response dibungkus `{ invitation }`/`{ invitations }`/`{ invitation }`, client lama yang mengakses field mentah di top-level `data` diperbarui mengikuti wrapper baru (breaking, sama kelas dengan 0.17/0.18 — belum ada consumer produksi).
**DoD:** `pnpm exec vitest run` 100% hijau; test yang mengasumsikan bentuk lama diperbarui, bukan dihapus.
