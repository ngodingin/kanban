...
| ID | Status | CL | % | Prior | Goal Description | Reference | Dependency |
|---|:--:|:--:|:--:|:--:|---|---|---|
| 0.17.1 | ✅ | [CL-70](#cl-70)<br>[Review-CL-20](#review-cl-20)<br>[QA-CL-70](#qa-cl-70) | 100 | P1 | Migrasi parsing request body Milestone/Board/List/Card (`routes/milestones.ts`, `boards.ts`, `lists.ts`, `cards.ts`): field `expected_version`→`expectedVersion`, `start_date`/`due_date`→`startDate`/`dueDate`, `destination_list_id`→`destinationListId` (Move Card), PATCH `allowedFields` array disesuaikan. Field response TIDAK berubah (sudah camelCase). | [02-SPEC C.2.1](docs/02-SPEC.md), C.5, C.8 (amandemen 3.0.0) | — |
| 0.17.2 | ✅ | [CL-71](#cl-71)<br>[Review-CL-20](#review-cl-20)<br>[QA-CL-70](#qa-cl-70) | 100 | P1 | Migrasi Label/Card-Label/Comment (`routes/labels.ts`, `card-labels.ts`, `comments.ts`): `label_id`→`labelId` (assign ke Card). | [02-SPEC C.2.1](docs/02-SPEC.md), C.11 | — |
| 0.17.3 | ✅ | [CL-73](#cl-73)<br>[Review-CL-20](#review-cl-20)<br>[Review-CL-23](#review-cl-23)<br>[QA-CL-70](#qa-cl-70) | 100 | P1 | Migrasi Permission/Membership/Invitation (`project-admin.ts` + route terkait): `group_id`/`scope_type`/`scope_id`→`groupId`/`scopeType`/`scopeId`, `permission_id`/`card_read_visibility`→`permissionId`/`cardReadVisibility` (scoped assignment), `expires_at`→`expiresAt` pada create Invitation; nested `assignments[]` mengikuti nama camelCase yang sama. | [02-SPEC C.2.1](docs/02-SPEC.md), C.12, C.13 | — |
| 0.17.4 | ✅ | [CL-76](#cl-76)<br>[Review-CL-20](#review-cl-20)<br>[Review-CL-23](#review-cl-23)<br>[QA-CL-70](#qa-cl-70) | 100 | P1 | Implementasikan pola reusable collect-all untuk `VALIDATION_ERROR.details`, lalu terapkan ke seluruh parser request Project/Milestone/Board/List/Card (`projects.ts`, `milestones.ts`, `boards.ts`, `lists.ts`, `cards.ts`). Payload dengan beberapa field invalid MUST menghasilkan seluruh `{field, reason}` sekaligus; bukan fail-fast. | [02-SPEC C.2](docs/02-SPEC.md) (amandemen 3.0.0) | 0.17.1, 0.17.5 |
| 0.17.5 | ✅ | [CL-70](#cl-70)<br>[Review-CL-21](#review-cl-21)<br>[QA-CL-70](#qa-cl-70) | 100 | P1 | **Gap cakupan ditemukan saat review total 2026-08-24:** `routes/projects.ts` TIDAK pernah masuk daftar file 0.17.1–0.17.3 di atas, padahal `readExpectedVersionField` (`apps/api/src/routes/projects.ts:142-148`) membaca `body.expected_version` (snake_case) untuk SELURUH mutasi lifecycle Project (`PATCH`, `/archive`, `/restore`, `/delete`) — persis pola yang seharusnya sudah dimigrasi 0.17.1 untuk Milestone/Board/List/Card, tapi Project sendiri terlewat. Migrasi ke `expectedVersion`, field lama tidak lagi dibaca. | [02-SPEC C.2.1](docs/02-SPEC.md), C.4 (amandemen 3.0.0) | — |
| 0.17.6 | ✅ | [CL-87](#cl-87)<br>[CL-86](#cl-86)<br>[CL-77](#cl-77)<br>[Review-CL-23](#review-cl-23)<br>[QA-CL-70](#qa-cl-70)<br>[QA-CL-72](#qa-cl-72) | 100 | P1 | Terapkan pola collect-all `VALIDATION_ERROR.details` dari 0.17.4 ke seluruh parser request tersisa: Label/Card-Label/Comment, Permission/Membership/Invitation, API Key, dan PAT (`labels.ts`, `card-labels.ts`, `comments.ts`, `project-admin.ts`, `api-keys.ts`, `personal-access-tokens.ts`). Tidak ada endpoint JSON-body yang boleh tetap fail-fast karena C.2 berlaku global. | [02-SPEC C.2](docs/02-SPEC.md), C.10–C.14 (amandemen 3.0.0) | 0.17.2, 0.17.3, 0.17.4 |
...
| ID | Status | CL | % | Prior | Goal Description | Reference | Dependency |
|---|:--:|:--:|:--:|:--:|---|---|---|
| 0.21.1 | ✅ | [CL-83](#cl-83)<br>[CL-82](#cl-82)<br>[CL-72](#cl-72)<br>[Review-CL-24](#review-cl-24)<br>[QA-CL-68](#qa-cl-68)<br>[QA-CL-69](#qa-cl-69)<br>[QA-CL-70](#qa-cl-70) | 100 | P0 | Migrasikan `idempotency_keys` dan `DbIdempotencyStore` dari cache `get/put` menjadi state machine atomic claim: simpan `request_fingerprint`, unguessable `claim_token`, `state` (`IN_PROGRESS`/`COMPLETED`), `response_status`, `result`, `lease_expires_at`, `expires_at`, timestamps; UNIQUE `(key,scope)` tetap. Sediakan operasi atomik claim/complete/release/reclaim-expired; complete/release MUST compare claim token dan reclaim MUST merotasinya agar worker lama tidak dapat overwrite owner baru. Completed retention minimum 24 jam. | [02-SPEC C.3](docs/02-SPEC.md), [03-ENG B.2](docs/03-ENGINEERING.md), C.5 (amandemen 4.0.0) | — |
| 0.21.2 | ✅ | [CL-85](#cl-85)<br>[CL-83](#cl-83)<br>[CL-82](#cl-82)<br>[CL-72](#cl-72)<br>[Review-CL-24](#review-cl-24)<br>[QA-CL-68](#qa-cl-68)<br>[QA-CL-69](#qa-cl-69)<br>[QA-CL-70](#qa-cl-70)<br>[QA-CL-71](#qa-cl-71) | 100 | P0 | Perbarui kontrak/wrapper route generik: hitung fingerprint deterministik canonical request, atomic claim sebelum handler, exact replay untuk completed fingerprint sama, `409 IDEMPOTENCY_CONFLICT` untuk payload berbeda, `409 IDEMPOTENCY_IN_PROGRESS` untuk fingerprint sama yang masih leased, dan release claim saat handler gagal/non-2xx. Tambahkan kedua kode ke katalog dan HTTP mapping. Scope wajib mengikat identity+method+normalized path. | [02-SPEC C.2](docs/02-SPEC.md), C.3 (amandemen 4.0.0) | 0.21.1 |
| 0.21.3 | ✅ | [CL-85](#cl-85)<br>[CL-83](#cl-83)<br>[CL-82](#cl-82)<br>[CL-72](#cl-72)<br>[Review-CL-24](#review-cl-24)<br>[QA-CL-68](#qa-cl-68)<br>[QA-CL-69](#qa-cl-69)<br>[QA-CL-70](#qa-cl-70)<br>[QA-CL-71](#qa-cl-71) | 100 | P0 | Tambahkan regression/property-style integration test AC-033/AC-034: replay sequential identik; same key+scope payload berbeda; dua request benar-benar paralel dengan barrier sehingga keduanya overlap; failure melepaskan claim; lease expired dapat direclaim dan merotasi token; stale owner tidak dapat complete/release claim baru; completed expiry memproses sebagai request baru. Assert row-level tepat satu side-effect/Activity dan exact stored status/body. | [04-DELIVERY AC-033](docs/04-DELIVERY.md), AC-034, [02-SPEC C.3](docs/02-SPEC.md) | 0.21.2 |
...
**Bukti:** Menggunakan `VERCEL_AUTOMATION_BYPASS_SECRET` dari `.env` hanya sebagai header `x-vercel-protection-bypass` (nilai tidak dicetak/disimpan): `GET https://kanban-ngodingin.vercel.app/` → **200**; `GET /api/v1/health` → **200** dengan `{ "data": { "status": "ok", "env": "staging" } }`.

**Review:** Temuan akses staging Review-CL-27 tertutup. Vercel SSO tetap dapat dipertahankan; secret bypass menyediakan jalur smoke otomasi terautentikasi. `0.12.4` tetap `✅ 100%`; tidak ada perubahan Status/%, implementasi, atau SOT.
...
**Bukti:** Discovery read-only setelah lane dikonfirmasi menemukan `docs/01-PRODUCT.md` berisi `SPEC_VERSION = 4.1.1` dan changelog 4.1.1, tetapi display header dokumen masih `Version: 4.1.0` dan header `PHASE-0-TASKS.md` masih menulis `SOT version: 4.1.0` seolah versi terkini. Scope manusia dikonfirmasi untuk review drift metadata. Patch ini menyelaraskan display header `01-PRODUCT` ke 4.1.1 dan memperjelas header Phase 0 sebagai metadata historis: file digenerate pada SOT 4.1.0, sementara SOT terkini 4.1.1. Kemunculan lain `SOT 4.1.0` di gate Phase 6, PHASE-5/PHASE-6, dan Closure Log lama dipertahankan karena merupakan bukti historis/referensi gate pada saat keputusan/verifikasi dibuat, bukan current-version metadata.

**Catatan:** Tidak ada perubahan business invariant, authorization, lifecycle, API behavior, atau data-model semantics; `SPEC_VERSION` tidak dinaikkan. Tidak ada Status/% goal yang berubah.

<a id="qa-cl-72"></a>
### QA-CL-72 — 2026-08-24 · verifikasi independen remediasi 0.17.6 pasca QA-CL-70 — ✅ 100%

**Role:** AI-QA · **Model:** claude-sonnet-5 (Claude Code)

**Konteks:** Lint repo-wide (blocker cross-cutting dicatat QA-CL-71) sudah dibersihkan sesi lain (`526b040`), dan CL-86/CL-87 me-remediasi dua temuan blocking spesifik QA-CL-70 untuk goal ini: (1) `api-keys.ts`/`personal-access-tokens.ts` masih baca `expires_at`/wrapper snake_case, (2) unknown-field loop tetap fail-fast, (3) tidak ada skenario test PAT multi-field.

**Verifikasi independen ketiga temuan:**
1. `expires_at` — `grep` di `api-keys.ts`/`personal-access-tokens.ts` → nol hasil; kode sekarang genuinely baca `body.expiresAt` (camelCase). Wrapper response dikonfirmasi camelCase: `{ apiKey }`/`{ apiKeys }`/`{ personalAccessToken }`/`{ personalAccessTokens }` — bukan lagi `api_key`/`personal_access_token`.
2. Unknown-field loop dibaca langsung (`api-keys.ts` baris 76-82) — sekarang genuinely `collector.collect("unknownField:...", ...)` di dalam loop, dikumpulkan bersama field lain sebelum `collector.throwIfAny()` — collect-all sungguhan, bukan fail-fast lagi.
3. Test PAT multi-field ditemukan (`personal-access-tokens-route.test.ts`, describe baru "PAT validation collect-all — goal 0.17.6"): kirim `name` kosong + `expiresAt` salah tipe + field tak dikenal SEKALIGUS → assert `details` array memuat ketiganya. Dijalankan ulang independen bersama `validation-collect-all-0.17.6.test.ts` (API Key/Label/Permission/Invitation) → **11/11 PASS**.

**DoD grep ulang:** `grep -rn` pola snake_case request-body di `apps/api/src` → hanya menyisakan `c.req.param("label_id")`/`c.req.param("group_id")` — dikonfirmasi ini nama **URL path parameter** (`:label_id`, `:group_id`), bukan field JSON body; SOT sendiri mengunci path segment snake_case (`02-SPEC` contoh route `:project_id`, `:milestone_id`, dst) — bukan pelanggaran, DoD (eksplisit "di kode PARSING request body") terpenuhi.

**Full re-run independen:** `pnpm exec vitest run` → **98 file/597 test PASS**; `pnpm -r typecheck` → 6/6 Done; `pnpm lint` → **bersih** (mengonfirmasi cleanup lint cross-cutting sesi lain genuinely landing, bukan cuma klaim).

**Kesimpulan:** 0.17.6 ditutup `✅ 100%`. TASK-0.17 (6/6 goal) sekarang genuinely tuntas seluruhnya.

<a id="qa-cl-71"></a>
### QA-CL-71 — 2026-08-24 · verifikasi independen remediasi 0.20.1, 0.21.2, 0.21.3 pasca QA-CL-70 — ✅ 100% ketiganya

**Role:** AI-QA · **Model:** claude-sonnet-5 (Claude Code)

**Catatan freshness penting:** discovery awal sesi ini mengacu ke handoff manusia yang berhenti di commit `e50b176`. Sebelum menulis apa pun, `git log`/`git status` menunjukkan HEAD lokal sudah 33 commit lebih maju (sesi QA "Codex" lain sudah menjalankan QA-CL-68/69/70 — reject TASK-0.21 dua kali dengan bukti sangat rinci, Dev meremediasi via CL-82/83/85/86/87/88). Analisis awal saya terhadap commit `00b8daa` (sebelum remediasi) dibuang, diulang terhadap state terkini sesuai §6.1. Hanya 3 goal yang genuinely masih `🔎/80` (actionable) setelah QA-CL-70: 0.20.1, 0.21.2, 0.21.3 — 0.17.6 tetap `🔎/70` (Dev belum selesai, bukan actionable QA).

**0.20.1 (CL-88, remediasi atas penolakan QA-CL-70 "smoke consumer tidak dibuktikan jalan tanpa secret"):** `smoke-consumers-reloc.test.ts` dibaca penuh — genuinely men-spawn 4 script smoke sungguhan (`spawnSync`) dan assert exit 0 + output match `PASS|SKIP`, TANPA memerlukan credential Turso (graceful skip dikonfirmasi). Dijalankan ulang independen: **5/5 PASS**. Dikonfirmasi terpisah: `grep` fungsi hasil relokasi (`registerProject`/`recordProjectDatabaseMapping`/`deleteProjectRegistry`/`deleteProjectDatabaseMapping`/`createProjectClient`) di `packages/infrastructure/src/index.ts` → nol hasil, genuinely tidak lagi bocor ke permukaan paket produksi.

**0.21.2 + 0.21.3 (CL-85, remediasi atas 2 temuan blocking QA-CL-70 — "wiring belum menyeluruh" dan "AC-034 belum barrier sungguhan"):**
1. **Cakupan wiring** — audit ulang independen (bukan percaya klaim commit) atas SELURUH `router.post`/`router.patch` di `apps/api/src/routes/*.ts`: setiap satu (44 registrasi) memanggil `withIdempotentHandling` dalam radius 6 baris, nol yang tersisa memakai `withErrorHandling` polos. Sebaliknya, nol `router.get` yang salah dibungkus idempotency (GET semantically tidak idempotency-key-relevant — PAT list yang sebelumnya salah dibungkus QA-CL-70 sekarang dikonfirmasi sudah di-unwrap). Tidak ada `router.delete` (physical delete non-MVP, konsisten §9).
2. **Architecture drift ditutup** — `grep "IdempotencyStore"` di `packages/contracts/src/*.ts` → nol hasil, interface `get/put` lama genuinely terhapus total (bukan cuma tidak dipakai).
3. **Barrier concurrency sungguhan** — `idempotency-barrier.test.ts` dibaca penuh: test PERTAMA menahan handler via `Promise` gate eksplisit, POLL `started` flag sampai true (claim TERBUKTI terjadi) SEBELUM mengirim request kedua — bukan `Promise.all` yang cuma "biasanya" overlap. Assert row-level TEPAT SATU `milestones` + TEPAT SATU `activities` setelah keduanya selesai (memenuhi permintaan spesifik QA-CL-70 "assert Activity nyata"). Test kedua membuktikan completed-TTL-expired diproses sebagai request baru (2 side-effect independen). Dijalankan ulang independen: **2/2 PASS**.
4. Re-run penuh independen: `pnpm exec vitest run` → **98 file/597 test PASS**; `pnpm -r typecheck` → 6/6 Done. `apps/api/test/idempotency-barrier.test.ts` sendiri lint-bersih (dikonfirmasi tidak muncul di daftar error lint).

**Temuan cross-cutting TIDAK memblokir ketiga goal ini (dicatat, bukan diabaikan):** `pnpm lint` repo-wide SAAT INI gagal (5 error) — TAPI seluruhnya di file `prune-projects.ts`/`prune-projects.test.ts`/`revoke-recovery.test.ts`, bagian dari commit `47100c7`/`842ee89` ("goal 5.3.1 rework"/"goal 2.12.1 rework") milik Phase 5/Phase 2, sama sekali tidak tersentuh oleh diff 0.20.1/0.21.2/0.21.3 manapun. Dikonfirmasi via `git log -1 -- <file>` untuk masing-masing file. Ini blocker nyata untuk kesehatan repo/Exit Criteria Phase 0 keseluruhan, tapi bukan tanggung jawab atau cakupan goal-goal yang saya tutup di sini — perlu QA/Dev terpisah di lane Phase 2/5.

**Kesimpulan:** 0.20.1, 0.21.2, 0.21.3 ditutup `✅ 100%`. TASK-0.21 (gating) kini genuinely 3/3 goal `✅`. Sisa yang menghalangi closure TASK-0.17 penuh: hanya 0.17.6 (`🔎/70`, masih di tangan Dev).

<a id="qa-cl-70"></a>
### QA-CL-70 — 2026-08-24 · verifikasi seluruh goal Phase 0 yang menunggu QA terhadap SOT 4.1.0
**Role:** AI-QA · **Model:** Codex

**Bukti runtime:** dijalankan melalui host `distrobox enter envdev` (Node `v24.19.0`, pnpm `11.22.0`). Full Vitest **97 file/590 test PASS**; typecheck dan build 6/6 PASS. Smoke migration, Global schema, dan Global constraint PASS. Suite terarah Phase 0: idempotency **4 file/28 test PASS**; camelCase/validation/Invitation/Activity/error mapping **16 file/143 test PASS**; prune descendant/no-orphan **2 file/10 test PASS**. `pnpm lint` FAIL pada `apps/api/test/idempotency-barrier.test.ts:45` (`@typescript-eslint/no-unused-expressions`). Smoke yang memakai Turso nyata tidak dapat direproduksi karena `TURSO_DB_URL/TURSO_DB_TOKEN` tidak tersedia pada environment; script berhenti sebelum koneksi.

**Diterima — ✅ 100%:** 0.15.1/0.15.2; 0.17.1–0.17.5; 0.18.1/0.18.2; 0.19.1/0.19.2; 0.20.2; dan 0.21.1. Error mapping, request core/admin camelCase, Activity payload, wrapper Invitation, prune subtree, state-machine store, claim token, reclaim, serta migration idempotent terbukti lewat test positif/negatif dan pemeriksaan disk.

**0.17.6 — ⚠️ 70%:** `api-keys.ts` dan `personal-access-tokens.ts` masih membaca `expires_at` serta mengembalikan wrapper `api_key`/`api_keys`/`personal_access_token`/`personal_access_tokens`, bertentangan dengan C.2.1. Test 0.17.6 tidak memiliki skenario PAT multi-field; unknown-field loop juga tetap fail-fast setelah collector. Dev harus memakai JSON `camelCase` dan menambah bukti collect-all credential lengkap.

**0.20.1 — ⚠️ 70%:** relokasi statis dan typecheck terbukti, tetapi Test goal mewajibkan smoke consumer tetap berjalan. Consumer Turso berhenti karena credential runtime tidak tersedia dan CL-80 tidak memberi bukti run smoke tersebut. Dev perlu menyediakan bukti CI/reproducible atau test lokal tanpa secret eksternal.

**0.21.2 — ⚠️ 60%:** klaim CL-83 “seluruh mutation” tidak cocok dengan disk. PATCH Project dan lifecycle Label masih memakai `withErrorHandling`; accept Invitation dan revoke PAT tidak dibungkus; GET PAT justru salah dibungkus idempotency. C.3 belum menyeluruh.

**0.21.3 — ⚠️ 70%:** barrier deterministik ada dan test lulus, tetapi file test membuat lint suite gagal. Test barrier hanya mengasersi satu row `side_effects`, belum satu entity dan Activity seperti Test goal. Perbaiki lint dan tambahkan assertion Activity nyata/representatif.

**Verdict fase:** Phase 0 belum closed; TASK-0.17, 0.20, dan 0.21 berstatus derived ⚠️. Gate Phase 5.5/Phase 6 tetap tertutup.

<a id="qa-cl-69"></a>
### QA-CL-69 — 2026-08-24 · reproduksi ulang TASK-0.21 melalui host distrobox `envdev` (status tetap ⚠️)
**Role:** AI-QA · **Model:** Codex

**Koreksi lingkungan:** manusia menginformasikan Docker/runtime host tersedia melalui `distrobox enter envdev` atau `flatpak-spawn --host bash`. Seluruh pemeriksaan kritis QA-CL-68 direproduksi ulang melalui `flatpak-spawn --host distrobox enter envdev` memakai Node `v24.19.0` dan pnpm `11.22.0`; tidak ada implementasi yang diubah.

**Hasil host:** targeted TASK-0.21 tetap **3 file/26 test PASS**; full Vitest tetap **95 file/577 test PASS**; `pnpm -r typecheck`, `pnpm -r build`, dan `pnpm lint` exit 0. `pnpm --filter @kanban/infrastructure test:smoke-migration` tetap exit 1 dengan pesan identik `jumlah tabel setelah apply ulang = 17, harus 16` pada `smoke-migration.ts:40`.

**Verdict:** kegagalan smoke migration dan temuan statis contract/wiring/barrier QA-CL-68 bukan artefak runtime sementara. Status/persentase 0.21.1–0.21.3 tetap ⚠️ 70%/60%/60%; remediation QA-CL-68 tetap berlaku.

<a id="qa-cl-68"></a>
### QA-CL-68 — 2026-08-24 · TASK-0.21 gagal verifikasi independen (🔎 80% → ⚠️ 70%/60%/60%)
**Role:** AI-QA · **Model:** Codex

**Pergantian lane:** sesi ini berpindah dari AI-Planning & Review ke AI-QA setelah konfirmasi eksplisit manusia: “ya lakukan - pindah ke sesi AI-QA dan verifikasi TASK-0.21 terlebih dahulu”. QA tidak mengubah implementasi.

**Bukti yang lulus:** runtime temporer Node `v24.18.0` + Corepack pnpm `11.22.0`; targeted `pnpm exec vitest run packages/infrastructure/test/idempotency.test.ts apps/api/test/idempotency-wiring.test.ts packages/contracts/test/error-codes.test.ts` → **3 file/26 test PASS**; full `pnpm exec vitest run` → **95 file/577 test PASS**; `pnpm -r typecheck`, `pnpm -r build`, dan `pnpm lint` exit 0. Store test membuktikan claim/replay/conflict/release/reclaim/token stale secara positif/negatif, sedangkan API test membuktikan conflict/replay dan row-level satu Milestone pada run tersebut.

**Temuan blocking 0.21.1:** smoke migration resmi gagal: `pnpm --filter @kanban/infrastructure test:smoke-migration` exit 1 pada `smoke-migration.ts:40`, `jumlah tabel setelah apply ulang = 17, harus 16`. Journal migration memang tetap tidak bertambah pada apply kedua, tetapi regression harness yang menjadi bukti migration idempotent belum diselaraskan dengan tabel `idempotency_keys`; DoD migration/smoke belum hijau. `test:smoke-global-schema` dan `test:smoke-global-constraints` lulus, sehingga bukti implementasi dipertahankan 70%, bukan di-reset.

**Temuan blocking 0.21.2:** kontrak publik `packages/contracts/src/http-mapping.ts` masih mengekspor `IdempotencyStore.get/put`, walau SOT 4.0.0 dan implementasi baru melarang pola cache lama. Impact scan seluruh mutation route juga menemukan `withIdempotentHandling` hanya terpasang pada sebagian create/lifecycle/move Project–Card; PATCH core dan seluruh mutation Label, Project Admin/Membership/Invitation, Comment, Card-Label, API Key, dan PAT tetap mengabaikan `Idempotency-Key`. C.3 menetapkan kontrak server ketika header hadir untuk mutation berisiko, sehingga wrapper/wiring belum menyeluruh dan interface lama menjadi architecture drift aktif.

**Temuan blocking 0.21.3:** AC-034 diberi label “concurrency sungguhan”, tetapi unit dan HTTP test hanya memanggil `Promise.all`; tidak ada barrier/controllable handler (`rg 'barrier|controllable'` kosong). Ini bertentangan langsung dengan Test TASK-0.21 yang mewajibkan overlap dibuktikan dengan barrier, sehingga hasil hijau belum menjadi bukti concurrency yang sah. Test wajib menahan handler pertama sesudah claim, memulai request kedua saat claim pasti `IN_PROGRESS`, baru melepas handler pertama; assert handler/side-effect/Activity tepat satu dan respons kedua `IDEMPOTENCY_IN_PROGRESS`.

**Verdict/remediation:** 0.21.1 → ⚠️ 70%; 0.21.2 → ⚠️ 60%; 0.21.3 → ⚠️ 60%. Dev MUST melakukan ⚠️→🔄 dengan CL baru, memperbaiki smoke count/coverage migration, mengganti kontrak `get/put`, menerapkan wrapper pada seluruh mutation yang tunduk C.3, dan menambah test barrier deterministik. TASK-0.21 tetap gating; Phase 6 tetap blocked.
...
<a id="cl-83"></a>
### CL-83 — 2026-08-24 · TASK-0.21 selesai sisi Dev (⚠️ → 🔄 → 🔎 · → 80% ×3) — remediasi tiga temuan QA-CL-68
**Role:** AI-Dev · **Model:** ox-alpha-free (opencode)
**Bukti:** `pnpm exec vitest run` → **97 file / 590 test lulus**; `pnpm -r typecheck` Done; `pnpm lint` bersih; `pnpm --filter @kanban/infrastructure test:smoke-migration` → **PASS**. Per temuan: **(0.21.1)** harness `smoke-migration.ts` diselaraskan — daftar tabel ekspektasi eksplisit (18, termasuk `idempotency_keys` & `project_deprovision_jobs`) dengan diff hilang/ekstra yang informatif; **(0.21.2)** interface legacy `IdempotencyStore{get,put}` DIHAPUS dari contracts (+re-export index), wiring `withIdempotentHandling` ditambahkan ke SELURUH mutation yang QA sebutkan: PATCH core ×4, Label ms/bd create+patch+lifecycle ×6, Comment ×2, Card-Label ×2, Project Admin ×10 (groups CRUD, group/permission assignment create+revoke, invitations create/revoke, member revoke), API Key create/revoke, PAT create/revoke — store di-thread via semua builder + inline deps index.ts; **(0.21.3)** test barrier baru `idempotency-barrier.test.ts`: handler pertama DITAHAN sesudah claim, request kedua saat IN_PROGRESS → 409 IDEMPOTENCY_IN_PROGRESS, side-effect row-level tepat satu; plus completed-expired diproses ulang sebagai request baru.
**Catatan:** State machine store tidak diubah (sudah lulus verifikasi positif QA). Wiring bersifat opsional per-deps (`idempotencyStore?`) sehingga seluruh test double lama tetap hijau tanpa modifikasi.

<a id="cl-85"></a>
### CL-85 — 2026-08-24 · goal 0.21.2 & 0.21.3 remediasi selesai sisi Dev (⚠️ → 🔄 → 🔎 · 60/70 → 80%)
**Role:** AI-Dev · **Model:** ox-alpha-free (opencode)
**Bukti:** `pnpm exec vitest run` → **97 file / 590 test lulus**; `pnpm lint` **bersih** (baris 45 barrier diperbaiki); `pnpm -r typecheck` Done. **0.21.2 persis temuan QA-CL-70:** PATCH Project dibungkus; lifecycle Label ms/bd dibungkus; accept Invitation dibungkus; revoke PAT dibungkus; GET PAT list DI-UNWRAP; store di-thread `getDeps()` konsisten di seluruh site baru. **0.21.3:** side-effect barrier kini entity domain sungguhan (Milestone + Activity `milestone.created`); assert row-level milestones=1 & activities=1 saat overlap; completed-expired diproses ulang → milestone/activity bertambah 2 lalu 3.
**Catatan:** Wiring opsional (`idempotencyStore?`) — test double lama tetap hijau tanpa modifikasi.

<a id="cl-86"></a>
### CL-86 — 2026-08-24 · goal 0.17.6 dikerjakan ulang (⚠️ → 🔄 · 70% dipertahankan) — credential camelCase + collect-all
**Role:** AI-Dev · **Model:** ox-alpha-free (opencode)
**Bukti:** Freshness check dari disk: row `⚠️/70`; temuan QA-CL-70 spesifik: `expires_at` snake_case di api-keys/PAT, wrapper `api_key(s)`/`personal_access_token(s)`, skenario PAT multi-field absen.
**Catatan:** Wrapper → camelCase (`apiKey`/`apiKeys`/`personalAccessToken`/`personalAccessTokens`); body field `expiresAt`; unknown-field masuk collector (bukan fail-fast); test route diperbarui + PAT multi-field.

<a id="cl-87"></a>
### CL-87 — 2026-08-24 · goal 0.17.6 selesai sisi Dev (⚠️ → 🔄 → 🔎 · 70 → 80%) — credential camelCase + collect-all penuh
**Role:** AI-Dev · **Model:** ox-alpha-free (opencode)
**Bukti:** `pnpm exec vitest run` → **97 file / 591 test lulus**; typecheck+lint bersih. Per temuan QA-CL-70: body credential kini membaca `expiresAt` (camelCase); response wrapper menjadi `apiKey`/`apiKeys`/`personalAccessToken`/`personalAccessTokens` (C.2.1); unknown-field TIDAK lagi fail-fast — masuk `VALIDATION_ERROR.details` via collector (`unknownField:<nama>`); test route diperbarui + skenario PAT multi-field baru + test collect-all credentials diperluas (name+expiresAt+unknown sekaligus).
**Catatan:** Permission key `api_key.*`/`pat` self-managed tidak berubah — ini murni kontrak JSON C.2.1.

<a id="cl-88"></a>
### CL-88 — 2026-08-24 · goal 0.20.1 selesai sisi Dev (⚠️ → 🔄 → 🔎 · 70 → 80%) — bukti smoke consumer reproducible
**Role:** AI-Dev · **Model:** ox-alpha-free (opencode)
**Bukti:** Test baru `smoke-consumers-reloc.test.ts` (5 lulus): KEEMPAT consumer (`smoke-global-mapping/pipeline/migrate-projects/rollback`) di-spawn sebagai proses nyata pada environment TANPA kredensial → exit 0 dengan output PASS/SKIP graceful (bukti import-path pasca-relokasi valid, reproducible di CI tanpa secret); + assert helper hasil relokasi terekspos penuh. Suite: **98 file / 596 test**; typecheck+lint bersih.
**Catatan:** Smoke yang membutuhkan Turso nyata tetap SKIP tanpa kredensial sesuai desain skrip; bukti live-provider menyusul di environment ber-kredensial (QA-CL-70 lingkungan envdev juga tidak memilikinya).

<a id="cl-89"></a>
### CL-89 — 2026-08-24 · goal 0.17.6 — blocker repo-lint diselesaikan; bukti lengkap (🔎 · 70 → 80%)
**Role:** AI-Dev · **Model:** ox-alpha-free (opencode)
**Bukti:** Blocker "lint repo-wide gagal 5 error" (laporan sesi QA) ditangani: 2 error milik Phase 5 & Phase 2 diperbaiki (lihat CL-19 PHASE-5 / CL-66 PHASE-2), sisanya bersih. Pasca-fix: `pnpm lint` **bersih**, full suite **98 file / 597 test PASS**, typecheck Done, smoke-migration PASS. Dengan seluruh verifikasi hijau, bukti goal lengkap → `%` dikoreksi 70 → 80 (Dev maksimal).
**Catatan:** Perubahan hanya pada file lint-milik fase lain + entry ini; substansi kontrak credential (CL-87) tidak tersentuh.

<a id="cl-82"></a>
### CL-82 — 2026-08-24 · TASK-0.21 (0.21.1+0.21.2+0.21.3) dikerjakan ulang (⚠️ → 🔄 · 70/60/60% dipertahankan) — remediasi tiga temuan QA-CL-68
**Role:** AI-Dev · **Model:** ox-alpha-free (opencode)
**Bukti:** Freshness check dari disk: ketiga row `⚠️` dengan QA-CL-68 (blocking findings) + QA-CL-69 (reproduksi host envdev). Temuan dipetakan: (1) harness `smoke-migration.ts` masih expect 16 tabel; (2) kontrak lama `IdempotencyStore.get/put` masih diekspor + wiring `withIdempotentHandling` belum menyeluruh; (3) test AC-034 belum memakai barrier/controllable handler.
**Catatan:** Perbaikan per temuan, tanpa mengubah semantik state machine yang sudah lulus verifikasi positif QA.

<a id="cl-81"></a>
### CL-81 — 2026-08-24 · TASK-0.20.2 batasi SELECT prune eligibility, koreksi pendekatan naif goal (⬜️ → 🔎 · 0 → 80%)
**Role:** AI-Dev · **Model:** claude-sonnet-5 (Claude Code)
**[TEMUAN PENTING — pendekatan literal goal SALAH, dikoreksi sebelum implementasi]:** Teks goal menyarankan menambah `WHERE deleted_at IS NOT NULL` polos di KEENAM query (milestones/boards/lists/cards/milestone_labels/board_labels), diklaim "bukan bug korektnes, murni efisiensi". **Analisis membuktikan klaim ini salah untuk 5 dari 6 tabel** (semua KECUALI `milestones`, satu-satunya tabel root tanpa ancestor): docstring `pruneDescendantSubtrees` sendiri menyatakan eksplisit "begitu sebuah root diputuskan prune, SELURUH descendant ikut terhapus fisik TERLEPAS `deleted_at`-nya sendiri" — konsisten 02-SPEC C.5 ("archive/delete Milestone hanya mengubah Milestone; Board/List/Card descendant mempertahankan local state", tidak ikut ter-set `deleted_at`). Descendant ACTIVE di bawah ancestor yang di-prune WAJIB tetap ikut ditemukan dan dihapus. Filter naif `deleted_at IS NOT NULL` di level board/list/card/label akan membuat baris ACTIVE tsb **TIDAK PERNAH termuat sama sekali** — orphan row permanen (Board/List/Card yatim tanpa ancestor), pelanggaran langsung terhadap invariant subtree-cascade yang sudah dikonfirmasi test `[BR-016][subtree cascade]` di `prune-descendants.test.ts:98`. Ini BUKAN cuma soal efisiensi — implementasi literal goal akan jadi REGRESI KOREKTNES data.
**Fix yang diimplementasikan (mempertahankan invariant, tetap capai tujuan efisiensi):** query tiap level dibatasi `WHERE deleted_at IS NOT NULL OR <parentColumn> IN (<id-set level-parent yang SUDAH diketahui prune-eligible>)`, dihitung BERURUTAN level-demi-level (milestones → boards → lists/milestone_labels → cards/board_labels — pola sama seperti alur cascade JS yang sudah ada, cuma filter-nya didorong ke SQL). `milestones` sendiri (root, tanpa parent) dibatasi murni `WHERE deleted_at IS NOT NULL` — SATU-SATUNYA tabel yang saran goal literal benar. Fungsi baru `selectCandidateRows()` (helper reusable, `packages/infrastructure/src/database/prune.ts`) menggantikan `selectRows()` polos untuk keenam query; `selectRows()` dipertahankan sebagai primitif generik (masih dipakai `selectCandidateRows`).
**Hasil:** kasus umum (tidak ada Milestone yang di-prune hari itu) sekarang HANYA memuat baris yang sudah soft-deleted per tabel — bukan full-table-scan seluruh baris ACTIVE Project besar seperti sebelumnya. Kasus cascade (Milestone di-prune) tetap memuat SELURUH descendant relevan (ACTIVE maupun DELETED) — korektnes 100% dipertahankan, TIDAK ada perubahan pada `PruneResult` akhir maupun urutan DELETE.
**Kenapa TIDAK ada `git stash` regression proof:** ini optimisasi query MURNI — perilaku/hasil akhir byte-for-byte identik dengan sebelumnya (dibuktikan seluruh test existing PASS tanpa modifikasi SATU PUN, termasuk skenario kritis `[BR-016][subtree cascade] ... walau descendant ACTIVE` yang jadi bukti utama korektnes cascade tetap terjaga). Tidak ada bug untuk dibuktikan regresinya — kode LAMA sudah benar secara fungsional, hanya boros; kode BARU sama benarnya, cuma lebih hemat. Konsisten prinsip CL-72/CL-80 untuk perubahan struktural/optimisasi murni.
**Verifikasi:** `pnpm -r typecheck` bersih 6/6; `pnpm exec vitest run` → 95 file/577 test PASS (TIDAK BERUBAH dari sebelumnya, nol modifikasi test — bukti kuat bahwa perilaku benar-benar tidak berubah); `pnpm lint` bersih.
**Catatan:** tidak ada test BARU ditambahkan karena tidak ada perilaku baru untuk diuji — cakupan korektnes cascade (termasuk skenario descendant ACTIVE) SUDAH ada dan sudah cukup di `prune-descendants.test.ts`/`prune-no-orphan.test.ts` (pre-existing, TIDAK disentuh).
